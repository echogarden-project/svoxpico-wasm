...
static pico_status_t klexInitialize(register picoknow_KnowledgeBase this, picoos_Common common) {
	...
	if (PICO_OK == picoos_read_mem_pi_uint16(this->base, &curpos, &(klex->nrblocks))) {
		if (klex->nrblocks > 0) {
		...
		klex->lexblocks =
			this->base + PICOKLEX_LEX_NRBLOCKS_SIZE + (klex->nrblocks * (PICOKLEX_LEX_SIE_SIZE));
		return PICO_OK;
	} else {
...
static picoos_uint32 klex_getSearchIndexVal(const klex_SubObj this, picoos_uint16 index) {
...
static picoos_uint16 klex_getLexblockNr(const klex_SubObj this, const picoos_uint8* graphsi) {
...
#if defined(PICO_DEBUG)
	{
		...
		PICODBG_DEBUG(("binary search result is %c%c%c (%d)", this->searchind[pos],
					   this->searchind[pos + 1], this->searchind[pos + 2], low));
	}
#endif
...
static picoos_uint16 klex_getLexblockRange(const klex_SubObj this, picoos_uint16 index) {
	picoos_uint16 count;
...
static picoos_int8 klex_lexMatch(picoos_uint8* lexentry,
	...
	for (i = 0; (i < graphlen) && (i < lexlen); i++) {
		PICODBG_TRACE(("%d|%d  graph|lex: %c|%c", graphlen, lexlen, graph[i], lexgraph[i]));
	...
	while ((rv < 0) && (lexpos < lexposEnd)) {
		...
		if (rv == 0) { /* found */
			...
			if (lexres->phonfound) {
				...
				while ((lexres->nrres < PICOKLEX_MAX_NRRES) && (lexpos < lexposEnd)) {
					...
					if (lexpos < lexposEnd) {
						if (klex_lexMatch(&(this->lexblocks[lexpos]), graph, graphlen) == 0) {
							klex_setLexResult(&(this->lexblocks[lexpos]), lexpos, lexres);
						} else {
