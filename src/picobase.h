...
#define picobase_det_utf8_length(x)                     \
	(((x) < (picoos_uint8)'\200')                       \
		 ? 1                                            \
		 : (((x) >= (picoos_uint8)'\370')               \
				? 0                                     \
 ...
 */
picoos_int32 picobase_lowercase_utf8_str(picoos_uchar utf8str[],
										 picoos_char lowercase[],
										 picoos_int32 lowercaseMaxLen,
										 picoos_uint8* done);
 ...
 * @return  TRUE if successful, FALSE otherwise
 */
picoos_int32 picobase_uppercase_utf8_str(picoos_uchar utf8str[],
 ...
 * @param    utf8char : zero terminated UTF8 character containing 1 to 4 bytes (output)
 * @return  TRUE if okay
 * @return  FALSE if there is no valid UTF8 char preceeding pos or no more UTF8 char available
 * within utf8len
 */
