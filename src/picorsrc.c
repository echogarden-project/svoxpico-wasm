...
typedef struct picorsrc_resource {
	...
	/* next connects all active resources of a resource manager and the garbaged resources of the
	 * manager's free list */
...
#define SET_MAGIC_NUMBER(res) (res)->magic = ((picoos_uint32)(res)) ^ MAGIC_MASK

#define CHECK_MAGIC_NUMBER(res) ((res)->magic == (((picoos_uint32)(res)) ^ MAGIC_MASK))
...
/*
static void picorsrc_disposeVoiceDefinition(picoos_MemoryManager mm, picorsrc_VoiceDefinition *
this)
{
	if (NULL != (*this)) {
...
picorsrc_ResourceManager picorsrc_newResourceManager(
	picoos_MemoryManager mm,
	picoos_Common common /* , picoos_char * configFile */) {
...
static pico_status_t findResource(picorsrc_ResourceManager this,
...
static pico_status_t parse_resource_name(picoos_char* fileName) {
	...
	if (picoos_has_extension(fileName, (picoos_char*)PICO_BIN_EXTENSION)) {
		return PICO_OK;
	} else {
...
static pico_status_t readHeader(picorsrc_ResourceManager this,
	...
	if (PICO_OK == status) {
		...
		return picoos_emRaiseException(this->common->em, status, NULL,
									   (picoos_char*)"problem reading file header");
	}
...
static pico_status_t picorsrc_createKnowledgeBase(picorsrc_ResourceManager this,
												  picoos_uint8* data,
												  picoos_uint32 size,
	...
	switch (kbid) {
		...
		case PICOKNOW_KBID_DT_POSP:
			return picokdt_specializeDtKnowledgeBase(*kb, this->common, PICOKDT_KDTTYPE_POSP);
			break;
		case PICOKNOW_KBID_DT_POSD:
			return picokdt_specializeDtKnowledgeBase(*kb, this->common, PICOKDT_KDTTYPE_POSD);
			break;
		case PICOKNOW_KBID_DT_G2P:
			return picokdt_specializeDtKnowledgeBase(*kb, this->common, PICOKDT_KDTTYPE_G2P);
			break;
		case PICOKNOW_KBID_DT_PHR:
			return picokdt_specializeDtKnowledgeBase(*kb, this->common, PICOKDT_KDTTYPE_PHR);
			break;
		case PICOKNOW_KBID_DT_ACC:
			return picokdt_specializeDtKnowledgeBase(*kb, this->common, PICOKDT_KDTTYPE_ACC);
			break;
		...
		case PICOKNOW_KBID_DT_MGC5:
			return picokdt_specializeDtKnowledgeBase(*kb, this->common, PICOKDT_KDTTYPE_PAM);
			break;
		case PICOKNOW_KBID_PDF_DUR:
			return picokpdf_specializePdfKnowledgeBase(*kb, this->common, PICOKPDF_KPDFTYPE_DUR);

			break;
		case PICOKNOW_KBID_PDF_LFZ:
			return picokpdf_specializePdfKnowledgeBase(*kb, this->common, PICOKPDF_KPDFTYPE_MUL);
			break;
		case PICOKNOW_KBID_PDF_MGC:
			return picokpdf_specializePdfKnowledgeBase(*kb, this->common, PICOKPDF_KPDFTYPE_MUL);
			break;
		case PICOKNOW_KBID_PDF_PHS:
			return picokpdf_specializePdfKnowledgeBase(*kb, this->common, PICOKPDF_KPDFTYPE_PHS);
			break;
...
static pico_status_t picorsrc_releaseKnowledgeBase(picorsrc_ResourceManager this,
												   picoknow_KnowledgeBase* kb) {
	(*kb) = NULL;
	return PICO_OK;
}
...
static pico_status_t picorsrc_getKbList(picorsrc_ResourceManager this,
	...
	*kbList = NULL;
	...
	while ((PICO_OK == status) && (i++ < numKbs)) {
		status = (picoos_get_str((picoos_char*)data, &curpos, str, PICOOS_MAX_FIELD_STRING_LEN))
					 ? PICO_OK
	...
	while ((PICO_OK == status) && (i++ < numKbs)) {
		...
		if (PICO_OK == status) {
			if (0 == offset) {
				/* currently we consider a kb mentioned in resource but with offset 0 (no knowledge)
				 * as different form a kb not mentioned at all. We might reconsider that later. */
				PICODBG_DEBUG((" kb (id %i) is mentioned but empty (base:%i, size:%i)", kb->id,
							   kb->base, kb->size));
				status =
					picorsrc_createKnowledgeBase(this, NULL, size, (picoknow_kb_id_t)kbid, &kb);
			} else {
				status = picorsrc_createKnowledgeBase(this, data + offset, size,
													  (picoknow_kb_id_t)kbid, &kb);
			}
			PICODBG_DEBUG(
				("found kb (id %i) starting at %i with size %i", kb->id, kb->base, kb->size));
			if (PICO_OK == status) {
				kb->next = *kbList;
				*kbList = kb;
			}
...
pico_status_t picorsrc_loadResource(picorsrc_ResourceManager this,
	...
	if (PICO_MAX_NUM_RESOURCES <= this->numResources) {
		...
		return picoos_emRaiseException(this->common->em, PICO_EXC_MAX_NUM_EXCEED, NULL,
	...
	if (!picoos_OpenBinary(this->common, &res->file, fileName)) {
		...
		picoos_emRaiseException(this->common->em, PICO_EXC_CANT_OPEN_FILE, NULL, (picoos_char*)"%s",
								fileName);
	}
	if (PICO_OK == status) {
	...
	if (PICO_OK == status && isResourceLoaded(this, header.field[PICOOS_HEADER_NAME].value)) {
		...
		picoos_emRaiseWarning(this->common->em, PICO_WARN_RESOURCE_DOUBLE_LOAD, NULL,
							  (picoos_char*)"%s", header.field[PICOOS_HEADER_NAME].value);
	...
	if (PICO_OK == status) {
		...
		if (PICO_OK == status) {
			...
			status = (picoos_ReadBytes(res->file, res->start, &len)) ? PICO_OK : PICO_ERR_OTHER;
		...
		if (PICO_OK == status) {
			if (picoos_strlcpy(res->name, header.field[PICOOS_HEADER_NAME].value,
				...
				PICODBG_ERROR(("failed assigning name %s to resource", res->name));
				picoos_emRaiseException(this->common->em, PICO_ERR_INDEX_OUT_OF_RANGE, NULL,
...
static pico_status_t picorsrc_releaseKbList(picorsrc_ResourceManager this,
											picoknow_KnowledgeBase* kbList) {
...
pico_status_t picorsrc_createDefaultResource(picorsrc_ResourceManager this
	...
	if (picoos_strlcpy(res->name, PICOKNOW_DEFAULT_RESOURCE_NAME, PICORSRC_MAX_RSRC_NAME_SIZ) <
	...
	status = picorsrc_createKnowledgeBase(this, NULL, 0, (picoknow_kb_id_t)PICOKNOW_KBID_FIXED_IDS,
										  &res->kbList);
...
pico_status_t picorsrc_addResourceToVoiceDefinition(picorsrc_ResourceManager this,
	...
	if ((PICO_OK == findVoiceDefinition(this, voiceName, &vdef)) && (NULL != vdef)) {
		if (PICO_MAX_NUM_RSRC_PER_VOICE <= vdef->numResources) {
			return picoos_emRaiseException(this->common->em, PICO_EXC_MAX_NUM_EXCEED, NULL,
										   (picoos_char*)"no more than %i resources per voice",
		...
		if (picoos_strlcpy(vdef->resourceName[vdef->numResources++], resourceName,
			...
			return picoos_emRaiseException(this->common->em, PICO_EXC_NAME_ILLEGAL, NULL,
										   (picoos_char*)"%s", resourceName);
		}

	} else {
		return picoos_emRaiseException(this->common->em, PICO_EXC_NAME_UNDEFINED, NULL,
									   (picoos_char*)"%s", voiceName);
	}
}
...
pico_status_t picorsrc_createVoiceDefinition(picorsrc_ResourceManager this,
	...
	if (PICO_MAX_NUM_VOICE_DEFINITIONS <= this->numVdefs) {
		...
		return picoos_emRaiseException(this->common->em, PICO_EXC_MAX_NUM_EXCEED, NULL,
									   (picoos_char*)"no more than %i voice definitions",
	...
	if (picoos_strlcpy(vdef->voiceName, voiceName, PICO_MAX_VOICE_NAME_SIZE) <
		PICO_MAX_VOICE_NAME_SIZE) {
		vdef->next = this->vdefs;
		this->vdefs = vdef;
		this->numVdefs++;
		if (PICO_OK != picorsrc_addResourceToVoiceDefinition(this, voiceName,
															 PICOKNOW_DEFAULT_RESOURCE_NAME)) {
			return picoos_emRaiseException(this->common->em, PICO_ERR_OTHER, NULL,
										   (picoos_char*)"problem loading default resource %s",
										   voiceName);
		}
...
pico_status_t picorsrc_createVoice(picorsrc_ResourceManager this,
								   const picoos_char* voiceName,
	...
	if (PICORSRC_MAX_NUM_VOICES <= this->numVoices) {
		...
		return picoos_emRaiseException(this->common->em, PICO_EXC_MAX_NUM_EXCEED, NULL,
									   (picoos_char*)"no more than %i voices",
									   PICORSRC_MAX_NUM_VOICES);
	}
	...
	if (!(PICO_OK == findVoiceDefinition(this, voiceName, &vdef)) || (NULL == vdef)) {
		...
		return picoos_emRaiseException(this->common->em, PICO_EXC_NAME_UNDEFINED, NULL,
									   (picoos_char*)"voice definition %s", voiceName);
	}
	...
	for (i = 0; i < vdef->numResources; i++) {
		...
		if (required && !isResourceLoaded(this, vdef->resourceName[i])) {
			...
			return picoos_emRaiseException(this->common->em, PICO_EXC_RESOURCE_MISSING, NULL,
										   (picoos_char*)"resource %s for voice %s",
										   vdef->resourceName[i], voiceName);
		}
	}
		...
		if (required) {
			...
			while (NULL != kb) {
				if (NULL != (*voice)->kbArray[kb->id]) {
					picoos_emRaiseWarning(this->common->em, PICO_WARN_KB_OVERWRITE, NULL,
										  (picoos_char*)"%i", kb->id);
