...
void picorsrc_disposeResourceManager(picoos_MemoryManager mm, picorsrc_ResourceManager* that);
...
/* load resource file. the type of resource file, magic numbers, checksum etc. are in the header,
 * then follows the directory (with fixed structure per resource type), then the knowledge bases
 * themselves (as byte streams) */
pico_status_t picorsrc_loadResource(picorsrc_ResourceManager that,
									picoos_char* fileName,
									picorsrc_Resource* resource);

/* unload resource file. (warn if resource file is busy) */
pico_status_t picorsrc_unloadResource(picorsrc_ResourceManager that, picorsrc_Resource* rsrc);

pico_status_t picorsrc_createDefaultResource(picorsrc_ResourceManager that /*,
        picorsrc_Resource * resource */);
...
pico_status_t picorsrc_createVoiceDefinition(picorsrc_ResourceManager that, picoos_char* voiceName);

pico_status_t picorsrc_releaseVoiceDefinition(picorsrc_ResourceManager that,
											  picoos_char* voiceName);

pico_status_t picorsrc_addResourceToVoiceDefinition(picorsrc_ResourceManager that,
													picoos_char* voiceName,
...
pico_status_t picorsrc_createVoice(picorsrc_ResourceManager that,
								   const picoos_char* voiceName,
								   picorsrc_Voice* voice);

/* dispose voice. the corresponding lock counts are decremented. */
pico_status_t picorsrc_releaseVoice(picorsrc_ResourceManager that, picorsrc_Voice* voice);
