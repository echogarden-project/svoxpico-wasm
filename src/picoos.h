...
/* "Common"                                                        */
/* *****************************************************************/
/* picoos_common is a collection of basic functionalities that must be globally accesible from every
 * "big" function. It includes pointers to the MemoryManasger, ExceptionManager and a list of open
 * files. */
...
/* the picoos_common structure itself is exported so no access functions are needed. Handle with
 * care! (might be changed later) */
...
void picoos_disposeCommon(picoos_MemoryManager mm, picoos_Common* that);
...
void* picoos_allocate(picoos_MemoryManager that, picoos_objsize_t byteSize);
void picoos_deallocate(picoos_MemoryManager that, void** adr);
...
void picoos_disposeExceptionManager(picoos_MemoryManager mm, picoos_ExceptionManager* that);

void picoos_emReset(picoos_ExceptionManager that);
 ...
 * the error! */
pico_status_t picoos_emRaiseException(picoos_ExceptionManager that,
									  pico_status_t exceptionCode,
									  picoos_char* baseMessage,
									  picoos_char* fmt,
									  ...);

pico_status_t picoos_emGetExceptionCode(picoos_ExceptionManager that);

void picoos_emGetExceptionMessage(picoos_ExceptionManager that,
								  picoos_char* msg,
								  picoos_uint16 maxsize);

void picoos_emRaiseWarning(picoos_ExceptionManager that,
						   pico_status_t warningCode,
						   picoos_char* baseMessage,
						   picoos_char* fmt,
						   ...);

picoos_uint8 picoos_emGetNumOfWarnings(picoos_ExceptionManager that);

pico_status_t picoos_emGetWarningCode(picoos_ExceptionManager that, picoos_uint8 warnNum);

void picoos_emGetWarningMessage(picoos_ExceptionManager that,
								picoos_uint8 warnNum,
								picoos_char* msg,
								picoos_uint16 maxsize);
...
picoos_uint8 picoos_ReadBytes(picoos_File f, picoos_uint8 bytes[], picoos_uint32* len);
...
picoos_uint8 picoos_WriteBytes(picoos_File f, const picoos_char bytes[], picoos_int32* len);
...
typedef enum { FILE_TYPE_WAV, FILE_TYPE_AU, FILE_TYPE_RAW, FILE_TYPE_OTHER } wave_file_type_t;
...
#define PICOOS_MAX_HEADER_STRING_LEN \
	(PICOOS_MAX_NUM_HEADER_FIELDS * (2 * PICOOS_MAX_FIELD_STRING_LEN))
...
/* 'stringlen' is the part of input string to be considered, possibly not containing NULLC (e.g.
 * result of strlen). 'maxsize' is the maximal size of 'part' including a byte for the terminating
 * NULLC! */
void picoos_get_sep_part_str(picoos_char string[],
							 picoos_int32 stringlen,
							 picoos_int32* ind,
...
/* searches for the first contiguous string of printable characters (> ' ') inside fromStr, possibly
 * skipping chars <= ' ') and returns it in toStr. fromStr is assumed to be NULLC terminated and
 * toStr is forced to be NULLC terminated within maxsize. The search is started at *pos inside
 * fromStr and at return, *pos is the position within fromStr after the found string, or the
 * position after the end of fromStr, if no string was found. the function returns TRUE if a string
 * was found and fitted toStr, or FALSE otherwise. */
