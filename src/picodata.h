...
void picodata_disposeCharBuffer(picoos_MemoryManager mm, picodata_CharBuffer* that);

/* should not be used for PUs but only for feeding the initial cb */
pico_status_t picodata_cbPutCh(register picodata_CharBuffer that, picoos_char ch);

/* should not be used for PUs other than first PU in the chain (picotok) */
picoos_int16 picodata_cbGetCh(register picodata_CharBuffer that);

/* reset cb (as if after newCharBuffer) */
pico_status_t picodata_cbReset(register picodata_CharBuffer that);
 ...
 * These values should be applied BEFORE POS-disambiguation. The POS lingware either assigns the
 * same ids to corresponding internal unique or composed POS or else the POS-D will consider these
 * values "default" */
...
#define PICODATA_ITEMINFO1_CMD_PLAY \
	'p' /* 112 play command : PU in info 2 will read items from file-->Filename in item content.*/
#define PICODATA_ITEMINFO1_CMD_SAVE \
	's' /* 115 save command : PU in info 2 will save items to file-->Filename in item content.*/
#define PICODATA_ITEMINFO1_CMD_UNSAVE \
	'u' /* 117 save command : PU in info 2 will stop saving items to file*/
#define PICODATA_ITEMINFO1_CMD_PROSDOMAIN \
	'd' /* 100 prosody domain : domain type in info 2, domain name in item content */
#define PICODATA_ITEMINFO1_CMD_SPELL                            \
	'e' /* 101 spell command : info 2 contains start/stop info, \
		  spell type/pause len as little endian uint16 in item content */
#define PICODATA_ITEMINFO1_CMD_IGNSIG \
	'i' /* ignore signal command : info 2 contains start/stop info */
#define PICODATA_ITEMINFO1_CMD_PHONEME \
	'o' /* phoneme command : info 2 contains start/stop info, phonemes in item content */
#define PICODATA_ITEMINFO1_CMD_IGNORE \
	'I' /* ignore text command : info 2 contains start/stop info */
#define PICODATA_ITEMINFO1_CMD_SIL                                                               \
	'z'									   /* silence command : info 2 contains type of silence; \
...
#define PICODATA_ITEMINFO1_CMD_PITCH                                               \
	'P' /* 80 pitch command : abs/rel info in info 2; pitch level as little endian \
		   uint16 in item content; relative value is in promille */
#define PICODATA_ITEMINFO1_CMD_SPEED                                               \
	'R' /* 82 speed command : abs/rel info in info 2, speed level as little endian \
		   uint16 in item content; elative value is in promille */
#define PICODATA_ITEMINFO1_CMD_VOLUME                                                \
	'V' /* 86 volume command : abs/rel info in info 2, volume level as little endian \
		   uint16 in item content; relative value is in promille */
#define PICODATA_ITEMINFO1_CMD_SPEAKER                                                 \
	'S' /* 83 speaker command : abs/rel info in info 2, speaker level as little endian \
		   uint16 in item content; relative value is in promille */
...
#if 0
#define PICODATA_ITEMINFO2_CMD_TO_FST \
...
*/
pico_status_t picodata_cbGetItem(register picodata_CharBuffer that,
								 picoos_uint8* buf,
								 const picoos_uint16 blenmax,
								 picoos_uint16* blen);
...
/* gets the speech data (without item head) from a CharBuffer in buf;
   ...
   set to the number of bytes gotten in buf; return values:
	 ...
	 PICO_EXC_BUF_OVERFLOW   <- buf not large enough
*/
pico_status_t picodata_cbGetSpeechData(register picodata_CharBuffer that,
									   picoos_uint8* buf,
									   const picoos_uint16 blenmax,
									   picoos_uint16* blen);
...
*/
pico_status_t picodata_cbPutItem(register picodata_CharBuffer that,
								 const picoos_uint8* buf,
								 const picoos_uint16 blenmax,
								 picoos_uint16* blen);

/* unsafe, just for measuring purposes */
picoos_uint8 picodata_cbGetFrontItemType(register picodata_CharBuffer that);
...
*/
pico_status_t picodata_get_itemparts_nowarn(const picoos_uint8* buf,
											const picoos_uint16 blenmax,
											picodata_itemhead_t* head,
											picoos_uint8* content,
											const picoos_uint16 clenmax,
											picoos_uint16* clen);
...
/* gets from buf a single item, values in head set and item content
   ...
   number of bytes gotten in content; return values:
	 ...
	 PICO_EXC_BUF_OVERFLOW   <- overflow in content
*/
pico_status_t picodata_get_itemparts(const picoos_uint8* buf,
									 const picoos_uint16 blenmax,
									 picodata_itemhead_t* head,
									 picoos_uint8* content,
									 const picoos_uint16 clenmax,
...
/* gets from buf info of a single item, values in head are set and
   ...
   max lengths (in number of bytes) accessible in buf; return values:
	 ...
	 PICO_EXC_BUF_UNDERFLOW  <- blenmax problem, or no valid item
*/
pico_status_t picodata_get_iteminfo(picoos_uint8* buf,
									const picoos_uint16 blenmax,
									picodata_itemhead_t* head,
									picoos_uint8** content);
...
pico_status_t picodata_copy_item(const picoos_uint8* inbuf,
								 const picoos_uint16 inlenmax,
								 picoos_uint8* outbuf,
								 const picoos_uint16 outlenmax,
								 picoos_uint16* numb);
...
typedef enum picodata_step_result {
	...
	PICODATA_PU_ATOMIC,	 /* same as pu_busy, but wants to get next time slot (while in an "atomar"
							operation) */
	PICODATA_PU_OUT_FULL /* can't proceed because output is full. (next time slot to be assigned to
							pu's output's consumer) */
} picodata_step_result_t;

typedef struct picodata_processing_unit* picodata_ProcessingUnit;

picodata_ProcessingUnit picodata_newProcessingUnit(picoos_MemoryManager mm,
												   picoos_Common common,
												   picodata_CharBuffer cbIn,
												   picodata_CharBuffer cbOut,
												   picorsrc_Voice voice);

void picodata_disposeProcessingUnit(picoos_MemoryManager mm, picodata_ProcessingUnit* that);

picodata_CharBuffer picodata_getCbIn(picodata_ProcessingUnit that);
picodata_CharBuffer picodata_getCbOut(picodata_ProcessingUnit that);
pico_status_t picodata_setCbIn(picodata_ProcessingUnit that, picodata_CharBuffer cbIn);
pico_status_t picodata_setCbOut(picodata_ProcessingUnit that, picodata_CharBuffer cbOut);

/* protected */
typedef pico_status_t (*picodata_puInitializeMethod)(register picodata_ProcessingUnit that,
													 picoos_int32 mode);
typedef pico_status_t (*picodata_puTerminateMethod)(register picodata_ProcessingUnit that);
typedef picodata_step_result_t (*picodata_puStepMethod)(register picodata_ProcessingUnit that,
														picoos_int16 mode,
														picoos_uint16* numBytesOutput);
typedef pico_status_t (*picodata_puSubDeallocateMethod)(register picodata_ProcessingUnit that,
