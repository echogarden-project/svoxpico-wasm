 ...
 * Conventions:
 *
 * - The input to the transducer is a list of pos/sym pairs, where pos are arbitrary position
 * markers
 * - All positions are allowed on input (in particular all those coming as an output of a previous
 * transduction)
 * - A phone sequence to be transduced has to begin with PICOKNOW_PHON_START_ID and end with
 * PICOKNOW_PHON_TERM_ID These special symbols are kept in the transduction output (as first and
 * last symbol)
 * - Symbols inserted by the transduction process allways get their position marker
 * pos=PICOTRNS_POS_INSERT
 * - The order of positions on output must be the same as that on input, i.e. apart from inserted
 * pairs, the output position sequence must be a sub-sequence of the input position sequence.
 * - Inserted symbols are allways preceded by a positioned pos/sym pair, e.g.
 *   if the sequence pos1/sym1, pos2/sym2 should be tranduced to x/sym3, y/sym4, z/sym5, then x must
 * be pos1 or pos2 and not PICOTRNS_POS_INSERT
 *
 ...
 *          - The output is pos1/phon1'  -1/phon_ins pos2/phon2'  [assuming -1 is the special
 * insertion pos]
 *     - Then the new sequence will be recomposed as phon1' , phon_ins , command , phon2'  [note
 * position of command!]
...
#define PICOTRNS_POS_INSERT \
	(picoos_int16) -        \
		1 /* position returned by transducer to mark symbols inserted by the transducer */
#define PICOTRNS_POS_INVALID \
	(picoos_int16) - 2 /* value to mark an invalid (e.g. uninitiated) position */
#define PICOTRNS_POS_IGNORE                                                                        \
	(picoos_int16) - 3 /* value to mark a pos/sym pair to be ignored (e.g. start/term symbols only \
						  used by the transducer) */
...
/* type of function for printing transduction solutions;
   ...
   are produced */
typedef void picotrns_printSolutionFct(const picotrns_possym_t outSeq[],
 ...
 */
extern pico_status_t picotrns_transduce(picokfst_FST fst,
...
/* transduce 'inSeq' into 'outSeq' 'inSeq' has to be terminated with the id for symbol '#'. 'outSeq'
 * is terminated in the same way. */
/*
pico_status_t picotrns_transduce_sequence(picokfst_FST fst, const picotrns_possym_t inSeq[],
picoos_uint16 inSeqLen, picotrns_possym_t outSeq[], picoos_uint16 * outSeqLen);
*/
...
pico_status_t picotrns_stAddWithPlane(picotrns_SimpleTransducer that,
									  picoos_char* inStr,
									  picoos_uint8 plane);

pico_status_t picotrns_stTransduce(picotrns_SimpleTransducer that, picokfst_FST fst);

pico_status_t picotrns_stGetSymSequence(picotrns_SimpleTransducer that,
