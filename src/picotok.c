...
static void tok_treatChar(picodata_ProcessingUnit this,
						  tok_subobj_t* tok,
						  picoos_uchar ch,
						  picoos_bool markupHandling);
...
static void tok_getParamIntVal(MarkupParams params,
							   picoos_uchar paramId[],
							   picoos_int32* paramVal,
...
static void tok_getParamStrVal(MarkupParams params,
							   picoos_uchar paramId[],
							   picoos_uchar paramStrVal[],
							   picoos_bool* paramFound) {
	int i = 0;
...
static void tok_getParamPhonesStr(MarkupParams params,
								  picoos_uchar paramId[],
								  picoos_uchar alphabet[],
								  picoos_uchar phones[],
								  picoos_int32 phoneslen,
								  picoos_bool* paramFound) {
	int i;
	picoos_bool done;
...
static void tok_putItem(picodata_ProcessingUnit this,
						tok_subobj_t* tok,
...
static void tok_putItem2(picodata_ProcessingUnit this,
						 tok_subobj_t* tok,
						 picoos_uint8 type,
						 picoos_uint8 info1,
						 picoos_uint8 info2,
...
static MarkupId tok_markupTagId(picoos_uchar tagId[]) {
	...
	if (tok_strEqual(tagId, TOK_MARKUP_KW_IGNORE)) {
		...
		return MIGenFile;
	} else if (tok_strEqual(tagId, TOK_MARKUP_KW_SENTENCE) ||
			   tok_strEqual(tagId, TOK_MARKUP_KW_S)) {
		return MISentence;
	} else if (tok_strEqual(tagId, TOK_MARKUP_KW_PARAGRAPH) ||
			   tok_strEqual(tagId, TOK_MARKUP_KW_P)) {
		return MIParagraph;
...
static void tok_checkLimits(picodata_ProcessingUnit this,
							picoos_uint32* value,
							picoos_uint32 min,
							picoos_uint32 max,
							picoos_uchar valueType[]) {
	if ((((*value) < min) || ((*value) > max))) {
		picoos_emRaiseWarning(
			this->common->em, PICO_ERR_MARKUP_VALUE_OUT_OF_RANGE, (picoos_char*)"",
			(picoos_char*)"attempt to set illegal value %i for %s", *value, valueType);
		if (((*value) < min)) {
			(*value) = min;
		} else if (((*value) > max)) {
			(*value) = max;
...
static void tok_checkRealLimits (picodata_ProcessingUnit this, picoos_single * value, picoos_single
min, picoos_single max, picoos_uchar valueType[])
{
	if ((((*value) < min) || ((*value) > max))) {
		  picoos_emRaiseWarning(this->common->em, PICO_ERR_MARKUP_VALUE_OUT_OF_RANGE,
(picoos_char*)"", (picoos_char*)"attempt to set illegal value %f for %s", *value, valueType); if
(((*value) < min)) {
			(*value) = min;
		} else if (((*value) > max)) {
			(*value) = max;
...
static void tok_interpretMarkup(picodata_ProcessingUnit this,
								tok_subobj_t* tok,
								picoos_bool isStartTag,
								MarkupId mId) {
	picoos_bool done;
	picoos_int32 ival;
	picoos_uint32 uval;
	...
	switch (mId) {
		case MIIgnore:
			if (isStartTag && tok_strEqual(tok->markupParams[0].paramId, (picoos_uchar*)"")) {
				tok_startIgnore(tok);
					   ...
					   tok_strEqual(tok->markupParams[0].paramId, (picoos_uchar*)"")) {
				tok_endIgnore(tok);
				done = TRUE;
			}
			break;
		case MISpeed:
			if (isStartTag && tok_strEqual(tok->markupParams[0].paramId, KWLevel)) {
				if (tok_isRelative(tok->markupParams[0].paramVal, &uval)) {
					tok_checkLimits(this, &uval, PICO_SPEED_FACTOR_MIN, PICO_SPEED_FACTOR_MAX,
									(picoos_uchar*)"relative speed factor");
					tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_SPEED,
								PICODATA_ITEMINFO2_CMD_RELATIVE, uval, (picoos_uchar*)"");
				} else {
					uval = picoos_atoi((picoos_char*)tok->markupParams[0].paramVal);
					tok_checkLimits(this, &uval, PICO_SPEED_MIN, PICO_SPEED_MAX,
									(picoos_uchar*)"speed");
					tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_SPEED,
								PICODATA_ITEMINFO2_CMD_ABSOLUTE, uval, (picoos_uchar*)"");
				}
				done = TRUE;
			} else if (!isStartTag &&
					   tok_strEqual(tok->markupParams[0].paramId, (picoos_uchar*)"")) {
				tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_SPEED,
							PICODATA_ITEMINFO2_CMD_ABSOLUTE, PICO_SPEED_DEFAULT, (picoos_uchar*)"");
				done = TRUE;
			}
			break;
		case MIPitch:
			if (isStartTag && tok_strEqual(tok->markupParams[0].paramId, KWLevel)) {
				if (tok_isRelative(tok->markupParams[0].paramVal, &uval)) {
					tok_checkLimits(this, &uval, PICO_PITCH_FACTOR_MIN, PICO_PITCH_FACTOR_MAX,
									(picoos_uchar*)"relative pitch factor");
					tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_PITCH,
								PICODATA_ITEMINFO2_CMD_RELATIVE, uval, (picoos_uchar*)"");
				} else {
					uval = picoos_atoi((picoos_char*)tok->markupParams[0].paramVal);
					tok_checkLimits(this, &uval, PICO_PITCH_MIN, PICO_PITCH_MAX,
									(picoos_uchar*)"pitch");
					tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_PITCH,
								PICODATA_ITEMINFO2_CMD_ABSOLUTE, uval, (picoos_uchar*)"");
				}
					   ...
					   tok_strEqual(tok->markupParams[0].paramId, (picoos_uchar*)"")) {
				tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_PITCH,
							PICODATA_ITEMINFO2_CMD_ABSOLUTE, PICO_PITCH_DEFAULT, (picoos_uchar*)"");
				done = TRUE;
			}
			break;
		case MIVolume:
			if (isStartTag && tok_strEqual(tok->markupParams[0].paramId, KWLevel)) {
				if (tok_isRelative(tok->markupParams[0].paramVal, &uval)) {
					tok_checkLimits(this, &uval, PICO_VOLUME_FACTOR_MIN, PICO_VOLUME_FACTOR_MAX,
									(picoos_uchar*)"relative volume factor");
					tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_VOLUME,
								PICODATA_ITEMINFO2_CMD_RELATIVE, uval, (picoos_uchar*)"");
				} else {
					uval = picoos_atoi((picoos_char*)tok->markupParams[0].paramVal);
					tok_checkLimits(this, &uval, PICO_VOLUME_MIN, PICO_VOLUME_MAX,
									(picoos_uchar*)"volume");
					tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_VOLUME,
								PICODATA_ITEMINFO2_CMD_ABSOLUTE, uval, (picoos_uchar*)"");
				}
					   ...
					   tok_strEqual(tok->markupParams[0].paramId, (picoos_uchar*)"")) {
				tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_VOLUME,
							PICODATA_ITEMINFO2_CMD_ABSOLUTE, PICO_VOLUME_DEFAULT,
							(picoos_uchar*)"");
				done = TRUE;
			}
			break;
		case MISpeaker:
			if (isStartTag && tok_strEqual(tok->markupParams[0].paramId, KWLevel)) {
				if (tok_isRelative(tok->markupParams[0].paramVal, &uval)) {
					tok_checkLimits(this, &uval, PICO_SPEAKER_FACTOR_MIN, PICO_SPEAKER_FACTOR_MAX,
									(picoos_uchar*)"relative speaker factor");
					tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_SPEAKER,
								PICODATA_ITEMINFO2_CMD_RELATIVE, uval, (picoos_uchar*)"");
				} else {
					uval = picoos_atoi((picoos_char*)tok->markupParams[0].paramVal);
					tok_checkLimits(this, &uval, PICO_SPEAKER_MIN, PICO_SPEAKER_MAX,
									(picoos_uchar*)"volume");
					tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_SPEAKER,
								PICODATA_ITEMINFO2_CMD_ABSOLUTE, uval, (picoos_uchar*)"");
				}
					   ...
					   tok_strEqual(tok->markupParams[0].paramId, (picoos_uchar*)"")) {
				tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_SPEAKER,
							PICODATA_ITEMINFO2_CMD_ABSOLUTE, PICO_SPEAKER_DEFAULT,
							(picoos_uchar*)"");
				done = TRUE;
			}
			break;
		...
		case MIVoice:
			if (isStartTag && tok_strEqual(tok->markupParams[0].paramId, KWName)) {
				tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_VOICE,
							PICODATA_ITEMINFO2_NA, 0, tok->markupParams[0].paramVal);
				tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_FLUSH,
							PICODATA_ITEMINFO2_NA, 0, (picoos_uchar*)"");
				tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_PROSDOMAIN, 0, 0,
							(picoos_uchar*)"");
				done = TRUE;
			} else if (!isStartTag &&
					   tok_strEqual(tok->markupParams[0].paramId, (picoos_uchar*)"")) {
		...
		case MIPreprocContext:
			if (isStartTag && tok_strEqual(tok->markupParams[0].paramId, KWName)) {
				tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_CONTEXT,
							PICODATA_ITEMINFO2_NA, 0, tok->markupParams[0].paramVal);
				done = TRUE;
			} else if (!isStartTag &&
					   tok_strEqual(tok->markupParams[0].paramId, (picoos_uchar*)"")) {
				tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_CONTEXT,
							PICODATA_ITEMINFO2_NA, 0, PICO_CONTEXT_DEFAULT);
				done = TRUE;
			}
			break;
		case MIMarker:
			if (isStartTag && tok_strEqual(tok->markupParams[0].paramId, KWName)) {
				tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_MARKER,
							PICODATA_ITEMINFO2_NA, 0, tok->markupParams[0].paramVal);
				done = TRUE;
			} else if (!isStartTag &&
					   tok_strEqual(tok->markupParams[0].paramId, (picoos_uchar*)"")) {
		...
		case MISentence:
			if (isStartTag) {
				tok_getParamStrVal(tok->markupParams, KWProsDomain, (picoos_uchar*)valStr,
								   &paramFound);
				tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_FLUSH,
							PICODATA_ITEMINFO2_NA, 0, (picoos_uchar*)"");
				tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_PROSDOMAIN, 2, 0,
							valStr);
				done = TRUE;
			} else if (!isStartTag &&
					   tok_strEqual(tok->markupParams[0].paramId, (picoos_uchar*)"")) {
		...
		case MIParagraph:
			if (isStartTag) {
				...
				tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_FLUSH,
							PICODATA_ITEMINFO2_NA, 0, (picoos_uchar*)"");
				tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_PROSDOMAIN, 1, 0,
							valStr);
				done = TRUE;
			} else if (!isStartTag &&
					   tok_strEqual(tok->markupParams[0].paramId, (picoos_uchar*)"")) {
				tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_FLUSH,
							PICODATA_ITEMINFO2_NA, 0, (picoos_uchar*)"");
				tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_SIL,
							PICODATA_ITEMINFO2_NA, PARAGRAPH_PAUSE_DUR, (picoos_uchar*)"");
				tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_PROSDOMAIN, 1, 0,
							(picoos_uchar*)"");
				done = TRUE;
			}
		...
		case MIBreak:
			if (isStartTag && tok_strEqual(tok->markupParams[0].paramId, KWTime)) {
				...
				if (done1) {
					tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_SIL,
								PICODATA_ITEMINFO2_NA, dur, (picoos_uchar*)"");
					done = TRUE;
				}
			} else if (!isStartTag &&
					   tok_strEqual(tok->markupParams[0].paramId, (picoos_uchar*)"")) {
		...
		case MISpell:
			if (isStartTag) {
				...
				tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_SPELL,
							PICODATA_ITEMINFO2_CMD_START, uval, (picoos_uchar*)"");
				done = TRUE;
			} else if (!isStartTag &&
					   tok_strEqual(tok->markupParams[0].paramId, (picoos_uchar*)"")) {
				tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_SPELL,
							PICODATA_ITEMINFO2_CMD_END, 0, (picoos_uchar*)"");
				done = TRUE;
			}
			break;
		case MIGenFile:
			if (isStartTag && tok_strEqual(tok->markupParams[0].paramId, KWFile)) {
				if (tok->saveFile[0] != 0) {
					tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_UNSAVE,
								picodata_getPuTypeFromExtension(tok->saveFile, /*input*/ FALSE), 0,
								tok->saveFile);
					tok->saveFile[0] = 0;
				}
				tok_putItem(
					this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_SAVE,
					picodata_getPuTypeFromExtension(tok->markupParams[0].paramVal, /*input*/ FALSE),
					0, tok->markupParams[0].paramVal);
				picoos_strcpy((picoos_char*)tok->saveFile,
							  (picoos_char*)tok->markupParams[0].paramVal);
				done = TRUE;
			} else if (!isStartTag &&
					   tok_strEqual(tok->markupParams[0].paramId, (picoos_uchar*)"")) {
				if (tok->saveFile[0] != 0) {
		...
		case MIPlay:
			if (isStartTag && tok_strEqual(tok->markupParams[0].paramId, KWFile)) {
				if (picoos_FileExists(this->common, (picoos_char*)tok->markupParams[0].paramVal)) {
					...
					tok_getParamPhonesStr(tok->markupParams, KWXFadeBeg, valStr3, valStr,
										  VAL_STR_LEN, &paramFound);
					tok_getParamPhonesStr(tok->markupParams, KWXFadeEnd, valStr3, valStr2,
										  VAL_STR_LEN, &paramFound);
					tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_PLAY,
								picodata_getPuTypeFromExtension(tok->markupParams[0].paramVal,
																/*input*/ TRUE),
								0, tok->markupParams[0].paramVal);
					tok_startIgnore(tok);
				} else {
				...
				if (picoos_FileExists(this->common, (picoos_char*)tok->markupParams[0].paramVal)) {
					...
					tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_PLAY,
								picodata_getPuTypeFromExtension(tok->markupParams[0].paramVal,
																/*input*/ TRUE),
								0, tok->markupParams[0].paramVal);
					tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_IGNSIG,
								PICODATA_ITEMINFO2_CMD_START, 0, (picoos_uchar*)"");
				} else {
					if (tok->ignLevel <= 0) {
						picoos_emRaiseWarning(
		...
		case MIPhoneme:
			i2 = 0;
			if (isStartTag) {
				if (tok_strEqual(tok->markupParams[0].paramId, KWAlphabet) &&
					tok_strEqual(tok->markupParams[1].paramId, KWPH)) {
					if (tok_strEqual(tok->markupParams[2].paramId, KWOrthMode) &&
						tok_strEqual(tok->markupParams[2].paramVal, KWIgnorePunct)) {
						i2 = 1;
					}
					if (picodata_mapPAStrToPAIds(tok->transducer, this->common, tok->xsampa_parser,
												 tok->svoxpa_parser, tok->xsampa2svoxpa_mapper,
												 tok->markupParams[1].paramVal,
												 tok->markupParams[0].paramVal, tok->phonemes,
												 sizeof(tok->phonemes) - 1) == PICO_OK) {
						tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_PHONEME,
									PICODATA_ITEMINFO2_CMD_START, i2, tok->phonemes);
						done = TRUE;
					} else {
						PICODBG_WARN(("cannot map phonetic string '%s'; synthesizeing text instead",
									  tok->markupParams[1].paramVal));
					...
					if (picodata_mapPAStrToPAIds(tok->transducer, this->common, tok->xsampa_parser,
												 tok->svoxpa_parser, tok->xsampa2svoxpa_mapper,
												 tok->markupParams[0].paramVal, PICODATA_XSAMPA,
												 tok->phonemes, sizeof(tok->phonemes)) == PICO_OK) {
						tok_putItem(this, tok, PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_PHONEME,
									PICODATA_ITEMINFO2_CMD_START, i2, tok->phonemes);
						done = TRUE;
					} else {
		...
		case MIItem:
			if (isStartTag && tok_strEqual(tok->markupParams[0].paramId, KWType) &&
				...
				while (n < len2) {
					while ((tok->markupParams[3].paramVal[n] != 0) &&
						   (tok->markupParams[3].paramVal[n] <= 32)) {
						n++;
					}
				...
				if (is_valid_itemtype(type)) {
					...
					len = 0;
					pos = 0;
					picoos_get_sep_part_str(tok->markupParams[3].paramVal,
											picoos_strlen(tok->markupParams[3].paramVal), &pos, ',',
											part, 10, &done1);
					while (done && done1) {
						n = picoos_atoi(part);
...
static void tok_putToSimpleToken(picodata_ProcessingUnit this,
								 tok_subobj_t* tok,
								 picoos_uchar str[],
								 pico_tokenType type,
								 pico_tokenSubType subtype) {
	int i, len;
	...
	if (str[0] != 0) {
		...
		for (i = 0; i < len; i++) {
			if (tok->tokenPos >= IN_BUF_SIZE) {
				picoos_emRaiseWarning(this->common->em, PICO_ERR_INTERNAL_LIMIT, (picoos_char*)"",
									  (picoos_char*)"simple token too long; forced treatment");
...
static void tok_putToMarkup(picodata_ProcessingUnit this, tok_subobj_t* tok, picoos_uchar str[]) {
	...
	for (i = 0; i < len; i++) {
		if (tok->markupPos >= (MARKUP_STRING_BUF_SIZE - 1)) {
			if ((tok->markupPos == (MARKUP_STRING_BUF_SIZE - 1)) &&
				(tok_markupTagId(tok->markupTagName) != MIDummyEnd)) {
				picoos_emRaiseWarning(this->common->em, PICO_ERR_INTERNAL_LIMIT, (picoos_char*)"",
									  (picoos_char*)"markup tag too long");
			}
			tok->markupState = MSErrorTooLong;
				   ...
				   ((tok->markupState == MSExpectingmarkupTagName) ||
					...
					(tok->markupState == MSGotAttrName) || (tok->markupState == MSGotEqual) ||
					(tok->markupState == MSGotAttrValue))) {
		} else if ((str[i] == (picoos_uchar)'>') && ((tok->markupState == MSGotmarkupTagName) ||
			...
			switch (tok->markupState) {
				...
				case MSGotStart:
					...
					break;
				case MSInmarkupTagName:
				case MSExpectingmarkupTagName:
					if (tok_idChar(str[i], tok->markupState == MSExpectingmarkupTagName)) {
						tok->markupTagName[tok->strPos] = str[i];
						tok->strPos++;
						tok->markupTagName[(tok->strPos)] = 0;
						tok->markupState = MSInmarkupTagName;
					} else if ((tok->markupState == MSInmarkupTagName) &&
							   (str[i] == (picoos_uchar)' ')) {
						tok->markupState = MSGotmarkupTagName;
						picobase_lowercase_utf8_str(
							tok->markupTagName, (picoos_char*)tok->markupTagName, IN_BUF_SIZE, &ok);
						tok->strPos = 0;
					} else {
					...
					if (tok_attrChar(str[i], TRUE)) {
						if (tok->markupTagType == MTEnd) {
							...
							if (tok->nrMarkupParams < MAX_NR_MARKUP_PARAMS) {
								tok->markupParams[tok->nrMarkupParams].paramId[tok->strPos] =
									str[i];
								tok->strPos++;
								tok->markupParams[tok->nrMarkupParams].paramId[tok->strPos] = 0;
							} else {
								picoos_emRaiseWarning(
									this->common->em, PICO_ERR_INTERNAL_LIMIT, (picoos_char*)"",
									(picoos_char*)"too many attributes in markup; ignoring");
							}
				...
				case MSInAttrName:
					if (tok_attrChar(str[i], FALSE)) {
						...
						picobase_lowercase_utf8_str(
							tok->markupParams[tok->nrMarkupParams].paramId,
							(picoos_char*)tok->markupParams[tok->nrMarkupParams].paramId,
							IN_BUF_SIZE, &ok);
						tok_setIsFileAttr(tok->markupParams[tok->nrMarkupParams].paramId,
										  &tok->isFileAttr);
...
static void tok_treatMarkup(picodata_ProcessingUnit this, tok_subobj_t* tok) {
	...
	if (tok_markupTagId(tok->markupTagName) != MIDummyEnd) {
		if (tok->markupTagErr == MENone) {
			tok->markupState = MSNotInMarkup;
			if ((tok->tokenType != PICODATA_ITEMINFO1_TOKTYPE_SPACE) &&
				(tok->tokenType != PICODATA_ITEMINFO1_TOKTYPE_UNDEFINED)) {
				tok_treatSimpleToken(this, tok);
			}
			tok_putToSimpleToken(this, tok, (picoos_uchar*)" ", PICODATA_ITEMINFO1_TOKTYPE_SPACE,
								 -1);
		...
		if (tok->markupTagErr != MENone) {
			if (!tok->aborted) {
				picoos_emRaiseWarning(
					this->common->em, PICO_ERR_INVALID_MARKUP_TAG, (picoos_char*)"",
					(picoos_char*)"syntax error in markup token '%s'", tok->markupStr);
			}
	...
	if (ch == NULLC) {
		...
		switch (tok_putToUtf(tok, ch)) {
			...
			case UTF_CHAR_COMPLETE:
				markupHandling =
					(markupHandling && (tok->markupHandlingMode == MARKUP_HANDLING_ENABLED));
				...
				if (markupHandling && (tok->markupState != MSNotInMarkup)) {
					...
					if ((type != tok->tokenType) || (type == PICODATA_ITEMINFO1_TOKTYPE_CHAR) ||
						(subtype != tok->tokenSubType)) {
						tok_treatSimpleToken(this, tok);
					} else if ((tok->utf[tok->utfpos - 1] == EOL) && (tok->nrEOL == 2)) {
						tok_treatSimpleToken(this, tok);
						tok_putToSimpleToken(this, tok, (picoos_uchar*)".",
											 PICODATA_ITEMINFO1_TOKTYPE_CHAR, -1);
						tok_treatSimpleToken(this, tok);
					}
...
static void tok_treatSimpleToken(picodata_ProcessingUnit this, tok_subobj_t* tok) {
	...
	if (tok->markupState != MSNotInMarkup) {
		if (!(tok->aborted) && (tok->markupState >= MSGotmarkupTagName) &&
			(tok_markupTagId(tok->markupTagName) != MIDummyEnd)) {
			picoos_emRaiseWarning(this->common->em, PICO_ERR_INVALID_MARKUP_TAG, (picoos_char*)"",
								  (picoos_char*)"unfinished markup tag '%s'", tok->markupStr);
		}
		tok_treatMarkupAsSimpleToken(this, tok);
		tok_treatSimpleToken(this, tok);
	} else if ((tok->tokenPos > 0) &&
			   ((tok->ignLevel <= 0) || (tok->tokenType == PICODATA_ITEMINFO1_TOKTYPE_SPACE))) {
		tok_putItem(this, tok, PICODATA_ITEM_TOKEN, tok->tokenType, (picoos_uint8)tok->tokenSubType,
					0, tok->tokenStr);
	}
	tok->tokenPos = 0;
	tok->tokenType = PICODATA_ITEMINFO1_TOKTYPE_UNDEFINED;
...
static picodata_step_result_t tokStep(register picodata_ProcessingUnit this,
									  picoos_int16 mode,
...
picodata_ProcessingUnit picotok_newTokenizeUnit(picoos_MemoryManager mm,
	...
	*numBytesOutput = 0;
	while (1) { /* exit via return */
		...
		if ((tok->outWritePos - tok->outReadPos) > 0) {
			if (picodata_cbPutItem(this->cbOut, &tok->outBuf[tok->outReadPos],
								   tok->outWritePos - tok->outReadPos, numBytesOutput) == PICO_OK) {
				PICODATA_INFO_ITEM(this->voice->kbArray[PICOKNOW_KBID_DBG],
								   (picoos_uint8*)"tok:", &tok->outBuf[tok->outReadPos],
								   tok->outWritePos - tok->outReadPos);
