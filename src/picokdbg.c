...
static pico_status_t kdbgInitialize(register picoknow_KnowledgeBase this, picoos_Common common) {
	kdbg_subobj_t* kdbg;
...
picoos_uint8 picokdbg_getPhoneId(const picokdbg_Dbg this, const picoos_char* phsym) {
	kdbg_subobj_t* kdbg;
	picoos_uint16 i;
	...
	/* sequential search */
	for (i = 0; i < 256; i++) {
		if (!picoos_strcmp(phsym, (picoos_char*)&(kdbg->phonesyms[i * KDBG_PHONESYMLEN_MAX]))) {
...
picoos_char* picokdbg_getPhoneSym(const picokdbg_Dbg this, const picoos_uint8 phid) {
	kdbg_subobj_t* kdbg;
