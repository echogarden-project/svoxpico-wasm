 ...
 *
 ...
 buffer allmost full), with the trailing PUNCT item included (last item).\n If the trailing PUNC is
 a a primary phrase separator, the item is not output, but instead, the bound state is set to PPHR,
 so that the correct BOUND can be output at the start of the next primary phrase.\n Otherwise, the
 item is converted to the corresponding BOUND and output. the bound state is set to SSEP, so that a
 BOUND of type SBEG is output at the start of the next primary phrase.
...
static pico_status_t acphInitialize(register picodata_ProcessingUnit this, picoos_int32 resetMode) {
	...
	/* kb tabphones */
	acph->tabphones = picoktab_getPhones(this->voice->kbArray[PICOKNOW_KBID_TAB_PHONES]);
...
#ifdef PICO_DEBUG_1
	{
		...
		PICODBG_DEBUG(("tabphones primstressID: %d", picoktab_getPrimstressID(acph->tabphones)));
		PICODBG_DEBUG(("tabphones secstressID: %d", picoktab_getSecstressID(acph->tabphones)));
		PICODBG_DEBUG(("tabphones syllboundID: %d", picoktab_getSyllboundID(acph->tabphones)));
		PICODBG_DEBUG(("tabphones pauseID: %d", picoktab_getPauseID(acph->tabphones)));
	}
#endif
...
static pico_status_t acphSubPhrasing(register picodata_ProcessingUnit this,
	...
	while ((upbound > 0) && (acph->headx[upbound].head.type != PICODATA_ITEM_WORDPHON)) {
		upbound--;
	}
	...
	for (i = upbound; (i > 0) && (nrwordspre > 1); i--) {
		...
		PICODBG_TRACE(("%d: [%d,%d|%d|%d,%d|%d,%d,%d]", i, valbuf[0], valbuf[1], valbuf[2],
					   valbuf[3], valbuf[4], nrwordspre, nrwordsfol, nrsyllsfol));

		/* no continue so far => subphrasing needed */
		/* construct input vector, which is set in dtphr */
		if (!picokdt_dtPHRconstructInVec(acph->dtphr, valbuf[0], valbuf[1], valbuf[2], valbuf[3],
			...
			if (nosubphrases) {
				...
				switch (acph->headx[acph->headxLen - 1].head.info2) {
					case PICODATA_ITEMINFO2_PUNC_SENT_T:
						acph->headx[i].boundtype = PICODATA_ITEMINFO2_BOUNDTYPE_T;
						break;
					case PICODATA_ITEMINFO2_PUNC_SENT_Q:
						acph->headx[i].boundtype = PICODATA_ITEMINFO2_BOUNDTYPE_Q;
						break;
					case PICODATA_ITEMINFO2_PUNC_SENT_E:
						acph->headx[i].boundtype = PICODATA_ITEMINFO2_BOUNDTYPE_E;
						break;
					case PICODATA_ITEMINFO2_PUNC_PHRASE:
					case PICODATA_ITEMINFO2_PUNC_PHRASE_FORCED:
						acph->headx[i].boundtype = PICODATA_ITEMINFO2_BOUNDTYPE_P;
						break;
					default:
	...
	switch (acph->boundStrengthState) {
		case SA_BOUNDSTRENGTH_SSEP:
			acph->headx[0].boundstrength = PICODATA_ITEMINFO1_BOUND_SBEG;
			break;
		case SA_BOUNDSTRENGTH_PPHR:
			acph->headx[0].boundstrength = PICODATA_ITEMINFO1_BOUND_PHR1;
			break;
		default:
	...
	if (nosubphrases) {
		...
		switch (acph->headx[acph->headxLen - 1].head.info2) {
			case PICODATA_ITEMINFO2_PUNC_SENT_T:
				acph->headx[0].boundtype = PICODATA_ITEMINFO2_BOUNDTYPE_T;
				break;
			case PICODATA_ITEMINFO2_PUNC_SENT_Q:
				acph->headx[0].boundtype = PICODATA_ITEMINFO2_BOUNDTYPE_Q;
				break;
			case PICODATA_ITEMINFO2_PUNC_SENT_E:
				acph->headx[0].boundtype = PICODATA_ITEMINFO2_BOUNDTYPE_E;
				break;
			case PICODATA_ITEMINFO2_PUNC_PHRASE:
			case PICODATA_ITEMINFO2_PUNC_PHRASE_FORCED:
				acph->headx[0].boundtype = PICODATA_ITEMINFO2_BOUNDTYPE_P;
				break;
			default:
				PICODBG_WARN(("invalid boundary type, not set"));
				break;
...
static picoos_uint8 acphAccGetNrsRight(register picodata_ProcessingUnit this,
	...
	*footsyllsfol = s2;
	i = ind + 1;
	while ((i < acph->headxLen) && (acph->headx[i].head.info2 != PICODATA_ACC1)) {
		if (acph->headx[i].head.type == PICODATA_ITEM_WORDPHON) {
			(*footwordsfol)++;
...
static picoos_uint8 acphAccGetNrsLeft(register picodata_ProcessingUnit this,
	...
	*nrsyllspre = s1;
	i = ind - 1;
	while ((i >= 0) && (acph->headx[i].boundstrength == PICODATA_ITEMINFO1_BOUND_PHR0)) {
		if (acph->headx[i].head.type == PICODATA_ITEM_WORDPHON) {
			(*nrwordspre)++;
...
static pico_status_t acphAccentuation(register picodata_ProcessingUnit this,
	...
	for (i = upbound + 1; i > 0;) {
		...
		/* get distances */
		if ((!acphAccGetNrsRight(this, acph, i, &nrwordsfol, &nrsyllsfol, &footwordsfol,
								 &footsyllsfol)) ||
		...
		PICODBG_TRACE(("%d: [%d,%d,%d,%d,%d|%d,%d|%d,%d,%d,%d|%d,%d]", i, valbuf[0], valbuf[1],
					   valbuf[2], valbuf[3], valbuf[4], hist1, hist2, nrwordspre, nrsyllspre,
		...
		if (!picokdt_dtACCconstructInVec(acph->dtacc, valbuf[0], valbuf[1], valbuf[2], valbuf[3],
										 valbuf[4], hist1, hist2, nrwordspre, nrsyllspre,
										 nrwordsfol, nrsyllsfol, footwordsfol, footsyllsfol)) {
			/* error constructing invec */
			PICODBG_WARN(("problem with invec"));
			picoos_emRaiseWarning(this->common->em, PICO_WARN_INVECTOR, NULL, NULL);
		...
		if (okay && dtres.set) {
			...
			PICODBG_DEBUG(("%d - after-nostress-corr: %d", i, acph->headx[i].head.info2));
		} else {
...
static picoos_uint8 acphPutBoundItem(register picodata_ProcessingUnit this,
	...
	*dopuoutfull = FALSE;
	...
	tmphead.info2 = type;
	tmphead.len = 0;
	rv = picodata_put_itemparts(&tmphead, NULL, 0, acph->tmpbuf, PICODATA_MAX_ITEMSIZE, &blen);
	if (rv != PICO_OK) {
...
static picodata_step_result_t acphStep(register picodata_ProcessingUnit this,
	...
	*numBytesOutput = 0;
	while (1) { /* exit via return */
		PICODBG_DEBUG(
			("doing state %i, hLen|c1Len: %d|%d", acph->procState, acph->headxLen, acph->cbufLen));
		...
		switch (acph->procState) {
			...
			case SA_STEPSTATE_COLLECT:
				...
				while (acph->inspaceok && acph->needsmoreitems &&
					   (PICO_OK == (rv = picodata_cbGetItem(this->cbIn, acph->tmpbuf,
															PICODATA_MAX_ITEMSIZE, &blen)))) {
					rvP = picodata_get_itemparts(
						acph->tmpbuf, PICODATA_MAX_ITEMSIZE, &(acph->headx[acph->headxLen].head),
						&(acph->cbuf[acph->cbufLen]), acph->cbufBufSize - acph->cbufLen, &clen);
					if (rvP != PICO_OK) {
					...
					if ((acph->headx[acph->headxLen].head.type == PICODATA_ITEM_CMD) &&
						(acph->headx[acph->headxLen].head.info1 == PICODATA_ITEMINFO1_CMD_FLUSH)) {
						acph->headx[acph->headxLen].head.type = PICODATA_ITEM_PUNC;
						acph->headx[acph->headxLen].head.info1 = PICODATA_ITEMINFO1_PUNC_FLUSH;
						acph->headx[acph->headxLen].head.info2 = PICODATA_ITEMINFO2_PUNC_SENT_T;
						acph->headx[acph->headxLen].head.len = 0;
					}

					/* check/set needsmoreitems */
					if (acph->headx[acph->headxLen].head.type == PICODATA_ITEM_PUNC) {
						acph->needsmoreitems = FALSE;
					}

					/* check/set inspaceok, keep spare slot for forcing */
					if ((acph->headxLen >= (PICOACPH_MAXNR_HEADX - 2)) ||
						((acph->cbufBufSize - acph->cbufLen) < PICODATA_MAX_ITEMSIZE)) {
				...
				if (!acph->needsmoreitems) {
					...
					acph->headx[acph->headxLen].head.type = PICODATA_ITEM_PUNC;
					acph->headx[acph->headxLen].head.info1 = PICODATA_ITEMINFO1_PUNC_PHRASEEND;
					acph->headx[acph->headxLen].head.info2 = PICODATA_ITEMINFO2_PUNC_PHRASE_FORCED;
					acph->headx[acph->headxLen].head.len = 0;
				...
				if (1) {
					...
					for (i = 0; i < acph->headxLen; i++) {
						if ((acph->headx[i].boundstrength != 0) &&
							(acph->headx[i].boundstrength != PICODATA_ITEMINFO1_BOUND_PHR0)) {
							PICODBG_INFO(
								("acph-p: boundstrength '%c', "
								 "boundtype '%c'",
								 acph->headx[i].boundstrength, acph->headx[i].boundtype));
						}
						...
						for (j = 0; j < acph->headx[i].head.len; j++) {
							if ((ittype == PICODATA_ITEM_CMD)) {
								PICODBG_INFO_MSG(("%c", acph->cbuf[acph->headx[i].cind + j]));
							} else {
								PICODBG_INFO_MSG(("%4d", acph->cbuf[acph->headx[i].cind + j]));
							}
						}
						PICODBG_INFO_MSG(("\n"));
					}
			...
			case SA_STEPSTATE_FEED: {
				...
				PICODBG_DEBUG(
					("put out items (bot, len): (%d, %d)", acph->headxBottom, acph->headxLen));
				...
				if (acph->headxBottom == 0) {
					...
					if (acph->headx[0].head.info1 != PICODATA_ITEMINFO1_PUNC_FLUSH) {
						if (!acphPutBoundItem(this, acph, acph->headx[0].boundstrength,
				...
				for (i = acph->headxBottom; i < indupbound; i++) {
					switch (acph->headx[i].head.type) {
						case PICODATA_ITEM_PUNC:
							...
							if ((acph->headx[i].head.info1 == PICODATA_ITEMINFO1_PUNC_SENTEND) &&
								(i == (indupbound - 1))) {
								/* construct and put BOUND item */
								if (!acphPutBoundItem(this, acph, PICODATA_ITEMINFO1_BOUND_SEND,
													  PICODATA_ITEMINFO2_NA, &dopuoutfull,
													  numBytesOutput)) {
									if (dopuoutfull) {
										PICODBG_DEBUG(("feeding overflow"));
										return PICODATA_PU_OUT_FULL;
									   ...
									   (i == (indupbound - 1))) {
								/* construct and put BOUND item */
								if (!acphPutBoundItem(this, acph, PICODATA_ITEMINFO1_BOUND_TERM,
													  PICODATA_ITEMINFO2_NA, &dopuoutfull,
													  numBytesOutput)) {
									if (dopuoutfull) {
										PICODBG_DEBUG(("feeding overflow"));
						...
						default:
							...
							if ((acph->headx[i].boundstrength == PICODATA_ITEMINFO1_BOUND_PHR2) ||
								(acph->headx[i].boundstrength == PICODATA_ITEMINFO1_BOUND_PHR3)) {
								if (!acphPutBoundItem(this, acph, acph->headx[i].boundstrength,
													  acph->headx[i].boundtype, &dopuoutfull,
													  numBytesOutput)) {
									if (dopuoutfull) {
										PICODBG_DEBUG(("feeding overflow"));
										return PICODATA_PU_OUT_FULL;
							...
							rv = picodata_put_itemparts(&(acph->headx[i].head),
														...
														acph->headx[i].head.len, acph->tmpbuf,
														PICODATA_MAX_ITEMSIZE, &blen);
