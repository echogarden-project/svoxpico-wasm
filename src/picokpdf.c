...
static pico_status_t kpdfDURInitialize(register picoknow_KnowledgeBase this, picoos_Common common) {
	...
	pdfdur->numframes = ((picoos_uint16)(this->base[pos + 1])) << 8 | this->base[pos];
	pos += 2;
	...
	PICODBG_DEBUG(
		("numframes %d, vecsize %d, phonquantlen %d, "
		 "statequantlen %d",
		 pdfdur->numframes, pdfdur->vecsize, pdfdur->phonquantlen, pdfdur->statequantlen));
	if ((picoos_uint32)(pos + (pdfdur->numframes * pdfdur->vecsize)) != this->size) {
		PICODBG_DEBUG(("header-spec size %d, kb-size %d",
					   pos + (pdfdur->numframes * pdfdur->vecsize), this->size));
...
static pico_status_t kpdfMULInitialize(register picoknow_KnowledgeBase this, picoos_Common common) {
	...
	pdfmul->numframes = ((picoos_uint16)(this->base[pos + 1])) << 8 | this->base[pos];
	pos += 2;
	...
	pdfmul->numstates = this->base[pos++];
	{
		...
		for (i = 1; i < pdfmul->numstates; i++) {
			pdfmul->stateoffset[i] = pdfmul->stateoffset[i - 1] +
									 (this->base[pos] | ((picoos_uint16)this->base[pos + 1] << 8));
			pos += 2;
		}
		pos += 2; /* we don't need the last number if we only need the offset (i.e. how to get to
					 the vector start) */
	}
	...
	if (maxbigpow < PICOKPDF_BIG_POW) {
		PICODBG_ERROR(("bigpow %i is larger than maxbigpow %i defined in pdf lingware",
					   PICOKPDF_BIG_POW, maxbigpow));
	...
	/* bigpow corrected by scmeanpow, multiply means by 2^meanpow to obtain fixed point
	 * representation */
	...
	/* check numdeltas */
	if ((pdfmul->numdeltas == 0xFF) &&
		(pdfmul->vecsize != (pdfmul->numvuv + pdfmul->ceporder * 3 * (2 + 1)))) {
		PICODBG_ERROR(
			("header has inconsistent values for vecsize, ceporder, numvuv, and numdeltas"));
		return picoos_emRaiseException(common->em, PICO_EXC_FILE_CORRUPT, NULL, NULL);
	}
	...
	*/
	if ((pdfmul->numdeltas != 0xFF) &&
		(pdfmul->vecsize !=
		 pdfmul->numvuv + pdfmul->ceporder * 2 + pdfmul->numdeltas * 3 + pdfmul->ceporder * 3)) {
		PICODBG_ERROR(
			("header has inconsistent values for vecsize, ceporder, numvuv, and numdeltas\n"
	...
	PICODBG_DEBUG(
		("numframes %d, vecsize %d, numstates %d, ceporder %d, "
		 "numvuv %d, numdeltas %d, meanpow %d, bigpow %d",
		 pdfmul->numframes, pdfmul->vecsize, pdfmul->numstates, pdfmul->ceporder, pdfmul->numvuv,
		 pdfmul->numdeltas, pdfmul->meanpow, pdfmul->bigpow));
	if ((picoos_uint32)(pos + (pdfmul->numframes * pdfmul->vecsize)) != this->size) {
		PICODBG_DEBUG(("header-spec size %d, kb-size %d",
					   pos + (pdfmul->numframes * pdfmul->vecsize), this->size));
		return picoos_emRaiseException(common->em, PICO_EXC_FILE_CORRUPT, NULL, NULL);
	}
...
static pico_status_t kpdfPHSInitialize(register picoknow_KnowledgeBase this, picoos_Common common) {
	...
	pdfphs->numvectors = ((picoos_uint16)(this->base[pos + 1])) << 8 | this->base[pos];
	pos += 2;
