...
typedef pico_status_t (*picodata_cbGetItemMethod)(register picodata_CharBuffer this,
												  ...
												  const picoos_uint16 blenmax,
												  picoos_uint16* blen,
												  const picoos_uint8 issd);

typedef pico_status_t (*picodata_cbSubResetMethod)(register picodata_CharBuffer this);
typedef pico_status_t (*picodata_cbSubDeallocateMethod)(register picodata_CharBuffer this,
...
void picodata_disposeCharBuffer(picoos_MemoryManager mm, picodata_CharBuffer* this) {
	if (NULL != (*this)) {
		/* terminate */
		if (NULL != (*this)->subObj) {
...
static pico_status_t data_cbGetItem(register picodata_CharBuffer this,
	...
	*blen = PICODATA_ITEM_HEADSIZE +
			(picoos_uint8)(this->buf[((this->front) + PICODATA_ITEMIND_LEN) % this->size]);

	/* if getting speech data in item */
	if (issd) {
		/* check item type */
		if (this->buf[this->front] != PICODATA_ITEM_FRAME) {
			PICODBG_WARN(("item type mismatch for speech data: %c", this->buf[this->front]));
			for (i = 0; i < *blen; i++) {
				this->front++;
				this->front %= this->size;
				this->len--;
...
#if defined(PICO_DEBUG)
	PICODBG_DEBUG(("putting item: %c|%d|%d|%d||", buf[PICODATA_ITEMIND_TYPE],
				   buf[PICODATA_ITEMIND_INFO1], buf[PICODATA_ITEMIND_INFO2],
				   buf[PICODATA_ITEMIND_LEN]));
	for (i = PICODATA_ITEM_HEADSIZE; i < *blen; i++) {
		if (buf[PICODATA_ITEMIND_TYPE] == PICODATA_ITEM_WORDGRAPH) {
...
pico_status_t picodata_get_itemparts(const picoos_uint8* buf,
	...
	pico_status_t status =
		picodata_get_itemparts_nowarn(buf, blenmax, head, content, clenmax, clen);
...
picoos_uint8 picodata_is_valid_item(const picoos_uint8* item, const picoos_uint16 ilenmax) {
	if (ilenmax >= PICODATA_ITEM_HEADSIZE) {
		...
		head.info2 = item[2];
		head.len = item[3];
		if ((ilenmax >= (head.len + PICODATA_ITEM_HEADSIZE)) && picodata_is_valid_itemhead(&head)) {
			return TRUE;
		}
	}
	return FALSE;
...
void picodata_disposeProcessingUnit(picoos_MemoryManager mm, picodata_ProcessingUnit* this) {
	if (NULL != (*this)) {
		/* terminate */
		if (NULL != (*this)->subObj) {
			(*this)->subDeallocate(*this, mm);
...
static void transDurUniform(picoos_uint8 frame_duration_exp, /* 2's exponent of frame duration in
																ms, e.g. 2 for 4ms, 3 for 8ms */
...
static void transDurWeighted(picoos_uint8 frame_duration_exp, /* 2's exponent of frame duration in
																 ms, e.g. 2 for 4ms, 3 for 8ms */
							 picoos_int8 array_length,
							 picoos_uint8* inout,
	...
	if (targetdur > inputdur) {
		...
		fact =
			-((inputdur - targetdur) << (PICODATA_PRECISION - frame_duration_exp)) / weighted_sum;
	}
...
void picodata_transformDurations(
	...
	picoos_int16
		facttarget,		   /* factor to be multiplied with original length to get the target
	...
	PICODBG_TRACE(("######## transforming duration fact=%i, limits = [%i,%i] (input frames: %i)",
				   facttarget, mintarget, maxtarget, inputdur));
	...
	if (facttarget || (targetdur < mintarget) || (maxtarget < targetdur)) {
		...
		if (NULL == weight) {
			...
			transDurWeighted(frame_duration_exp, array_length, inout, weight, inputdur, targetdur,
							 dur_rest);
		}
	}
}
...
pico_status_t picodata_mapPAStrToPAIds(picotrns_SimpleTransducer transducer,
	...
	if (picoos_strcmp(alphabet, PICODATA_XSAMPA) == 0) {
		if ((NULL != xsampa_parser) && (NULL != xsampa2svoxpa_mapper)) {
			...
			status = picotrns_stAddWithPlane(transducer, inputPhones, PICOKFST_PLANE_ASCII);
			if (PICO_OK != status) {
				picoos_emRaiseWarning(common->em, status, NULL,
									  (picoos_char*)"phoneme sequence too long (%s)", inputPhones);
			} else {
		...
		if ((NULL != svoxpa_parser)) {
			...
			if (PICO_OK == status) {
				status = picotrns_stGetSymSequence(transducer, outputPhoneIds, maxOutputPhoneIds);
			}
			return status;
		}
	}
...
static picoos_char* data_itemtype_to_string(const picoos_uint8 itemtype,
	...
	picopal_slprintf((picopal_char*)str, strsize, (picopal_char*)"%s", tmpstr);
	return str;
}
...
picoos_char* picodata_head_to_string(const picodata_itemhead_t* head,
	...
	if (NULL == head) {
		...
		picopal_slprintf((picopal_char*)str + typelen, strsize - typelen,
						 (picopal_char*)"|%c|%c|%i", head->info1, head->info2, head->len);
	}
...
#define SA_USE_PHST 1
	...
	for (i = 0; i < ch; i++) {
		if ((item[0] == PICODATA_ITEM_WSEQ_GRAPH) || (item[0] == PICODATA_ITEM_TOKEN) ||
...
#include "picokdbg.h"
		...
		if ((item[0] == PICODATA_ITEM_WORDPHON) || (item[0] == PICODATA_ITEM_SYLLPHON) ||
			...
			if (picokdbg_getPhoneSym(kdbg, item[4])) {
				...
				for (j = 0; j < item[3]; j++) {
					PICODBG_INFO_MSG_F(filterfn, ("%s", picokdbg_getPhoneSym(kdbg, item[4 + j])));
				}
			}
		}
	}
