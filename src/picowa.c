...
static pico_status_t waInitialize(register picodata_ProcessingUnit this, picoos_int32 resetMode) {
	...
	/* kb tabpos */
	wa->tabpos = picoktab_getPos(this->voice->kbArray[PICOKNOW_KBID_TAB_POS]);
	if (wa->tabpos == NULL) {
		return picoos_emRaiseException(this->common->em, PICO_EXC_KB_MISSING, NULL, NULL);
	}
...
static pico_status_t waProcessWordgraph(register picodata_ProcessingUnit this,
	...
	*/
	...
	if (!found && !picoklex_lexLookup(wa->lex, content, head->len, &lexres)) {
		...
		if (PICO_OK ==
			picodata_copy_item(wa->inBuf, wa->inLen, wa->outBuf, wa->outBufSize, &wa->outLen)) {
			wa->inLen = 0;
			...
			if (PICO_OK != picodata_set_iteminfo1(wa->outBuf, wa->outLen,
				...
				return picoos_emRaiseException(this->common->em, PICO_EXC_BUF_OVERFLOW, NULL, NULL);
			}
		}
		...
		if (lexres.phonfound) { /* incl. ind-phone and possibly multi-ent. */
			...
			if (lexres.nrres == 1) {
				...
				head->info1 = picoktab_getPosGroup(wa->tabpos, posbuf, lexres.nrres);
			}
			head->info2 = foundIndex;
			head->len = lexres.posindlen;
			if ((status = picodata_put_itemparts(head, lexres.posind, lexres.posindlen, wa->outBuf,
												 wa->outBufSize, &wa->outLen)) == PICO_OK) {
				wa->inLen = 0;
			} else {
				return picoos_emRaiseException(this->common->em, status, NULL, NULL);
			}
			...
			if (PICO_OK ==
				...
				if (PICO_OK != picodata_set_iteminfo1(wa->outBuf, wa->outLen, lexres.posind[0])) {
					return picoos_emRaiseException(this->common->em, PICO_EXC_BUF_OVERFLOW, NULL,
												   NULL);
				}
			}
		}
...
static picodata_step_result_t waStep(register picodata_ProcessingUnit this,
	...
	*numBytesOutput = 0;
	while (1) { /* exit via return */
		PICODBG_DEBUG(
			("doing state %i, inLen: %d, outLen: %d", wa->procState, wa->inLen, wa->outLen));
		...
		switch (wa->procState) {
			...
			case WA_STEPSTATE_COLLECT:
				if (wa->inLen == 0) { /* is input buffer empty? */
					picoos_uint16 blen;
					/* try to get one item */
					rv = picodata_cbGetItem(this->cbIn, wa->inBuf, wa->inBufSize, &blen);
			...
			case WA_STEPSTATE_PROCESS:
				...
				if ((wa->inLen > 0) && picodata_is_valid_item(wa->inBuf, wa->inLen)) {
					picodata_itemhead_t ihead;
					picoos_uint8* icontent;
					pico_status_t rvP = PICO_OK;

					rv = picodata_get_iteminfo(wa->inBuf, wa->inLen, &ihead, &icontent);
					if (PICO_OK == rv) {
						switch (ihead.type) {
							case PICODATA_ITEM_WORDGRAPH:

								if (0 < ihead.len) {
									rvP = waProcessWordgraph(this, wa, &ihead, icontent);
								} else {
							...
							default:
								...
								rvP = picodata_copy_item(wa->inBuf, wa->inLen, wa->outBuf,
					...
					picoos_emRaiseWarning(this->common->em, PICO_WARN_PU_IRREG_ITEM, NULL, NULL);
					picoos_emRaiseWarning(this->common->em, PICO_WARN_PU_DISCARD_BUF, NULL, NULL);
					wa->inLen = 0;
					wa->procState = WA_STEPSTATE_COLLECT;
					return PICODATA_PU_BUSY;
				}
			...
			case WA_STEPSTATE_FEED:
				...
				rv = picodata_cbPutItem(this->cbOut, wa->outBuf, wa->outLen, numBytesOutput);

				PICODATA_INFO_ITEM(this->voice->kbArray[PICOKNOW_KBID_DBG],
								   (picoos_uint8*)"wana: ", wa->outBuf, wa->outLen);
				...
				} else if ((PICO_EXC_BUF_UNDERFLOW == rv) || (PICO_ERR_OTHER == rv)) {
