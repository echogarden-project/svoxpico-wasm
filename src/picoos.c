...
picoos_int8 picoos_strcmp(const picoos_char* a, const picoos_char* b) {
	picopal_int32 res = picopal_strcmp((const picopal_char*)a, (const picopal_char*)b);
...
picoos_int8 picoos_strncmp(const picoos_char* a, const picoos_char* b, picoos_objsize_t siz) {
	picopal_int32 res = picopal_strncmp((const picopal_char*)a, (const picopal_char*)b, siz);
	return (picoos_int8)((res < 0) ? -1 : (res > 0) ? 1 : 0);
}
...
picoos_char* picoos_strchr(const picoos_char* s, picoos_char c) {
	return (picoos_char*)picopal_strchr((const picopal_char*)s, (picopal_char)c);
}

picoos_char* picoos_strstr(const picoos_char* s, const picoos_char* substr) {
	return (picoos_char*)picopal_strstr((const picopal_char*)s, (const picopal_char*)substr);
}
...
picoos_char* picoos_strcpy(picoos_char* d, const picoos_char* s) {
	return (picoos_char*)picopal_strcpy((picopal_char*)d, (const picopal_char*)s);
}

picoos_char* picoos_strcat(picoos_char* d, const picoos_char* s) {
	return (picoos_char*)picopal_strcat((picopal_char*)d, (const picopal_char*)s);
}

picoos_objsize_t picoos_strlcpy(picoos_char* dst, const picoos_char* src, picoos_objsize_t siz) {
	return (picoos_objsize_t)picopal_strlcpy((picopal_char*)dst, (const picopal_char*)src,
											 (picopal_objsize_t)siz);
}
...
void* picoos_raw_malloc(byte_ptr_t raw_mem,
						picoos_objsize_t raw_mem_size,
						picoos_objsize_t alloc_size,
						byte_ptr_t* rest_mem,
...
static int os_init_mem_block(picoos_MemoryManager this) {
	...
	cend = (MemCellHdr)((picoos_objsize_t)newBlockAddr + size - this->fullCellHdrSize);
	cbeg->size = 0;
...
picoos_MemoryManager picoos_newMemoryManager(void* raw_memory,
											 picoos_objsize_t size,
	...
	this = picoos_raw_malloc(raw_memory, size, sizeof(memory_manager_t), &rest_mem, &rest_mem_size);
	if (this == NULL) {
		return NULL;
	}
	...
	this->fullCellHdrSize =
		((sizeof(mem_cell_hdr_t) + PICOOS_ALIGN_SIZE - 1) / PICOOS_ALIGN_SIZE) * PICOOS_ALIGN_SIZE;
	/* get aligned size of header without free-list fields; the result may be compiler-dependent;
	 the size is therefore computed by inspecting the end addresses of the fields 'size' and
	 'leftCell'; the higher of the ending addresses is used to get the (aligned) starting address of
	 the application contents */
	this->usedCellHdrSize =
		(picoos_objsize_t)&test_cell.size - (picoos_objsize_t)&test_cell + sizeof(picoos_objsize_t);
	size2 =
		(picoos_objsize_t)&test_cell.leftCell - (picoos_objsize_t)&test_cell + sizeof(MemCellHdr);
	...
	this->firstBlock = this->lastBlock =
		picoos_raw_malloc(raw_memory, size, sizeof(mem_block_hdr_t), &rest_mem, &rest_mem_size);
...
void picoos_protectMem(picoos_MemoryManager mm,
					   void* addr,
...
void picoos_getMemUsage(picoos_MemoryManager this,
...
void picoos_showMemUsage(picoos_MemoryManager this,
						 picoos_bool incremental,
						 picoos_bool resetIncremental) {
	picoos_int32 usedBytes, incrUsedBytes, maxUsedBytes;

	picoos_getMemUsage(this, resetIncremental, &usedBytes, &incrUsedBytes, &maxUsedBytes);
	if (incremental) {
...
void* picoos_allocate(picoos_MemoryManager this, picoos_objsize_t byteSize) {
	...
	while ((c != NULL) && (c->size != (picoos_ptrdiff_t)cellSize) &&
...
picoos_ExceptionManager picoos_newExceptionManager(picoos_MemoryManager mm) {
	picoos_ExceptionManager this = (picoos_ExceptionManager)picoos_allocate(mm, sizeof(*this));
	if (NULL != this) {
		/* initialize */
		picoos_emReset(this);
	}
...
void picoos_disposeExceptionManager(picoos_MemoryManager mm, picoos_ExceptionManager* this) {
	if (NULL != (*this)) {
		/* terminate */
		picoos_deallocate(mm, (void*)this);
	}
...
static void picoos_vSetErrorMsg(picoos_char* dst,
								picoos_objsize_t siz,
								picoos_int16 code,
								picoos_char* base,
								const picoos_char* fmt,
								va_list args) {
	picoos_uint16 bsize;
	...
	if (NULL == base) {
		switch (code) {
			...
			case PICO_EXC_KB_MISSING:
				...
				/* runtime exceptions (programming problems, usually a bug. E.g. trying to access
				 * null pointer) */
			...
			case PICO_ERR_INDEX_OUT_OF_RANGE:
				...
				/* errors ("external" errors, e.g. hardware failure. Usually cannot be dealt with
				 * from inside pico.) */
			case PICO_ERR_OTHER:
...
void picoos_setErrorMsg(picoos_char* dst,
						picoos_objsize_t siz,
						picoos_int16 code,
						picoos_char* base,
						const picoos_char* fmt,
...
/* For convenience, this function returns the resulting current exception code. The return value
 * therefore is NOT the status of raising the error! */
pico_status_t picoos_emRaiseException(picoos_ExceptionManager this,
									  pico_status_t exceptionCode,
									  picoos_char* baseMessage,
									  picoos_char* fmt,
									  ...) {
	va_list args;
	...
	if (PICO_OK == this->curExceptionCode && PICO_OK != exceptionCode) {
		...
		va_start(args, (char*)fmt);
		picoos_vSetErrorMsg(this->curExceptionMessage, PICOOS_MAX_EXC_MSG_LEN, exceptionCode,
							baseMessage, fmt, args);
		PICODBG_DEBUG(("exit with exception code=%i, exception message='%s'",
...
void picoos_emRaiseWarning(picoos_ExceptionManager this,
	...
	if ((this->curNumWarnings < PICOOS_MAX_NUM_WARNINGS) && (PICO_OK != warningCode)) {
		if (PICOOS_MAX_NUM_WARNINGS - 1 == this->curNumWarnings) {
			...
			picoos_strlcpy(this->curWarningMessage[this->curNumWarnings],
						   (picoos_char*)"too many warnings", PICOOS_MAX_WARN_MSG_LEN);
		} else {
			this->curWarningCode[this->curNumWarnings] = warningCode;
			va_start(args, (char*)fmt);
			picoos_vSetErrorMsg(this->curWarningMessage[this->curNumWarnings],
								PICOOS_MAX_WARN_MSG_LEN, warningCode, baseMessage, fmt, args);
			va_end(args);
		}
		this->curNumWarnings++;
	}
	PICODBG_DEBUG(("exit with code=%i and message='%s', resulting in #warnings=%i",
				   this->curWarningCode[this->curNumWarnings - 1],
				   this->curWarningMessage[this->curNumWarnings - 1], this->curNumWarnings));
}
...
void picoos_emGetWarningMessage(picoos_ExceptionManager this,
								picoos_uint8 index,
								picoos_char* msg,
								picoos_uint16 maxsize) {
	if (index < this->curNumWarnings) {
...
static picoos_bool LOpen(picoos_Common g,
						 picoos_File* f,
						 picoos_char fileName[],
						 picopal_access_mode mode) {
	picoos_bool done = TRUE;

	*f = picoos_newFile(g->mm);
	picopal_strcpy((*f)->name, fileName);
	(*f)->write = ((mode == PICOPAL_TEXT_WRITE) || (mode == PICOPAL_BINARY_WRITE));
...
static picoos_bool LReadBytes(picoos_File f, picoos_uint8 bytes[], picoos_uint32* len) {
	picoos_bool done;
	picoos_int32 res;
...
/* read a given string 'str' from file. If no match was found, the read position is advanced until
 * and including the first non-matching character */
 ...
 */
picoos_uint8 picoos_OpenBinary(picoos_Common g, picoos_File* f, picoos_char fileName[]) {
...
*/
picoos_bool picoos_ReadBytes(picoos_File f, picoos_uint8 bytes[], picoos_uint32* len) {
	picoos_bool done = TRUE;
...
picoos_bool picoos_CreateBinary(picoos_Common g, picoos_File* f, picoos_char fileName[]) {
...
static void arr_conv_le_int16(picoos_uint8 src[],
							  picoos_uint32 srcShortStartInd,
							  picoos_uint32 nrElems,
							  picoos_int16 dst[],
							  picoos_uint32 dstStartInd) {
	...
	for (i = 0; i < nrElems; i++) {
		*(dst_p++) =
			(*src_p)[0] + (((*src_p)[1] & 0x7F) << 8) - (((*src_p)[1] & 0x80) ? 0x8000 : 0);
		src_p++;
	}
}

/* convert array of int16 into little-endian format */
static void arr_conv_int16_le(picoos_int16 src[],
							  picoos_uint32 srcStartInd,
							  picoos_uint32 nrElems,
							  picoos_uint8 dst[],
							  picoos_uint32 dstShortStartInd) {
...
static picoos_bool picoos_readWavHeader(picoos_File f,
										picoos_uint32* sf,
										picoos_encoding_t* enc,
	...
	picoos_FileLength(f, &fileLen);
					...
					picoos_read_le_uint32(f, &n32)); /* length of fmt chunk in bytes; must be 16 */
	done = done && (n32 == 16);
	done = done && (PICO_OK == picoos_read_le_uint16(f, &formatTag));
	done =
		done && (PICO_OK == picoos_read_le_uint16(f, &n16)); /* number of channels; must be mono */
	done = done && (n16 == 1);
		   ...
		   (PICO_OK == picoos_read_le_uint32(f, &dataLength)); /* length of data chunk in bytes */
	(*hdrSize) = 44;
	if (done) {
		(*sf) = sampleRate;
		(*nrSamples) = 0;
		...
		if (!done) {
			...
			if (nrFileSamples != (*nrSamples)) {
				...
				PICODBG_WARN(("inconsistent number of samples in wav file: %d vs. %d",
							  nrFileSamples, (*nrSamples)));
...
extern picoos_bool picoos_sdfOpenIn(picoos_Common g,
	...
	if (picoos_OpenBinary(g, &(sdf->file), fileName)) {
		if (picoos_has_extension(fileName, (picoos_char*)".wav")) {
			...
			done = picoos_readWavHeader(sdf->file, &(sdf->sf), &(sdf->enc), &(sdf->nrFileSamples),
										&(sdf->hdrSize));
		} else {
...
static void picoos_sdfLoadSamples(picoos_SDFile sdFile, picoos_uint32* nrSamples) {
	picoos_uint32 len;
...
extern picoos_bool picoos_sdfGetSamples(picoos_SDFile sdFile,
										picoos_uint32 start,
										picoos_uint32* nrSamples,
...
static picoos_bool picoos_writeWavHeader(picoos_File f,
										 picoos_uint32 sf,
										 picoos_encoding_t enc,
										 picoos_uint32 nrSamples,
										 picoos_uint32* hdrSize) {
...
extern picoos_bool picoos_sdfOpenOut(picoos_Common g,
									 picoos_SDFile* sdFile,
									 picoos_char fileName[],
									 int sf,
									 picoos_encoding_t enc) {
		...
		if (picoos_CreateBinary(g, &(sdf->file), fileName)) {
			if (picoos_has_extension(fileName, (picoos_char*)".wav")) {
				sdf->fileType = FILE_TYPE_WAV;
					...
					picoos_writeWavHeader(sdf->file, sdf->sf, sdf->enc, DummyLen, &(sdf->hdrSize));
			} else {
				/* we prefer not to treat other formats, rather than treat it as raw */
				/* fileType = FILE_TYPE_RAW; */
		...
		switch (sdFile->enc) {
			case PICOOS_ENC_LIN:
				...
				len = (nrSamples * 2);
					...
					picoos_WriteBytes(sdFile->file, sdFile->bBuf, &len) && ((nrSamples * 2) == len);
				break;
			case PICOOS_ENC_ULAW:
			case PICOOS_ENC_ALAW:
			default:
...
extern picoos_bool picoos_sdfPutSamples(picoos_SDFile sdFile,
										picoos_uint32 nrSamples,
										picoos_int16 samples[]) {
	picoos_uint32 i;
	picoos_int32 s;
...
extern picoos_bool picoos_sdfCloseOut(picoos_Common g, picoos_SDFile* sdFile) {
	...
	if (NULL != (*sdFile)) {
		...
		if (FILE_TYPE_WAV == (*sdFile)->fileType) {
			done = picoos_writeWavHeader((*sdFile)->file, (*sdFile)->sf, (*sdFile)->enc,
										 (*sdFile)->nrFileSamples, &hdrSize);
		}
...
pico_status_t picoos_setHeaderField(picoos_FileHeader header,
	...
	if ((picoos_strlcpy(header->field[index].key, key, PICOOS_MAX_FIELD_STRING_LEN) <
		 PICOOS_MAX_FIELD_STRING_LEN) &&
		(picoos_strlcpy(header->field[index].value, value, PICOOS_MAX_FIELD_STRING_LEN)) <
			PICOOS_MAX_FIELD_STRING_LEN) {
		return PICO_OK;
	} else {
		return PICO_ERR_INDEX_OUT_OF_RANGE;
...
/* caller has to make sure allocated space at key and value are large enough to hold a
 * picoos_field_string */
pico_status_t picoos_getHeaderField(picoos_FileHeader header,
									picoos_uint8 index,
	...
	*op = header->field[index].op;
	if ((picoos_strlcpy(key, header->field[index].key, PICOOS_MAX_FIELD_STRING_LEN) <
		 PICOOS_MAX_FIELD_STRING_LEN) &&
		(picoos_strlcpy(value, header->field[index].value, PICOOS_MAX_FIELD_STRING_LEN)) <
			PICOOS_MAX_FIELD_STRING_LEN) {
		return PICO_OK;
	} else {
		return PICO_ERR_INDEX_OUT_OF_RANGE;
...
/* check whether 'str' of length strlen matches contents in circular buffer buf, located in the
 * first strlen bytes and ending position bufpos. */
static picoos_uint8 os_matched(picoos_char* str,
							   picoos_uint32 strlen,
							   picoos_char* buf,
							   picoos_int32 bufpos) {
...
pico_status_t picoos_getSVOXHeaderString(picoos_char* str,
										 picoos_uint8* len,
										 picoos_uint32 maxlen) {
	picoos_char* ch;
...
picoos_uint8 picoos_get_str(picoos_char* fromStr,
							picoos_uint32* pos,
							picoos_char* toStr,
							picoos_objsize_t maxsize) {
	picoos_uint8 i = 0;
	/* skip non-printables */
...
pico_status_t picoos_read_le_uint32(picoos_File file, picoos_uint32* val) {
	...
	if (picoos_ReadBytes(file, by, &n) && (4 == n)) {
		/* little-endian */
		PICODBG_TRACE(("reading uint 32:  %i %i %i %i", by[0], by[1], by[2], by[3]));
		*val = (((picoos_uint32)by[3] << 8 | (picoos_uint32)by[2]) << 8 | (picoos_uint32)by[1])
				   << 8 |
			   (picoos_uint32)by[0];
		PICODBG_TRACE(("uint 32:  %i %i %i %i corresponds %i", by[0], by[1], by[2], by[3], *val));
		return PICO_OK;
	} else {
		*val = 0;
		return PICO_ERR_OTHER;
...
pico_status_t picoos_read_mem_pi_uint16(picoos_uint8* data,
										picoos_uint32* pos,
										picoos_uint16* val) {
...
pico_status_t picoos_read_mem_pi_uint32(picoos_uint8* data,
										picoos_uint32* pos,
										picoos_uint32* val) {
	picoos_uint8* by = data + *pos;

	/* little-endian */
	*val = (((picoos_uint32)by[3] << 8 | (picoos_uint32)by[2]) << 8 | (picoos_uint32)by[1]) << 8 |
		   (picoos_uint32)by[0];
	(*pos) += 4;
	return PICO_OK;
}
...
/* write pi to mem */
pico_status_t picoos_write_mem_pi_uint16(picoos_uint8* data,
										 picoos_uint32* pos,
										 picoos_uint16 val) {
	picoos_uint8* by = data + *pos;
	/* little-endian */
...
void picoos_get_sep_part_str(picoos_char string[],
							 picoos_int32 stringlen,
							 picoos_int32* ind,
							 picoos_char sepCh,
							 picoos_char part[],
							 picoos_int32 maxsize,
							 picoos_uint8* done) {
	picoos_int32 j;
	picoos_uint8 done1;
	...
	if (((*ind) >= stringlen)) {
		...
		done1 = 1;
		j = 0;
		while ((((*ind) < stringlen) && (string[(*ind)] != sepCh)) &&
			   (string[((*ind))] != (picoos_char)NULLC)) {
			if ((j < maxsize - 1)) {
				part[(j)] = string[(*ind)];
				j++;
			} else {
