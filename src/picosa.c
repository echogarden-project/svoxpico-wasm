...
/** @addtogroup picosa
  ...
  3. phrasing (right-to-left):
	 ...
	 2-level bound state: The internal buffer contains one primary phrase (sometimes forced, if
buffer allmost full), with the trailing PUNCT item included (last item). If the trailing PUNC is a a
primary phrase separator, the item is not output, but instead, the bound state is set to PPHR, so
that the correct BOUND can be output at the start of the next primary phrase. Otherwise, the item is
converted to the corresponding BOUND and output. the bound state is set to SSEP, so that a BOUND of
type SBEG is output at the start of the next primary phrase.
...
static pico_status_t saInitialize(register picodata_ProcessingUnit this, picoos_int32 resetMode) {
	...
	/* kb tabgraphs */
	sa->tabgraphs = picoktab_getGraphs(this->voice->kbArray[PICOKNOW_KBID_TAB_GRAPHS]);
	if (sa->tabgraphs == NULL) {
		return picoos_emRaiseException(this->common->em, PICO_EXC_KB_MISSING, NULL, NULL);
	}
	PICODBG_DEBUG(("got tabgraphs"));

	/* kb tabphones */
	sa->tabphones = picoktab_getPhones(this->voice->kbArray[PICOKNOW_KBID_TAB_PHONES]);
...
#ifdef PICO_DEBU
	{
		...
		PICODBG_DEBUG(("tabphones primstressID: %d", picoktab_getPrimstressID(sa->tabphones)));
		PICODBG_DEBUG(("tabphones secstressID: %d", picoktab_getSecstressID(sa->tabphones)));
		PICODBG_DEBUG(("tabphones syllboundID: %d", picoktab_getSyllboundID(sa->tabphones)));
		PICODBG_DEBUG(("tabphones pauseID: %d", picoktab_getPauseID(sa->tabphones)));
	}
#endif

	/* kb tabpos */
	sa->tabpos = picoktab_getPos(this->voice->kbArray[PICOKNOW_KBID_TAB_POS]);
	if (sa->tabpos == NULL) {
		return picoos_emRaiseException(this->common->em, PICO_EXC_KB_MISSING, NULL, NULL);
	}
...
static pico_status_t saDisambPos(register picodata_ProcessingUnit this, register sa_subobj_t* sa) {
	...
	while ((first < sa->headxLen) && (sa->headx[first].head.type != PICODATA_ITEM_WORDGRAPH) &&
	...
	for (ci = first; ci < sa->headxLen; ci++) {
		...
		valbuf[PICOKDT_NRATT_POSD - 1] =
			saPosDItemSeqGetPosRight(this, sa, lastprev3, sa->headxLen, &lastprev3);
		...
		if (picoktab_isUniquePos(sa->tabpos, (picoos_uint8)curPOS)) {
			...
			inval = 0;
			fallback = 0;
			if (!picokdt_dtPosDreverseMapOutFixed(sa->dtposd, curPOS, &prevout, &fallback)) {
				if (fallback) {
					prevout = fallback;
		...
		PICODBG_DEBUG(("%d: [%d %d %d %d %d %d %d]", ci, valbuf[0], valbuf[1], valbuf[2], valbuf[3],
					   valbuf[4], valbuf[5], valbuf[6]));
		...
		if (sa->headx[ci].head.type == PICODATA_ITEM_WORDINDEX) {
			...
			for (i = 0; i < sa->headx[ci].head.len; i += PICOKLEX_POSIND_SIZE) {
				...
				if (picoktab_isPartOfPosGroup(sa->tabpos, (picoos_uint8)dtres.class,
											  sa->cbuf1[sa->headx[ci].cind + i])) {
					PICODBG_DEBUG(("found match for entry %d", i / PICOKLEX_POSIND_SIZE + 1));
					sa->headx[ci].cind += i;
					okay = TRUE;
					break;
				}
...
static pico_status_t saCopyItemContent1to2(register picodata_ProcessingUnit this,
	...
	if (sa->headx[ind].head.len > (sa->cbuf2BufSize - sa->cbuf2Len)) {
		...
		picoos_emRaiseWarning(this->common->em, PICO_WARN_INCOMPLETE, NULL, NULL);
	...
	PICODBG_DEBUG(("%c item, len: %d", sa->headx[ind].head.type, sa->headx[ind].head.len));
...
static pico_status_t saLexIndLookup(register picodata_ProcessingUnit this,
	...
	if (picoklex_lexIndLookup(lex, &(sa->cbuf1[sa->headx[ind].cind + 1]), PICOKLEX_IND_SIZE, &pos,
							  &phones, &plen)) {
		sa->headx[ind].cind = sa->cbuf2Len;
		...
		PICODBG_DEBUG(("%c item, pos: %d, plen: %d", PICODATA_ITEM_WORDPHON, pos, plen));

	} else {
		PICODBG_WARN(("lexIndLookup problem"));
		picoos_emRaiseWarning(this->common->em, PICO_WARN_PU_IRREG_ITEM, NULL, NULL);
	}
	return PICO_OK;
}
...
static picoos_uint8 saGetNrVowel(register picodata_ProcessingUnit this,
	...
	*nVord = 0;
	...
	for (nCount = 0; nCount < inLen;) {
		...
		if (picoktab_hasVowellikeProp(sa->tabgraphs, cstr, PICOBASE_UTF8_MAXLEN)) {
			(*nVow)++;
			if (nCount == pos) {
				(*nVord) = (*nVow);
			}
...
static picoos_uint8 saDoG2P(register picodata_ProcessingUnit this,
	...
	*plen = 0;
	...
	while (nCount > 0) {
		...
		if (!saGetNrVowel(this, sa, graph, graphlen, nCount - 1, &nrvow, &ordvow)) {
			nrvow = 0;
			ordvow = 0;
		}
		...
		if (!picokdt_dtG2PconstructInVec(sa->dtg2p, graph, /*grapheme start*/
		...
		if (okay) {
			if ((dtresv.nr == 0) || (dtresv.classvec[0] == PICOKDT_EPSILON)) {
				...
				for (i = dtresv.nr;
					 ((((PICOSA_MAXITEMSIZE - 1) - phonesind) < phonesmaxlen) && (i > 0));) {
					i--;
					PICODBG_TRACE(("%c %d", graph[nCount - 1], dtresv.classvec[i]));
					if (dtresv.classvec[i] > 255) {
						PICODBG_WARN(
							("dt result outside valid range, "
							 "skipping phone"));
						continue;
					}
...
static pico_status_t saGraphemeToPhoneme(register picodata_ProcessingUnit this,
	...
	if (saDoG2P(this, sa, &(sa->cbuf1[sa->headx[ind].cind]), sa->headx[ind].head.len,
				sa->headx[ind].head.info1, &(sa->cbuf2[sa->cbuf2Len]),
				(sa->cbuf2BufSize - sa->cbuf2Len), &plen)) {
		/* check of cbuf2Len done in saDoG2P, phones skipped if needed */
		if (plen > 255) {
			PICODBG_WARN(("maximum number of phones exceeded (%d), skipping", plen));
			plen = 255;
		}
		...
		sa->headx[ind].cind = sa->cbuf2Len;
		sa->cbuf2Len += plen;
		PICODBG_DEBUG(("%c item, plen: %d", PICODATA_ITEM_WORDPHON, plen));
	} else {
...
static pico_status_t saAddPhoneme(register sa_subobj_t* sa, picoos_uint16 pos, picoos_uint16 sym) {
	...
	PICODBG_DEBUG(("adding %i/%i (%c on plane %i) at
	phonBuf[%i]",pos,sym,unshifted,plane,sa->phonWritePos));
	*/
...
static pico_status_t saExtractPhonemes(register picodata_ProcessingUnit this,
	...
	PICODBG_TRACE(("doing item %s", picodata_head_to_string(head, msgstr, SA_MSGSTR_SIZE)));
	/*
	 Items  considered in a transduction are WORDPHON item. its starting offset within the inBuf is
	 given as 'pos'. Elements that go into the transduction receive "their" position in the buffer.
	 */
	sa->phonWritePos = 0;
 ...
 * SA_POSSYM_OUT_OF_RANGE  : pos is out of range. 'pos' is set to that of the read possym, 'sym' is
 * undefined SA_POSSYM_UNDERFLOW     : no more data in buf. 'pos' is set to PICOTRNS_POS_INVALID,
 * 'sym' is undefined SA_POSSYM_INVALID       : "strange" pos.       'pos' is set to
 * PICOTRNS_POS_INVALID,    'sym' is undefined
 */
static pico_status_t getNextPosSym(sa_subobj_t* sa,
								   picoos_int16* pos,
								   picoos_int16* sym,
								   picoos_int16 rangeEnd) {
	/* skip POS_IGNORE */
	while ((sa->phonReadPos < sa->phonWritePos) &&
		   (PICOTRNS_POS_IGNORE == sa->phonBuf[sa->phonReadPos].pos)) {
		PICODBG_DEBUG(
			("ignoring phone at sa->phonBuf[%i] because it has pos==IGNORE", sa->phonReadPos));
		sa->phonReadPos++;
	}
...
static picodata_step_result_t saStep(register picodata_ProcessingUnit this,
	...
	*numBytesOutput = 0;
	while (1) { /* exit via return */
		PICODBG_DEBUG(("doing state %i, hLen|c1Len|c2Len: %d|%d|%d", sa->procState, sa->headxLen,
					   sa->cbuf1Len, sa->cbuf2Len));
		...
		switch (sa->procState) {
			...
			case SA_STEPSTATE_COLLECT:
				...
				while (sa->inspaceok && sa->needsmoreitems &&
					   (PICO_OK == (rv = picodata_cbGetItem(this->cbIn, sa->tmpbuf,
															PICOSA_MAXITEMSIZE, &blen)))) {
					rvP = picodata_get_itemparts(
						sa->tmpbuf, PICOSA_MAXITEMSIZE, &(sa->headx[sa->headxLen].head),
						&(sa->cbuf1[sa->cbuf1Len]), sa->cbuf1BufSize - sa->cbuf1Len, &clen);
					if (rvP != PICO_OK) {
						PICODBG_ERROR(("problem getting item parts"));
						picoos_emRaiseException(this->common->em, rvP, NULL, NULL);
						return PICODATA_PU_ERROR;
					}

					/* if CMD(...FLUSH...) -> PUNC(...FLUSH...),
					   construct PUNC-FLUSH item in headx */
					if ((sa->headx[sa->headxLen].head.type == PICODATA_ITEM_CMD) &&
						(sa->headx[sa->headxLen].head.info1 == PICODATA_ITEMINFO1_CMD_FLUSH)) {
						sa->headx[sa->headxLen].head.type = PICODATA_ITEM_PUNC;
						sa->headx[sa->headxLen].head.info1 = PICODATA_ITEMINFO1_PUNC_FLUSH;
						sa->headx[sa->headxLen].head.info2 = PICODATA_ITEMINFO2_PUNC_SENT_T;
					...
					if ((sa->headx[sa->headxLen].head.type == PICODATA_ITEM_CMD) &&
						(sa->headx[sa->headxLen].head.info1 == PICODATA_ITEMINFO1_CMD_PHONEME) &&
						(sa->headx[sa->headxLen].head.info2 == PICODATA_ITEMINFO2_CMD_START)) {
						picoos_uint8 i;
						...
						i = 0;
						while ((i < sa->headx[sa->headxLen].head.len) &&
							   (wordsep != sa->cbuf1[sa->headx[sa->headxLen].cind + i])) {
							PICODBG_INFO(("accepting phoneme %i",
										  sa->cbuf1[sa->headx[sa->headxLen].cind + i]));
				...
				if (!sa->needsmoreitems) {
					...
					sa->headx[sa->headxLen].head.type = PICODATA_ITEM_PUNC;
					sa->headx[sa->headxLen].head.info1 = PICODATA_ITEMINFO1_PUNC_PHRASEEND;
					sa->headx[sa->headxLen].head.info2 = PICODATA_ITEMINFO2_PUNC_PHRASE_FORCED;
					sa->headx[sa->headxLen].head.len = 0;
				...
				} else if ((rv == PICO_EXC_BUF_UNDERFLOW) || (rv == PICO_EXC_BUF_OVERFLOW)) {
			...
			case SA_STEPSTATE_PROCESS_POSD:
				...
				if (1) {
					...
					for (i = 0; i < sa->headxLen; i++) {
						...
						if ((32 <= sa->headx[i].head.info1) && (sa->headx[i].head.info1 < 127) &&
						...
						if ((32 <= sa->headx[i].head.info2) && (sa->headx[i].head.info2 < 127)) {
						...
						for (j = 0; j < sa->headx[i].head.len; j++) {
							if ((ittype == PICODATA_ITEM_WORDGRAPH) ||
								...
								PICODBG_INFO_MSG(("%c", sa->cbuf1[sa->headx[i].cind + j]));
							} else {
								PICODBG_INFO_MSG(("%4d", sa->cbuf1[sa->headx[i].cind + j]));
							}
						}
						PICODBG_INFO_MSG(("\n"));
					}
			...
			case SA_STEPSTATE_PROCESS_WPHO:
				...
				if (sa->headxLen > 0) {
					...
					if (sa->cbuf2Len > 0) {
						...
						picoos_emRaiseWarning(this->common->em, PICO_WARN_PU_DISCARD_BUF, NULL,
											  NULL);
					}
					...
					for (i = 0; i < sa->headxLen; i++) {
						switch (sa->headx[i].head.type) {
							case PICODATA_ITEM_WORDGRAPH:
								if (PICO_OK != saGraphemeToPhoneme(this, sa, i)) {
									/* not possible, phones skipped if needed */
									picoos_emRaiseException(this->common->em, PICO_ERR_OTHER, NULL,
															NULL);
									return PICODATA_PU_ERROR;
								}
								break;
							...
							default:
								...
								if (PICO_OK != saCopyItemContent1to2(this, sa, i)) {
									/* not possible, phones skipped if needed */
									picoos_emRaiseException(this->common->em, PICO_ERR_OTHER, NULL,
															NULL);
									return PICODATA_PU_ERROR;
					...
					for (i = 0; i < sa->headxLen; i++) {
						...
						for (j = 0; j < sa->headx[i].head.len; j++) {
							if ((ittype == PICODATA_ITEM_CMD)) {
								PICODBG_INFO_MSG(("%c", sa->cbuf2[sa->headx[i].cind + j]));
							} else {
								PICODBG_INFO_MSG(("%4d", sa->cbuf2[sa->headx[i].cind + j]));
							}
						}
						PICODBG_INFO_MSG(("\n"));
					}
			...
			case SA_STEPSTATE_PROCESS_TRNS_PARSE:
				...
				PICODBG_DEBUG(
					("transduce item (bot, remain): (%d, %d)", sa->headxBottom, sa->headxLen));
				...
				rv = picodata_put_itemparts(&(sa->headx[sa->headxBottom].head),
				...
				if (PICODATA_ITEM_WORDPHON == sa->headx[sa->headxBottom].head.type) {
					...
					if (PICO_OK == rv) {
						...
						PICODBG_WARN(
							("PARSE phone extraction returned exception %i, output WORDPHON "
							 "untransduced",
							 rv));
					}
				} else {
				...
				if (SA_STEPSTATE_FEED == sa->procState) {
					PICODATA_INFO_ITEM(this->voice->kbArray[PICOKNOW_KBID_DBG],
									   (picoos_uint8*)"sa-p: ", sa->tmpbuf, PICOSA_MAXITEMSIZE);
				}
			...
			case SA_STEPSTATE_PROCESS_TRNS_FST:
				...
				if (0 == sa->numFsts) {
					PICODBG_DEBUG(
						("doing trivial sylabification with %i phones", sa->phonWritePos));
#if defined(PICO_DEBUG)
					{
						PICODBG_INFO_CTX();
						PICODBG_INFO_MSG(("sa trying to trivially syllabify: "));
						PICOTRNS_PRINTSYMSEQ(this->voice->kbArray[PICOKNOW_KBID_DBG], sa->phonBuf,
											 sa->phonWritePos);
						PICODBG_INFO_MSG(("\n"));
					}
#endif

					picotrns_trivial_syllabify(sa->tabphones, sa->phonBuf, sa->phonWritePos,
											   sa->phonBufOut, &sa->phonWritePos,
											   PICOTRNS_MAX_NUM_POSSYM);
					PICODBG_DEBUG(
						("returned from trivial sylabification with %i phones", sa->phonWritePos));
#if defined(PICO_DEBUG)
					{
						PICODBG_INFO_CTX();
					...
					picotrns_eliminate_epsilons(sa->phonBufOut, sa->phonWritePos, sa->phonBuf,
												&sa->phonWritePos, PICOTRNS_MAX_NUM_POSSYM);
					PICODBG_DEBUG(
						("returning from epsilon elimination with %i phones", sa->phonWritePos));
					sa->phonReadPos = 0;
...
#endif
					picotrns_transduce(sa->fst[sa->curFst], FALSE, picotrns_printSolution,
									   sa->phonBuf, sa->phonWritePos, sa->phonBufOut,
									   &sa->phonWritePos, PICOTRNS_MAX_NUM_POSSYM, sa->altDescBuf,
				...
				/*
				 The trasduction output will contain equivalent items i.e. (x,y')  for each (x,y)
				 plus inserted deep symbols (-1,d). In case of deletions, (x,0) might also be
				 omitted...
				 */
				/* eliminate deep epsilons */
				picotrns_eliminate_epsilons(sa->phonBufOut, sa->phonWritePos, sa->phonBuf,
			...
			case SA_STEPSTATE_FEED:
				...
				if (sa->phonesTransduced) {
					...
					while (SA_POSSYM_OK == (rv = getNextPosSym(sa, &pos, &sym, sa->nextReadPos))) {
						PICODBG_TRACE(
							("FEED inserting phoneme %c into inBuf[%i]", sym, phonWritePos));
						sym = picotrns_unplane(sym, &plane);
						PICODBG_ASSERT((PICOKFST_PLANE_PHONEMES == plane));
						sa->tmpbuf[phonWritePos++] = (picoos_uint8)sym;
					}
					PICODBG_DEBUG(
						("FEED setting item length to %i", phonWritePos - PICODATA_ITEM_HEADSIZE));
					picodata_set_itemlen(sa->tmpbuf, PICODATA_ITEM_HEADSIZE,
										 phonWritePos - PICODATA_ITEM_HEADSIZE);
				...
				rvP = picodata_cbPutItem(this->cbOut, sa->tmpbuf, PICOSA_MAXITEMSIZE, &clen);
				...
				*numBytesOutput += clen;
				...
				PICODATA_INFO_ITEM(this->voice->kbArray[PICOKNOW_KBID_DBG],
								   (picoos_uint8*)"sana: ", sa->tmpbuf, PICOSA_MAXITEMSIZE);
