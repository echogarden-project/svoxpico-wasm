 ...
 *
 */
picoos_int32 picobase_utf8_length(const picoos_uint8* utf8str, const picoos_uint16 maxlen) {
	picoos_uint16 i;
	picoos_uint16 len;
	picoos_uint8 follow;
	picoos_uint8 ok;
	...
	while (ok && (i < maxlen) && (utf8str[i] != '\000')) {
		if (follow > 0) {
			if ((utf8str[i] >= (picoos_uint8)'\200') && (utf8str[i] < (picoos_uint8)'\300')) {
				follow--;
			} else {
				ok = FALSE;
			}
 ...
 */
static void picobase_get_utf8char(picoos_uint8 utf8[],
								  picoos_int32* pos,
...
picoos_uint8 picobase_get_next_utf8char(const picoos_uint8* utf8s,
	...
	if ((((*pos) + len) > utf8slenmax) || (len > PICOBASE_UTF8_MAXLEN)) {
		return FALSE;
	}
...
picoos_uint8 picobase_get_prev_utf8char(const picoos_uint8* utf8s,
	...
	poscnt = (*pos) - 1;
	i = 1;
	while ((i <= PICOBASE_UTF8_MAXLEN) && (poscnt >= utf8slenmin) && (utf8s[poscnt] != 0)) {
...
static picobase_utf32 picobase_utf8_to_utf32(picoos_uint8 utf8[], picoos_uint8* done) {
	...
	if ((utf8[0] < (picoos_uint8)'\200')) {
		...
		return ((((262144 * (utf8[0] % 8)) + (4096 * (utf8[1] % 64))) + (64 * (utf8[2] % 64))) +
				(utf8[3] % 64));
...
static picoos_int32 picobase_utf32_to_utf8(picobase_utf32 utf32,
										   picobase_utf8 utf8[],
										   picoos_int32 utf8MaxLen,
										   picoos_uint8* done) {
	picoos_int32 len;
...
extern picoos_int32 picobase_lowercase_utf8_str(picoos_uchar utf8str[],
												picoos_char lowercase[],
												int lowercaseMaxLen,
												picoos_uint8* done) {
	picobase_utf8char utf8char;
	picoos_int32 i;
...
extern picoos_int32 picobase_uppercase_utf8_str(picoos_uchar utf8str[],
												picoos_char uppercase[],
												int uppercaseMaxLen,
												picoos_uint8* done) {
	picobase_utf8char utf8char;
	picoos_int32 i;
