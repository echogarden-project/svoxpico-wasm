...
static pico_status_t ktabIdsInitialize(register picoknow_KnowledgeBase this, picoos_Common common) {
...
static pico_status_t ktabGraphsInitialize(register picoknow_KnowledgeBase this,
	...
	ktabgraphs->nrOffset = ((int)(this->base[KTAB_START_GRAPHS_NR_OFFSET])) +
						   256 * (int)(this->base[KTAB_START_GRAPHS_NR_OFFSET + 1]);
...
picoos_uint8 picoktab_hasVowellikeProp(const picoktab_Graphs this,
	...
	return g->graphTable[graphsOffset +
						 ktab_propOffset(this, graphsOffset, KTAB_GRAPH_PROPSET_TOKENTYPE)] ==
		   PICODATA_ITEMINFO1_TOKTYPE_LETTERV;
}

static void ktab_getStrProp(const picoktab_Graphs this,
							picoos_uint32 graphsOffset,
 ...
 If the property is found, a value > 0 is returned otherwise 0 */
{
	...
	if ((g->graphTable[graphsOffset] & prop) == prop) {
		...
		if (prop > KTAB_GRAPH_PROPSET_TO) {
			if ((g->graphTable[graphsOffset] & KTAB_GRAPH_PROPSET_TO) == KTAB_GRAPH_PROPSET_TO) {
				n = n + picobase_det_utf8_length(
							g->graphTable[graphsOffset + n]); /* overread TO field */
			}
		} else {
			return n;
		}
		...
		if (prop > KTAB_GRAPH_PROPSET_LOWERCASE) {
			if ((g->graphTable[graphsOffset] & KTAB_GRAPH_PROPSET_LOWERCASE) ==
				...
				n = n + picobase_det_utf8_length(
							g->graphTable[graphsOffset + n]); /* overread lowercase field */
			}
		} else {
			return n;
		}
		if (prop > KTAB_GRAPH_PROPSET_GRAPHSUBS1) {
			if ((g->graphTable[graphsOffset] & KTAB_GRAPH_PROPSET_GRAPHSUBS1) ==
				...
				n = n + picobase_det_utf8_length(
							g->graphTable[graphsOffset + n]); /* overread graphsubs1 field */
			}
		} else {
			return n;
		}
		if (prop > KTAB_GRAPH_PROPSET_GRAPHSUBS2) {
			if ((g->graphTable[graphsOffset] & KTAB_GRAPH_PROPSET_GRAPHSUBS2) ==
				...
				n = n + picobase_det_utf8_length(
							g->graphTable[graphsOffset + n]); /* overread graphsubs2 field */
			}
		} else {
			return n;
		}
	...
	if (g->nrOffset > 0) {
		...
		do {
			...
			if (g->sizeOffset == 1) {
				...
				graphsOffset =
					g->offsetTable[g->sizeOffset * m] + 256 * g->offsetTable[g->sizeOffset * m + 1];
				/* PICODBG_DEBUG(("picoktab_graphOffset: %i %i %i %i", m,
				 * g->offsetTable[g->sizeOffset*m], g->offsetTable[g->sizeOffset*m + 1],
				 * graphsOffset));
				 */
			}
			...
			/* PICODBG_DEBUG(("picoktab_graphOffset: %i %i %i '%s' '%s' '%s'", a, m, b, from,
			 * utf8graph, to));
			 */
...
void picoktab_graphsGetGraphInfo(const picoktab_Graphs this,
	...
	if (g->sizeOffset == 1) {
		...
		graphsOffset = g->offsetTable[2 * graphIndex] + (g->offsetTable[2 * graphIndex + 1] << 8);
	}
...
picoos_uint8 picoktab_hasVowelProp(const picoktab_Phones this, const picoos_uint8 ch) {
	return (KTAB_PPROP_VOWEL & ((ktabphones_SubObj)this)->props[ch]);
}

picoos_uint8 picoktab_hasDiphthProp(const picoktab_Phones this, const picoos_uint8 ch) {
	return (KTAB_PPROP_DIPHTH & ((ktabphones_SubObj)this)->props[ch]);
}

picoos_uint8 picoktab_hasGlottProp(const picoktab_Phones this, const picoos_uint8 ch) {
	return (KTAB_PPROP_GLOTT & ((ktabphones_SubObj)this)->props[ch]);
}

picoos_uint8 picoktab_hasNonsyllvowelProp(const picoktab_Phones this, const picoos_uint8 ch) {
	return (KTAB_PPROP_NONSYLLVOWEL & ((ktabphones_SubObj)this)->props[ch]);
}

picoos_uint8 picoktab_hasSyllconsProp(const picoktab_Phones this, const picoos_uint8 ch) {
	return (KTAB_PPROP_SYLLCONS & ((ktabphones_SubObj)this)->props[ch]);
}

picoos_bool picoktab_isSyllCarrier(const picoktab_Phones this, const picoos_uint8 ch) {
	picoos_uint8 props;
	props = ((ktabphones_SubObj)this)->props[ch];
	return (((KTAB_PPROP_VOWEL & props) && !(KTAB_PPROP_NONSYLLVOWEL & props)) ||
			(KTAB_PPROP_SYLLCONS & props));
}

picoos_bool picoktab_isPrimstress(const picoktab_Phones this, const picoos_uint8 ch) {
	return (ch == ((ktabphones_SubObj)this)->specids[KTAB_IND_PRIMSTRESS]);
}

picoos_bool picoktab_isSecstress(const picoktab_Phones this, const picoos_uint8 ch) {
	return (ch == ((ktabphones_SubObj)this)->specids[KTAB_IND_SECSTRESS]);
}

picoos_bool picoktab_isSyllbound(const picoktab_Phones this, const picoos_uint8 ch) {
	return (ch == ((ktabphones_SubObj)this)->specids[KTAB_IND_SYLLBOUND]);
}

picoos_bool picoktab_isWordbound(const picoktab_Phones this, const picoos_uint8 ch) {
	return (ch == ((ktabphones_SubObj)this)->specids[KTAB_IND_WORDBOUND]);
}

picoos_bool picoktab_isPause(const picoktab_Phones this, const picoos_uint8 ch) {
...
static pico_status_t ktabPosInitialize(register picoknow_KnowledgeBase this, picoos_Common common) {
	...
	for (i = 0, pos = 0; i < PICOKTAB_MAXNRPOS_IN_COMB; i++, pos += 4) {
		ktabpos->nrcomb[i] = ((picoos_uint16)(this->base[pos + 1])) << 8 | this->base[pos];
		if (ktabpos->nrcomb[i] > 0) {
			osprev = os;
			os = ((picoos_uint16)(this->base[pos + 3])) << 8 | this->base[pos + 2];
			ktabpos->nrcombstart[i] = &(this->base[os]);
			PICODBG_TRACE(
				("i %d, pos %d, nr %d, osprev %d, os %d", i, pos, ktabpos->nrcomb[i], osprev, os));
			if (osprev >= os) {
...
picoos_bool picoktab_isUniquePos(const picoktab_Pos this, const picoos_uint8 pos) {
	ktabpos_subobj_t* ktabpos;
	picoos_uint16 i;
	...
	while ((i < ktabpos->nrcomb[0]) && (pos > ktabpos->nrcombstart[0][i])) {
		PICODBG_TRACE(("compare with pos %d at position %d", ktabpos->nrcombstart[0][i], pos, i));
		i++;
	}
	...
	if ((poslistlen > 0) && (poslistlen <= PICOKTAB_MAXNRPOS_IN_COMB)) {
		...
		if (i > 0) {
			...
			if (!poscomb) {
				...
				PICODBG_WARN(
					("dynamically created POS combination not found in table; taking first (%i)",
					 poslist[0]));
				poscomb = poslist[0];
			}
