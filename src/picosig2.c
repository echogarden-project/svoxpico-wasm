...
static void get_simple_excitation(sig_innerobj_t* sig_inObj, picoos_int16* nextPeak);
static void enh_wind_init(sig_innerobj_t* sig_inObj);
static void init_rand(sig_innerobj_t* sig_inObj);
static void get_trig(picoos_int32 ang, picoos_int32* table, picoos_int32* cs, picoos_int32* sn);
...
pico_status_t sigAllocate(picoos_MemoryManager mm, sig_innerobj_t* sig_inObj) {
	...
	sig_inObj->int_vec22 = sig_inObj->int_vec23 = sig_inObj->int_vec24 = sig_inObj->int_vec25 =
		sig_inObj->int_vec26 = sig_inObj->int_vec28 = sig_inObj->int_vec29 = sig_inObj->int_vec30 =
			sig_inObj->int_vec31 = sig_inObj->int_vec32 = sig_inObj->int_vec33 =
				sig_inObj->int_vec34 = sig_inObj->int_vec35 = sig_inObj->int_vec36 =
					sig_inObj->int_vec37 = sig_inObj->int_vec38 = sig_inObj->int_vec39 =
						sig_inObj->int_vec40 = NULL;

	sig_inObj->sig_vec1 = NULL;

	sig_inObj->idx_vect1 = sig_inObj->idx_vect2 = sig_inObj->idx_vect4 = NULL;
	sig_inObj->idx_vect5 = sig_inObj->idx_vect6 = sig_inObj->idx_vect7 = sig_inObj->idx_vect8 =
	 ...
	 * ------------------------------------------------------------------*/
	data_i = (picoos_int16*)picoos_allocate(mm, sizeof(picoos_int16) * PICODSP_FFTSIZE);
	if (NULL == data_i) {
	...
	data_i = (picoos_int16*)picoos_allocate(mm, sizeof(picoos_int16) * PICODSP_HFFTSIZE_P1);
	if (NULL == data_i) {
		sigDeallocate(mm, sig_inObj);
		return PICO_ERR_OTHER;
	}
	...
	data_i = (picoos_int16*)picoos_allocate(mm, sizeof(picoos_int16) * PICODSP_MAX_EX);
	if (NULL == data_i) {
		sigDeallocate(mm, sig_inObj);
		return PICO_ERR_OTHER;
	}
	...
	d32 = (picoos_int32*)picoos_allocate(mm, sizeof(picoos_int32) * PICODSP_FFTSIZE);
	if (NULL == d32) {
		sigDeallocate(mm, sig_inObj);
		return PICO_ERR_OTHER;
	}
	...
	d32 = (picoos_int32*)picoos_allocate(mm, sizeof(picoos_int32) * PICODSP_FFTSIZE * 2);
	if (NULL == d32) {
		sigDeallocate(mm, sig_inObj);
		return PICO_ERR_OTHER;
	}
	...
	d32 = (picoos_int32*)picoos_allocate(mm, sizeof(picoos_int32) * PICODSP_N_RAND_TABLE);
	if (NULL == d32) {
		sigDeallocate(mm, sig_inObj);
		return PICO_ERR_OTHER;
	}
	...
	d32 = (picoos_int32*)picoos_allocate(mm, sizeof(picoos_int32) * PICODSP_HFFTSIZE_P1);
	if (NULL == d32) {
		sigDeallocate(mm, sig_inObj);
		return PICO_ERR_OTHER;
	}
	sig_inObj->int_vec39 = d32;
	d32 = (picoos_int32*)picoos_allocate(mm, sizeof(picoos_int32) * (1 + PICODSP_COS_TABLE_LEN));
	if (NULL == d32) {
		sigDeallocate(mm, sig_inObj);
		return PICO_ERR_OTHER;
	}
	...
	d32 = (picoos_int32*)picoos_allocate(
		mm, sizeof(picoos_int32) * PICODSP_FFTSIZE * 2); /* - fixed point */
	if (NULL == d32) {
		sigDeallocate(mm, sig_inObj);
		return PICO_ERR_OTHER;
	}
...
void sigDspInitialize(sig_innerobj_t* sig_inObj, picoos_int32 resetMode) {
	...
	sig_inObj->VCutoff_p =
		PICODSP_V_CUTOFF_FREQ; /*voicing cut off frequency in Hz (will be modeled in the future)*/
	sig_inObj->UVCutoff_p = PICODSP_UV_CUTOFF_FREQ; /*unvoiced frames only (periodize lowest
													   components to mask bad voicing transitions)*/
	...
	sig_inObj->voxbnd2_p = (picoos_int32)((picoos_single)sig_inObj->hfftsize_p /
										  ((picoos_single)sig_inObj->Fs_p / (picoos_single)2) *
										  (picoos_single)sig_inObj->UVCutoff_p);
	sig_inObj->hop_p = sig_inObj->framesz_p;
	sig_inObj->nextPeak_p =
		(((int)(PICODSP_FFTSIZE)) / ((int)PICODSP_DISPLACE) - 1) * sig_inObj->hop_p;
	...
	for (i = 0; i < PICODSP_FFTSIZE; i++) {
		sig_inObj->idx_vect1[i] = sig_inObj->idx_vect4[i] = sig_inObj->idx_vect5[i] =
			sig_inObj->idx_vect6[i] = 0;
...
void mel_2_lin_lookup(sig_innerobj_t* sig_inObj, picoos_uint32 scmeanMGC) {
	...
	i = sizeof(picoos_int32) *
		(PICODSP_FFTSIZE - m1);	 // fix 64-bit segfault by removing +1 ?? <> GMN
	...
	/* *****************************************************************************************
	 ...
	 - Start from 1 and stop at PICODSP_H_FFTSIZE-1 because 0 and PICODSP_H_FFTSIZE are invariant
	 points
...
void phase_spec2(sig_innerobj_t* sig_inObj) {
	...
	if (voiced == 1) {
		...
		FAST_DEVICE(VOXBND_M1, *(tmp1++) += *(tmp2)-PICODSP_FIX_SCALE2;
					*(tmp2) = (*tmp2 >= 0) ? (*tmp2) >> PICODSP_SHIFT_FACT4
										   : -((-(*tmp2)) >> PICODSP_SHIFT_FACT4);
					tmp2++);
		*tmp2 = (*tmp2 >= 0) ? (*tmp2) >> PICODSP_SHIFT_FACT4
							 : -((-(*tmp2)) >> PICODSP_SHIFT_FACT4); /*ang[voxbnd-1]/=2;*/
	}
	...
	*so = 0;
	...
	if (sig_inObj->iRand > PICODSP_N_RAND_TABLE - PICODSP_HFFTSIZE_P1) {
		sig_inObj->iRand = 1 + sig_inObj->iRand + PICODSP_HFFTSIZE_P1 - PICODSP_N_RAND_TABLE;
	}
} /*phase_spec2*/
...
void impulse_response(sig_innerobj_t* sig_inObj) {
	...
	*E = norm_result(m2, fr, norm_window);
	...
	t1 = fr;
	FAST_DEVICE(PICODSP_FFTSIZE, *(t1++) /= ff;); /* - fixed point */
...
void td_psola2(sig_innerobj_t* sig_inObj) {
	...
	t1 = &(v1[2 * PICODSP_FFTSIZE - PICODSP_DISPLACE]);
	FAST_DEVICE(PICODSP_DISPLACE, *(t1++) = 0;);
...
void save_transition_frame(sig_innerobj_t* sig_inObj) {
	...
	if (sig_inObj->voiced_p != sig_inObj->prevVoiced_p) {
		...
		if (sig_inObj->voiced_p == 1) {
			...
			sig_inObj->nU = 0; /*to avoid problems in case of very short voiced or unvoiced parts
								  (less than 4 frames long)*/
		}
	}
...
static void get_simple_excitation(sig_innerobj_t* sig_inObj, picoos_int16* nextPeak) {
	...
	*nextPeak -= hop;
	...
	if (voiced == 0) { /*Unvoiced*/
		...
		Ti = (picoos_int32)(rounding +
							(picoos_single)Fs / (picoos_single)sig_inObj->Fuv_p); /* round Period*/
		sqrtTi = (picoos_int32)(E * sqrt((double)Fs / (hop * sig_inObj->Fuv_p)) * fact *
								PICODSP_GETEXC_K1);
		while (k < winlen) {
			if (k < winlen) {
		...
		Ti = (picoos_int32)(rounding + (picoos_single)Fs / (picoos_single)F0); /*Period*/
		sqrtTi = (picoos_int32)(E * sqrt((double)Fs / (hop * sig_inObj->F0_p)) * fact *
								PICODSP_GETEXC_K1);
		while (k < winlen) {
