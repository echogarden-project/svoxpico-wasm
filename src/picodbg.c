...
#define PICODBG_DEFAULT_FORMAT (PICODBG_SHOW_LEVEL | PICODBG_SHOW_SRCNAME | PICODBG_SHOW_FUNCTION)
...
static void picodbg_logToStream(int level, int donewline, const char* context, const char* msg) {
	int oldAttr = 0;
...
void picodbg_assert(const char* file, int line, const char* func, const char* expr) {
	if (strlen(func) > 0) {
		fprintf(STDDBG, "assertion failed: %s, file %s, function %s, line %d", expr,
				picodbg_fileTitle(file), func, line);
	} else {
		fprintf(STDDBG, "assertion failed: %s, file %s, line %d", expr, picodbg_fileTitle(file),
				line);
	}
	picodbg_terminate();
	abort();
