...
#define SET_MAGIC_NUMBER(sys) (sys)->magic = ((picoos_uint32)(sys)) ^ MAGIC_MASK

#define CHECK_MAGIC_NUMBER(sys) ((sys)->magic == (((picoos_uint32)(sys)) ^ MAGIC_MASK))
...
pico_Status pico_initialize_priv(void* memory,
	...
	if (memory == NULL) {
		...
		sys = (pico_System)picoos_raw_malloc(memory, size, sizeof(pico_system_t), &rest_mem,
											 &rest_mem_size);
		if (sys != NULL) {
 ...
 * @callergraph
 */
PICO_FUNC pico_initialize(void* memory, const pico_Uint32 size, pico_System* system) {
 ...
 * @callergraph
 */
PICO_FUNC pico_terminate(pico_System* system) {
	pico_Status status = PICO_OK;
 ...
 * @callergraph
 */
PICO_FUNC pico_getSystemStatusMessage(pico_System system,
									  pico_Status errCode,
	...
	if (!is_valid_system_handle(system)) {
		status = PICO_ERR_INVALID_HANDLE;
		if (outMessage != NULL) {
			picoos_strlcpy((picoos_char*)outMessage, (picoos_char*)"'system' not initialized",
						   PICO_RETSTRINGSIZE);
		}
		...
		if (picoos_emGetExceptionCode(system->common->em) == PICO_OK) {
			if (errCode == PICO_OK) {
				picoos_strlcpy((picoos_char*)outMessage, (picoos_char*)"system ok",
							   PICO_RETSTRINGSIZE);
			} else {
				/* exceptionManager was not informed yet; produce default message */
				picoos_setErrorMsg((picoos_char*)outMessage, PICO_RETSTRINGSIZE, errCode, NULL,
								   NULL, NULL);
			}
		} else {
			picoos_emGetExceptionMessage(system->common->em, (picoos_char*)outMessage,
										 PICO_RETSTRINGSIZE);
		}
	}
 ...
 * @callergraph
 */
PICO_FUNC pico_getNrSystemWarnings(pico_System system, pico_Int32* outNrOfWarnings) {
	pico_Status status = PICO_OK;
 ...
 * @callergraph
 */
PICO_FUNC pico_getSystemWarning(pico_System system,
	...
	if (!is_valid_system_handle(system)) {
		...
		*outCode = picoos_emGetWarningCode(system->common->em, warningIndex);
		picoos_emGetWarningMessage(system->common->em, warningIndex, (picoos_char*)outMessage,
								   (picoos_uint16)PICO_RETSTRINGSIZE);
	}
 ...
 * @callergraph
 */
PICO_FUNC pico_loadResource(pico_System system,
	...
	if (!is_valid_system_handle(system)) {
		...
		status = picorsrc_loadResource(system->rm, (picoos_char*)lingwareFileName,
									   (picorsrc_Resource*)outLingware);
 ...
 * @callergraph
 */
PICO_FUNC pico_unloadResource(pico_System system, pico_Resource* inoutLingware) {
	pico_Status status = PICO_OK;
 ...
 * @callergraph
 */
PICO_FUNC pico_getResourceName(pico_System system, pico_Resource resource, pico_Retstring outName) {
	if (!is_valid_system_handle(system)) {
		return PICO_ERR_INVALID_HANDLE;
	} else if (NULL == outName) {
		return PICO_ERR_NULLPTR_ACCESS;
	}
	return picorsrc_rsrcGetName((picorsrc_Resource)resource, (picoos_char*)outName,
								PICO_RETSTRINGSIZE);
}
 ...
 * @callergraph
 */
PICO_FUNC pico_createVoiceDefinition(pico_System system, const pico_Char* voiceName) {
	pico_Status status = PICO_OK;
 ...
 * @callergraph
 */
PICO_FUNC pico_addResourceToVoiceDefinition(pico_System system,
	...
	if (!is_valid_system_handle(system)) {
		...
		picoos_emReset(system->common->em);
		status = picorsrc_addResourceToVoiceDefinition(system->rm, (picoos_char*)voiceName,
 ...
 * @callergraph
 */
PICO_FUNC pico_releaseVoiceDefinition(pico_System system, const pico_Char* voiceName) {
	pico_Status status = PICO_OK;
 ...
 * @callergraph
 */
PICO_FUNC pico_newEngine(pico_System system, const pico_Char* voiceName, pico_Engine* outEngine) {
	pico_Status status = PICO_OK;
			...
			*outEngine = (pico_Engine)picoctrl_newEngine(system->common->mm, system->rm, voiceName);
			if (*outEngine != NULL) {
				...
				status = picoos_emRaiseException(system->common->em, PICO_EXC_OUT_OF_MEM,
												 (picoos_char*)"out of memory creating new engine",
												 NULL);
			}
		} else {
			status = picoos_emRaiseException(system->common->em, PICO_EXC_MAX_NUM_EXCEED, NULL,
											 (picoos_char*)"no more than %i engines", 1);
		}
	}
 ...
 * @callergraph
 */
PICO_FUNC pico_disposeEngine(pico_System system, pico_Engine* inoutEngine) {
	pico_Status status = PICO_OK;
...
PICO_FUNC pico_putTextUtf8(pico_Engine engine,
	...
	if (!picoctrl_isValidEngineHandle((picoctrl_Engine)engine)) {
		...
		status =
			picoctrl_engFeedText((picoctrl_Engine)engine, (picoos_char*)text, textSize, bytesPut);
	}
 ...
 * @callergraph
 */
PICO_FUNC pico_getData(pico_Engine engine,
					   void* buffer,
	...
	if (!picoctrl_isValidEngineHandle((picoctrl_Engine)engine)) {
		...
		picoctrl_engResetExceptionManager((picoctrl_Engine)engine);
		status = picoctrl_engFetchOutputItemBytes((picoctrl_Engine)engine, (picoos_char*)buffer,
 ...
 * @callergraph
 */
PICO_FUNC pico_resetEngine(pico_Engine engine, pico_Int32 resetMode) {
	pico_Status status = PICO_OK;
 ...
 * @callergraph
 */
PICO_FUNC pico_getEngineStatusMessage(pico_Engine engine,
									  pico_Status errCode,
									  pico_Retstring outMessage) {
	pico_Status status = PICO_OK;
	...
	if (!picoctrl_isValidEngineHandle((picoctrl_Engine)engine)) {
		status = PICO_ERR_INVALID_HANDLE;
		if (outMessage != NULL) {
			picoos_strlcpy((picoos_char*)outMessage, (picoos_char*)"'engine' not initialized",
						   PICO_RETSTRINGSIZE);
		}
	} else if (outMessage == NULL) {
		status = PICO_ERR_NULLPTR_ACCESS;
		...
		if (picoos_emGetExceptionCode(common->em) == PICO_OK) {
			if (errCode == PICO_OK) {
				picoos_strlcpy((picoos_char*)outMessage, (picoos_char*)"engine ok",
							   PICO_RETSTRINGSIZE);
			} else {
				/* exceptionManager was not informed yet; produce default message */
				picoos_setErrorMsg((picoos_char*)outMessage, PICO_RETSTRINGSIZE, errCode, NULL,
 ...
 * @callergraph
 */
PICO_FUNC pico_getNrEngineWarnings(pico_Engine engine, pico_Int32* outNrOfWarnings) {
	pico_Status status = PICO_OK;
 ...
 * @callergraph
 */
PICO_FUNC pico_getEngineWarning(pico_Engine engine,
								const pico_Int32 warningIndex,
								pico_Status* outCode,
								pico_Retstring outMessage) {
	pico_Status status = PICO_OK;
	...
	if (!picoctrl_isValidEngineHandle((picoctrl_Engine)engine)) {
		...
		*outCode = picoos_emGetWarningCode(common->em, warningIndex);
		picoos_emGetWarningMessage(common->em, warningIndex, (picoos_char*)outMessage,
								   (picoos_uint16)PICO_RETSTRINGSIZE);
	}
