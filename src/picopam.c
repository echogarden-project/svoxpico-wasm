...
static picoos_uint8 pam_map_sentence_type(picoos_uint8 iteminfo1, picoos_uint8 iteminfo2);
static picoos_uint8 pam_map_phrase_type(picoos_uint8 iteminfo1, picoos_uint8 iteminfo2);

/*Adapter*/
static pico_status_t pam_reset_processors(register picodata_ProcessingUnit this);
static pico_status_t pam_reset_processors_back(register picodata_ProcessingUnit this);
static pico_status_t pam_create_syllable(register picodata_ProcessingUnit this,
										 picoos_uint8 syllType,
										 picoos_uint8* sContent,
										 picoos_uint8 sentType,
										 picoos_uint8 phType,
										 picoos_uint8 uBoundType,
										 picoos_uint16 uMin,
										 picoos_uint16 uMax);
static pico_status_t pam_process_event_feature(register picodata_ProcessingUnit this,
											   picoos_uint8 nFeat,
											   picoos_uint8 event_type,
											   picoos_uint8 direction);
static pico_status_t pam_process_event(register picodata_ProcessingUnit this,
									   picoos_uint8 event_type,
									   picoos_uint8 direction);
static pico_status_t pam_adapter_forward_step(register picodata_ProcessingUnit this,
											  picoos_uint8* itemBase);
static pico_status_t pam_adapter_backward_step(register picodata_ProcessingUnit this);
...
static picoos_uint8 pam_do_tree(register picodata_ProcessingUnit this,
								const picokdt_DtPAM dtpam,
								const picoos_uint8* invec,
								const picoos_uint8 inveclen,
								picokdt_classify_result_t* dtres);
static pico_status_t pam_get_f0(register picodata_ProcessingUnit this,
								picoos_uint16* lf0Index,
								picoos_uint8 nState,
								picoos_single* phonF0);
...
static pico_status_t pam_put_item(register picodata_ProcessingUnit this,
								  picoos_uint8* outBuff,
								  picoos_uint16 outWritePos,
...
static pico_status_t pam_allocate(picoos_MemoryManager mm, pam_subobj_t* pam) {
	 ...
	 * PAM Local buffers ALLOCATION
	 ------------------------------------------------------------------*/
	/*PAM Local buffers*/
	data = (picopal_uint8*)picoos_allocate(mm, sizeof(sFtVect) * PICOPAM_MAX_SYLL_PER_SENT);
	if (data == NULL) {
		return PICO_ERR_OTHER;
	}
	pam->sSyllFeats = (sFtVect*)data;

	data = (picopal_uint8*)picoos_allocate(mm, sizeof(picopal_uint8) * PICOPAM_MAX_PH_PER_SENT);
	if (data == NULL) {
	...
	data = (picopal_uint8*)picoos_allocate(mm, sizeof(picopal_uint8) * PICOPAM_VECT_SIZE);
	if (data == NULL) {
		pam_deallocate(mm, pam);
		return PICO_ERR_OTHER;
	}
	...
	data =
		(picopal_uint8*)picoos_allocate(mm, sizeof(picopal_uint8) * PICOPAM_MAX_ITEM_SIZE_PER_SENT);
	if (data == NULL) {
		pam_deallocate(mm, pam);
		return PICO_ERR_OTHER;
	}
	...
	dataI = (picoos_int16*)picoos_allocate(mm, sizeof(picoos_int16) * PICOPAM_MAX_ITEM_PER_SENT);
	if (data == NULL) {
		pam_deallocate(mm, pam);
		return PICO_ERR_OTHER;
	}
...
static pico_status_t pam_initialize(register picodata_ProcessingUnit this, picoos_int32 resetMode) {
	...
	if (pam->dtdur == NULL) {
		picoos_emRaiseException(this->common->em, PICO_EXC_KB_MISSING, NULL, NULL);
		return PICO_ERR_OTHER;
	}
	PICODBG_DEBUG(("got dtdur"));

	/* kb dtlfz* */
	pam->dtlfz[0] = picokdt_getDtPAM(this->voice->kbArray[PICOKNOW_KBID_DT_LFZ1]);
	pam->dtlfz[1] = picokdt_getDtPAM(this->voice->kbArray[PICOKNOW_KBID_DT_LFZ2]);
	pam->dtlfz[2] = picokdt_getDtPAM(this->voice->kbArray[PICOKNOW_KBID_DT_LFZ3]);
	pam->dtlfz[3] = picokdt_getDtPAM(this->voice->kbArray[PICOKNOW_KBID_DT_LFZ4]);
	pam->dtlfz[4] = picokdt_getDtPAM(this->voice->kbArray[PICOKNOW_KBID_DT_LFZ5]);
	...
	/* kb dtmgc* */
	pam->dtmgc[0] = picokdt_getDtPAM(this->voice->kbArray[PICOKNOW_KBID_DT_MGC1]);
	pam->dtmgc[1] = picokdt_getDtPAM(this->voice->kbArray[PICOKNOW_KBID_DT_MGC2]);
	pam->dtmgc[2] = picokdt_getDtPAM(this->voice->kbArray[PICOKNOW_KBID_DT_MGC3]);
	pam->dtmgc[3] = picokdt_getDtPAM(this->voice->kbArray[PICOKNOW_KBID_DT_MGC4]);
	pam->dtmgc[4] = picokdt_getDtPAM(this->voice->kbArray[PICOKNOW_KBID_DT_MGC5]);
	...
	/* kb pdfdur* */
	pam->pdfdur = picokpdf_getPdfDUR(this->voice->kbArray[PICOKNOW_KBID_PDF_DUR]);
	if (pam->pdfdur == NULL) {
		picoos_emRaiseException(this->common->em, PICO_EXC_KB_MISSING, NULL, NULL);
		return PICO_ERR_OTHER;
	}
	...
	/* kb pdflfz* */
	pam->pdflfz = picokpdf_getPdfMUL(this->voice->kbArray[PICOKNOW_KBID_PDF_LFZ]);
	if (pam->pdflfz == NULL) {
		picoos_emRaiseException(this->common->em, PICO_EXC_KB_MISSING, NULL, NULL);
		return PICO_ERR_OTHER;
	}
	...
	/* kb tabphones */
	pam->tabphones = picoktab_getPhones(this->voice->kbArray[PICOKNOW_KBID_TAB_PHONES]);
 ...
 * @return  void
 * @remarks    so far initializes to 0 both values; this will leave the values given by tree
 * prediction
 * @callgraph
 * @callergraph
 */
 ...
 * @callergraph
 */
static pico_status_t check_phones_size(pam_subobj_t* pam, picoos_int16 neededSize) {
 ...
 * @callergraph
 */
static pico_status_t check_syllables_size(pam_subobj_t* pam, picoos_int16 neededSize) {
 ...
 * @callergraph
 */
static pico_status_t pamCheckResourceLimits(register picodata_ProcessingUnit this,
	...
	switch (head.type) {
		...
		case PICODATA_ITEM_BOUND:
			if ((head.info1 == PICODATA_ITEMINFO1_BOUND_SBEG) ||
				(head.info1 == PICODATA_ITEMINFO1_BOUND_SEND) ||
				(head.info1 == PICODATA_ITEMINFO1_BOUND_TERM) ||
				(head.info1 == PICODATA_ITEMINFO1_BOUND_PHR1)
		...
		default:
			...
			if ((pam->nAttachedItemsSize + head.len) >= PICOPAM_MAX_ITEM_SIZE_PER_SENT - 1) {
...
static pico_status_t pam_check_immediate(register picodata_ProcessingUnit this,
	...
	if (pam->nCurrSyllable <= -1) {
		...
		if ((item[0] == PICODATA_ITEM_BOUND) && (item[1] == PICODATA_ITEMINFO1_BOUND_SBEG)) {
			return FALSE;
		}
 ...
 * @callergraph
 */
static pico_status_t pam_queue(register picodata_ProcessingUnit this, const picoos_uint8* item) {
	register pam_subobj_t* pam;
	picodata_itemhead_t head;
	picoos_uint8 nI;
	pico_status_t sResult;
	...
	if (pam->nCurrSyllable > -1) {
		...
		if (pam->sSyllFeats[pam->nCurrSyllable].phoneV[ITM] == 0) {
			pam->sSyllFeats[pam->nCurrSyllable].phoneV[ITM] = pam->nLastAttachedItemId;
		}
		pam->sSyllFeats[pam->nCurrSyllable].phoneV[itm] = pam->nLastAttachedItemId;
	} else {
 ...
 * @return    FALSE : item should not be processed (maybe it ontains commands or items for other
 * PUs)
 * @remarks item pointed to by *item should be already valid
 * @callgraph
 * @callergraph
...
static picoos_uint8 pamHasToProcess(register picodata_ProcessingUnit this) {
	...
	nCond3 = pam->nSyllPhoneme >= pam->sSyllFeats[pam->nCurrSyllable].phoneV[B3];
...
static pico_status_t pamUpdateProcess(register picodata_ProcessingUnit this) {
	...
	if (pam->sSyllFeats[pam->nCurrSyllable].phoneV[ITM] > 0) {
		...
		if (pam->nCurrAttachedItem == 0) {
			...
			pam->nCurrAttachedItem = pam->sSyllFeats[pam->nCurrSyllable].phoneV[ITM];
			return PICO_OK;
		} else {
			/*not the first item : check if more*/
			if (pam->nCurrAttachedItem < pam->sSyllFeats[pam->nCurrSyllable].phoneV[itm]) {
...
static picoos_uint8* pamPopAttachedSy0(register picodata_ProcessingUnit this) {
	...
	if (pam->nCurrAttachedItem > 0) {
		...
		if (pam->nCurrAttachedItem > pam->sSyllFeats[pam->nCurrSyllable].phoneV[itm] - 1) {
			return NULL;
		}
	}
	nItem = pam->nCurrAttachedItem;
...
static pico_status_t pam_get_duration(register picodata_ProcessingUnit this,
	...
	if (durIndex > pdf->numframes - 1) {
		PICODBG_ERROR(("PAM durPdf access error, index overflow -> index: %d , numframes: %d",
					   durIndex, pdf->numframes));
		return PICO_ERR_OTHER;
	}
	/* base pointer */
...
static pico_status_t pam_get_f0(register picodata_ProcessingUnit this,
	...
	if (lf0IndexOffset > pam->pdflfz->numframes - 1) {
		PICODBG_ERROR(("PAM flfzPdf access error, index overflow -> index: %d , numframes: %d",
					   lf0Index, pam->pdflfz->numframes));
		return PICO_ERR_OTHER;
	}
	/* base pointer */
	...
	numstreams = 3;
	lfivar = (picoos_single)(((picoos_uint16)lfItem[numstreams * 2]) << pam->pdflfz->ivarpow[0]);
...
static pico_status_t pam_update_vector(register picodata_ProcessingUnit this) {
	...
	picoos_single fDur, f0avg, f0quant, minf0, maxf0, durquant1, durquant2, mindur, maxdur1,
		maxdur2;
...
static pico_status_t pam_put_item(register picodata_ProcessingUnit this,
	...
	if ((pam->nCurrSyllable == 0) && (pam->nSyllPhoneme == 0)) {
		...
		pos32 = Min;
		picoos_read_mem_pi_uint16(pam->sSyllFeats[pam->nCurrSyllable].phoneV, &pos32, &uMinDur);
		pos32 = Max;
		picoos_read_mem_pi_uint16(pam->sSyllFeats[pam->nCurrSyllable].phoneV, &pos32, &uMaxDur);
	...
	for (nI = 0; nI < PICOPAM_NRSTPF; nI++) {
		picoos_write_mem_pi_uint16(sDest, &pos, (picoos_uint16)pam->numFramesState[nI]);
		picoos_write_mem_pi_uint16(sDest, &pos, (picoos_uint16)pam->lf0Index[nI]);
		picoos_write_mem_pi_uint16(sDest, &pos, (picoos_uint16)pam->mgcIndex[nI]);
	}
	*bytesWr = sizeof(picodata_itemhead_t) + sizeof(picoos_uint16) * PICOPAM_NRSTPF * 3;
	return PICO_OK;
} /*pam_put_item*/
 ...
 * @callergraph
 */
static pico_status_t pam_put_qItem(picoos_uint8* qItem,
								   picoos_uint8* outBuff,
								   picoos_uint16 outWritePos,
								   picoos_uint8* bytesWr) {
 ...
 * @callergraph
 */
static pico_status_t pamDoCommand(register picodata_ProcessingUnit this, picoos_uint8* qItem) {
	pam_subobj_t* pam;
	picoos_single fValue;
...
/**
 * translates one full phone into a PHONE Item including DT Dur, F0 and CEP trees feature generation
 * and traversal
...
static pico_status_t pamPhoneProcess(register picodata_ProcessingUnit this) {
	...
	if (!pam_do_tree(this, pam->dtdur, &(pam->sPhFeats[0]), PICOPAM_INVEC_SIZE, &dTreeResult)) {
		PICODBG_WARN(("problem using pam tree dtdur, using fallback value"));
		dTreeResult.class = 0;
	}
	pam->durIndex = dTreeResult.class;
	sResult = pam_get_duration(this, pam->durIndex, &(pam->phonDur), &(pam->numFramesState[0]));

	/*tree traversal for pitch*/
	for (nI = 0; nI < PICOPAM_MAX_STATES_PER_PHONE; nI++) {
		if (!pam_do_tree(this, pam->dtlfz[nI], &(pam->sPhFeats[0]), PICOPAM_INVEC_SIZE,
						 &dTreeResult)) {
	...
	/*cep tree traversal*/
	for (nI = 0; nI < PICOPAM_MAX_STATES_PER_PHONE; nI++) {
		if (!pam_do_tree(this, pam->dtmgc[nI], &(pam->sPhFeats[0]), PICOPAM_INVEC_SIZE,
						 &dTreeResult)) {
			PICODBG_WARN(("problem using pam tree lf0Tree, using fallback value"));
			dTreeResult.class = 0;
		}
 ...
 * @param    this  : Pam item subobject pointer
 * @return    PICO_OK (0) : default return code --> means no more items to be processed before 1st
 * syllable
...
static pico_status_t pamDoPreSyll(register picodata_ProcessingUnit this) {
	...
	if (((qItem = pamPopAttachedSy0(this)) != NULL) &&
		!((qItem[0] == PICODATA_ITEM_BOUND) && (qItem[1] == PICODATA_ITEMINFO1_BOUND_SBEG))) {
		if (isItemToPut(qItem)) {
			pam_put_qItem(qItem, pam->outBuf, pam->outWritePos,
						  &bWr); /*popped item has to be sent to next PU*/
			pam->outWritePos += bWr;
	...
	if ((qItem != NULL) && (qItem[0] == PICODATA_ITEM_BOUND) &&
		(qItem[1] == PICODATA_ITEMINFO1_BOUND_SBEG)) {
	...
	while (1) { /* exit via return */
		...
		switch (pam->procState) {
			case PICOPAM_COLLECT:
				...
				sResult = picodata_cbGetItem(this->cbIn, &(pam->inBuf[pam->inWritePos]),
											 pam->inBufSize - pam->inWritePos, &blen);
				if (sResult != PICO_OK) {
					if (sResult == PICO_EOF) {
						...
						PICODBG_DEBUG(
							("pam_step(PICOPAM_COLLECT) -- Errors on item buffer input, status: %d",
							 sResult));
						return PICODATA_PU_ERROR;
					}
				}
				...
				sResult = picodata_is_valid_item(&(pam->inBuf[pam->inWritePos]), blen);
				if (sResult != TRUE) {
					...
					PICODBG_DEBUG(
						("pam_step -- item is not valid, type: %d", pam->inBuf[pam->inWritePos]));
			...
			case PICOPAM_SCHEDULE:
				...
				if (is_pam_play_command(&(pam->inBuf[pam->inReadPos])) == TRUE) {
					...
					pam->inReadPos += pam->inBuf[pam->inReadPos + 3] + sizeof(picodata_itemhead_t);
					if (pam->inReadPos >= pam->inWritePos) {
						pam->inReadPos = 0;
						pam->inWritePos = 0;
					}
				...
				PICODBG_DEBUG(
					("pam_step (PICOPAM_SCHEDULE) -- unexpected item is sent to next PU !!"));
				pam->procState = PICOPAM_IMMEDIATE;
				return PICODATA_PU_BUSY;
			...
			case PICOPAM_FORWARD:
				...
				sResult = pam_adapter_forward_step(this, &(pam->inBuf[pam->inReadPos]));
				...
				switch (sResult) {
					case PICOPAM_READY:
						...
						pam->inReadPos +=
							pam->inBuf[pam->inReadPos + 3] + sizeof(picodata_itemhead_t);
						if (pam->inReadPos >= pam->inWritePos) {
							pam->inReadPos = 0;
							pam->inWritePos = 0;
						}
						...
						if (sResult == PICO_OK) {
							...
							PICODBG_DEBUG(
								("pam_step (PICOPAM_FORWARD) -- wrong return from BackwardStep: %d "
								 "-- Buffered sentence will be discarded",
								 sResult));
							pam_reset_processors(this);
							pam->nLastAttachedItemId = pam->nCurrAttachedItem = 0;
							pam->nAttachedItemsSize = 0;
				...
				switch (sResult) {
					case PICOPAM_READY:
						...
						if (sResult == PICO_OK) {
							...
							PICODBG_DEBUG(
								("pam_step (PICOPAM_FORWARD_FORCE_TERM) -- wrong return from "
								 "BackwardStep: %d -- Buffered sentence will be discarded",
								 sResult));
							pam_reset_processors(this);
							pam->nLastAttachedItemId = pam->nCurrAttachedItem = 0;
							pam->nAttachedItemsSize = 0;
					...
					default:
						PICODBG_DEBUG((
							"pam_step (PICOPAM_FORWARD_FORCE_TERM) -- Forced a TERM but processing "
							"do not appear to end -- Buffered sentence will be discarded",
							sResult));
						pam_reset_processors(this);
						pam->nLastAttachedItemId = pam->nCurrAttachedItem = 0;
						pam->nAttachedItemsSize = 0;
			...
			case PICOPAM_PROCESS:
				...
				if ((PICOPAM_FRAME_ITEM_SIZE + 4) > (pam->outBufSize - pam->outWritePos)) {
				...
				if (pam->nCurrSyllable == 0) {
					...
					if ((sResult == PICOPAM_FLUSH_RECEIVED) || (sResult == PICODATA_PU_ERROR)) {
						/*
				...
				if (pamHasToProcess(this)) {
					if (pamPhoneProcess(this) == PICO_OK) {
						...
						PICODBG_DEBUG(
							("pam_step(PICOPAM_PROCESS) --- NULL return from pamPhoneProcess"));
						return PICODATA_PU_ERROR;
					}
				}
				...
				if (pamHasToPop(this) != FALSE) {
					if ((qItem = pamPopItem(this)) == NULL) {
						PICODBG_DEBUG(
							("pam_step(PICOPAM_PROCESS) --- NULL return from pamPopItem"));
						return PICODATA_PU_ERROR;
					}
					...
					if (isItemToPut(qItem)) {
						...
						if (sResult != PICO_OK) {
							PICODBG_DEBUG(
								("pam_step(PICOPAM_PROCESS) --- Error on writing item to output "
								 "buffer"));
							return PICODATA_PU_ERROR;
						}
					...
					/*moved command processing here (after pam_put_qItem) because of FLUSH command
					 * could erase the syllable structure and make it impossible to transmit the
					 * flush to other PUs*/
					if (is_pam_command(qItem) == TRUE) {
						sResult = pamDoCommand(
							this, qItem); /*popped item is a PAM command : do it NOW!!*/
						if ((sResult == PICOPAM_FLUSH_RECEIVED) || (sResult == PICODATA_PU_ERROR)) {
							pam->retState = PICOPAM_SCHEDULE;
					...
					sResult =
						pamUpdateProcess(this); /*both "doCommand" or "put" : update PAM status*/
					return PICODATA_PU_BUSY;
				} else {
					pam->procState = PICOPAM_SCHEDULE; /*switch to schedule */
					return PICODATA_PU_BUSY;
			...
			case PICOPAM_IMMEDIATE:
				...
				numinb = PICODATA_ITEM_HEADSIZE + pam->inBuf[pam->inReadPos + 3];
				sResult = picodata_copy_item(&(pam->inBuf[pam->inReadPos]), numinb,
											 &(pam->outBuf[pam->outWritePos]),
				...
				if (sResult == PICO_OK) {
					 ...
					 */
					PICODBG_DEBUG(
						("pam_step(PICOPAM_IMMEDIATE) --- wrong return from picodata_copy_item:%d",
						 sResult));
					return PICODATA_PU_ERROR;
				}
				return PICODATA_PU_BUSY;
			...
			case PICOPAM_FEED:
				...
				sResult = picodata_cbPutItem(this->cbOut, &(pam->outBuf[pam->outReadPos]),
											 pam->outWritePos - pam->outReadPos, &numoutb);
				PICODBG_DEBUG(("pam_step -- put item, status: %d", sResult));
				if (PICO_OK == sResult) {
					PICODATA_INFO_ITEM(this->voice->kbArray[PICOKNOW_KBID_DBG],
									   (picoos_uint8*)"pam: ", pam->outBuf + pam->outReadPos,
									   pam->outBufSize);
				...
				} else if ((PICO_EXC_BUF_UNDERFLOW == sResult) || (PICO_ERR_OTHER == sResult)) {
...
static picoos_uint8 pam_do_tree(register picodata_ProcessingUnit this,
	...
	if (okay && (!picokdt_dtPAMclassify(dtpam))) {
		...
		picoos_emRaiseWarning(this->common->em, PICO_WARN_CLASSIFICATION, NULL, NULL);
		okay = FALSE;
	}
	/* decompose */
...
static pico_status_t pam_reset_processors(register picodata_ProcessingUnit this) {
	...
	pam->nCurrSyllable = -1;
	pam->nTotalPhonemes = pam->nSyllPhoneme = pam->nCurrPhoneme = pam->nTotalSyllables =
		pam->sType = pam->pType = 0;
	pam->dRest = 0.0f;
	/*set all to 0*/
	pam->a3_overall_syllable = pam->a3_primary_phrase_syllable = pam->b4_b5_syllable = pam->b6_b7_syllable =
		pam->b6_b7_state = pam->b8_b9_stressed_syllable = pam->b10_b11_accented_syllable = pam->b12_b13_syllable =
			pam->b12_b13_state = pam->b14_b15_syllable = pam->b14_b15_state = pam->b17_b19_syllable =
				pam->b17_b19_state = pam->b18_b20_b21_syllable = pam->b18_b20_b21_state = pam->c3_overall_syllable =
					pam->c3_primary_phrase_syllable = pam->d2_syllable_in_word = pam->d2_prev_syllable_in_word =
						pam->d2_current_primary_phrase_word = pam->e1_syllable_word_start =
									...
									pam->e5_e6_content_word = pam->e5_e6_content = pam->e7_e8_word =
										pam->e7_e8_content = pam->e7_e8_state = pam->e9_e11_word =
											pam->e9_e11_saw_word = pam->e9_e11_state = pam->e10_e12_e13_word =
												pam->e10_e12_e13_state = pam->e10_e12_e13_saw_word =
													pam->f2_overall_word = pam->f2_word_syllable = pam->f2_next_word_syllable =
														pam->f2_current_primary_phrase_word = pam->g1_current_secondary_phrase_syllable =
															pam->g1_current_syllable = pam->g2_current_secondary_phrase_word =
																pam->g2_current_word = pam->h1_current_secondary_phrase_syll =
																	pam->h2_current_secondary_phrase_word = pam->h3_h4_current_secondary_phrase_word =
																		pam->h5_current_phrase_type = pam->h5_syllable =
																			pam
																				->h5_state = pam
																								 ->i1_secondary_phrase_syllable =
																							...
																							pam->j1_utterance_syllable =
																								pam->j2_utterance_word =
																									pam->j3_utterance_sec_phrases =
																										0;
	/*Override 0 with 1*/
	pam->b4_b5_syllable = pam->b17_b19_syllable = pam->b18_b20_b21_syllable = pam->e9_e11_word =
		pam->e10_e12_e13_word = pam->e7_e8_word = pam->h2_current_secondary_phrase_word = 1;
	/*Override 0 with -1*/
	...
	switch (nFeat) {
				...
				case PICOPAM_DIR_FORW:
					if (event_type == PICOPAM_EVENT_SYLL) {
						if ((pam->sSyllFeats[pam->nCurrSyllable].phoneV[P1] == 1) ||
							(pam->a3_primary_phrase_syllable >= 1)) {
		...
		case B4: /*processor for B4,B5*/
			...
			if (event_type == PICOPAM_EVENT_SYLL) {
				if (pam->sSyllFeats[pam->nCurrSyllable].phoneV[P1] == 0) {
					pam->sSyllFeats[pam->nCurrSyllable].phoneV[sDest] = pam->b4_b5_syllable;
					pam->b4_b5_syllable++;
				} else {
					pam->sSyllFeats[pam->nCurrSyllable].phoneV[sDest] = 0;
				}
			}
			if ((event_type == PICOPAM_EVENT_W_BOUND) || (event_type == PICOPAM_EVENT_S_BOUND) ||
				(event_type == PICOPAM_EVENT_P_BOUND)) {
		...
		case B6: /*processor for B6,B7*/
			...
			switch (pam->b6_b7_state) {
				case 0:
					if (event_type == PICOPAM_EVENT_SYLL) {
						pam->sSyllFeats[pam->nCurrSyllable].phoneV[sDest] = PICOPAM_DONT_CARE_VALUE;
					}
				...
				case 1:
					if (event_type == PICOPAM_EVENT_SYLL) {
						pam->sSyllFeats[pam->nCurrSyllable].phoneV[sDest] = pam->b6_b7_syllable;
						pam->b6_b7_syllable++;
					}
					if (event_type == PICOPAM_EVENT_S_BOUND) {
						pam->b6_b7_syllable = 1;
		...
		case B8: /*processor for B8,B9*/
			...
			if (event_type == PICOPAM_EVENT_SYLL) {
				pam->sSyllFeats[pam->nCurrSyllable].phoneV[sDest] = pam->b8_b9_stressed_syllable;
		...
		case B10: /*processor for B10, B11*/
			...
			if (event_type == PICOPAM_EVENT_SYLL) {
				pam->sSyllFeats[pam->nCurrSyllable].phoneV[sDest] = pam->b10_b11_accented_syllable;
		...
		case B12: /*processor for B12,B13*/
			...
			switch (pam->b12_b13_state) {
				...
				case 1:
					if (event_type == PICOPAM_EVENT_SYLL) {
						pam->sSyllFeats[pam->nCurrSyllable].phoneV[sDest] = pam->b12_b13_syllable;
						if (pam->sSyllFeats[pam->nCurrSyllable].phoneV[B1] == 1) {
							pam->b12_b13_syllable = 0;
						} else {
		...
		case B14: /*processor for B14, B15*/
			...
			switch (pam->b14_b15_state) {
				...
				case 1:
					if (event_type == PICOPAM_EVENT_SYLL) {
						pam->sSyllFeats[pam->nCurrSyllable].phoneV[sDest] = pam->b14_b15_syllable;
						if (pam->sSyllFeats[pam->nCurrSyllable].phoneV[B2] == 1) {
							pam->b14_b15_syllable = 0;
						} else {
		...
		case C3: /*processor for C3*/
			switch (direction) {
				...
				case PICOPAM_DIR_BACK:
					if (event_type == PICOPAM_EVENT_SYLL) {
						if ((pam->sSyllFeats[pam->nCurrSyllable].phoneV[P1] == 1) ||
							(pam->c3_primary_phrase_syllable >= 1)) {
			...
			switch (direction) {
				case PICOPAM_DIR_FORW:
					...
					if ((event_type == PICOPAM_EVENT_W_BOUND) ||
						...
						(event_type == PICOPAM_EVENT_P_BOUND)) {
						pam->d2_current_primary_phrase_word = 1;
						pam->d2_prev_syllable_in_word = pam->d2_syllable_in_word;
				...
				case PICOPAM_DIR_FORW: /*remember : content syllable indicator already on P5*/
					if (event_type == PICOPAM_EVENT_SYLL) {
						if (pam->e1_syllable_word_start == -1) {
							pam->e1_syllable_word_start = (picoos_int8)pam->nCurrSyllable;
						}
						if (pam->sSyllFeats[pam->nCurrSyllable].phoneV[P5] == 1) {
							pam->e1_content = 1;
						}
						pam->e1_syllable_word_end = (picoos_int8)pam->nCurrSyllable;
					}
					if ((event_type == PICOPAM_EVENT_W_BOUND) ||
						(event_type == PICOPAM_EVENT_S_BOUND) ||
						(event_type == PICOPAM_EVENT_P_BOUND)) {
						if ((pam->e1_syllable_word_start != -1) &&
							...
							for (nI = pam->e1_syllable_word_start; nI <= pam->e1_syllable_word_end;
								 nI++) {
								pam->sSyllFeats[nI].phoneV[E1] = pam->e1_content;
							}
						}
						pam->e1_content = 0;
				...
				case PICOPAM_DIR_FORW:
					if (event_type == PICOPAM_EVENT_SYLL) {
						if (pam->e2_syllable_word_start == -1) {
							pam->e2_syllable_word_start = (picoos_int8)pam->nCurrSyllable;
						}
						pam->e2_syllable_word_end = (picoos_int8)pam->nCurrSyllable;
					}
					if ((event_type == PICOPAM_EVENT_W_BOUND) ||
						(event_type == PICOPAM_EVENT_S_BOUND) ||
						(event_type == PICOPAM_EVENT_P_BOUND)) {
						if ((pam->e2_syllable_word_start != -1) &&
							(pam->e2_syllable_word_end != -1)) {
							for (nI = pam->e2_syllable_word_start; nI <= pam->e2_syllable_word_end;
								 nI++) {
								pam->sSyllFeats[nI].phoneV[E2] =
									pam->e2_syllable_word_end - pam->e2_syllable_word_start + 1;
							}
						}
						pam->e1_content = 0;
		...
		case E3: /*processor for E3,E4*/
			...
			switch (pam->e3_e4_state) {
				...
				case 1:
					if (event_type == PICOPAM_EVENT_SYLL) {
						pam->sSyllFeats[pam->nCurrSyllable].phoneV[sDest] = pam->e3_e4_word;
					}
					if (event_type == PICOPAM_EVENT_S_BOUND) {
						pam->e3_e4_word = 1;
					}
		...
		case E5: /*processor for E5,E6*/
			...
			if (event_type == PICOPAM_EVENT_SYLL) {
				pam->sSyllFeats[pam->nCurrSyllable].phoneV[sDest] = pam->e5_e6_content_word;
				if (pam->sSyllFeats[pam->nCurrSyllable].phoneV[P5] == 1) {
					pam->e5_e6_content = 1;
				}
			}
		...
		case E7: /*processor for E7,E8*/
			...
			switch (pam->e7_e8_state) {
				...
				case 1:
					if (event_type == PICOPAM_EVENT_SYLL) {
						pam->sSyllFeats[pam->nCurrSyllable].phoneV[sDest] = pam->e7_e8_word;
						if (pam->sSyllFeats[pam->nCurrSyllable].phoneV[P5] == 1) {
							pam->e7_e8_content = 1;
						}
					}
						...
						case 0:
							if (event_type == PICOPAM_EVENT_SYLL) {
								pam->sSyllFeats[pam->nCurrSyllable].phoneV[E9] =
									PICOPAM_DONT_CARE_VALUE;
								pam->sSyllFeats[pam->nCurrSyllable].phoneV[E11] = pam->e9_e11_word;
								pam->e9_e11_saw_word =
									1; /*new variable, needs to be initialized to 0*/
							}
							if (event_type == PICOPAM_EVENT_W_BOUND) {
								pam->e9_e11_word++;
							}
				...
				case PICOPAM_DIR_BACK:
					switch (pam->e10_e12_e13_state) {
						case 0:
							...
							/*if
							 * (((event_type==PICOPAM_EVENT_P_BOUND)||(event_type==PICOPAM_EVENT_S_BOUND))&&(pam->e10_e12_e13_word>1))
							 * {*/
							if (((event_type == PICOPAM_EVENT_P_BOUND) ||
								 (event_type == PICOPAM_EVENT_S_BOUND)) &&
		...
		case F2:
			switch (direction) {
				...
				case PICOPAM_DIR_BACK:
					if (event_type == PICOPAM_EVENT_SYLL) {
						if (pam->f2_current_primary_phrase_word >=
							1) { /*at least second word in current primary phrase*/
					...
					if ((event_type == PICOPAM_EVENT_W_BOUND) ||
						(event_type == PICOPAM_EVENT_S_BOUND) ||
						(event_type == PICOPAM_EVENT_P_BOUND)) { /*word - end : switch*/
						pam->f2_next_word_syllable = pam->f2_word_syllable;
						pam->f2_word_syllable = 0;
					}
					if (event_type ==
						PICOPAM_EVENT_P_BOUND) { /*mark first word in current primary phrase*/
						pam->f2_current_primary_phrase_word = 0;
					} else /*mark next word in current primary phrase(enables output in
							  PICOPAM_EVENT_SYLL)*/
						if ((event_type == PICOPAM_EVENT_W_BOUND) ||
							(event_type == PICOPAM_EVENT_S_BOUND)) {
		...
		case G2:
			switch (direction) {
				case PICOPAM_DIR_FORW:
					...
					if (event_type == PICOPAM_EVENT_S_BOUND) {
						pam->g2_current_secondary_phrase_word = pam->g2_current_word + 1;
		...
		case H3: /*processor for H3,H4 unified */
			...
			if ((event_type == PICOPAM_EVENT_S_BOUND) || (event_type == PICOPAM_EVENT_P_BOUND)) {
				pam->h3_h4_current_secondary_phrase_word++;
			}
			break;
			...
			switch (direction) {
				...
				case PICOPAM_DIR_BACK:
					switch (pam->h5_state) {
						...
						case 1:
							if (event_type == PICOPAM_EVENT_SYLL) {
								if ((pam->sSyllFeats[pam->nCurrSyllable].phoneV[H5] == PICOPAM_P) &&
									(pam->sSyllFeats[pam->nCurrSyllable].phoneV[P1] == 0)) {
		...
		case I1: /*processor for I1*/
			switch (direction) {
				...
				case PICOPAM_DIR_BACK:
					...
					if (event_type == PICOPAM_EVENT_S_BOUND) {
						pam->i1_next_secondary_phrase_syllable = pam->i1_secondary_phrase_syllable;
						pam->i1_secondary_phrase_syllable =
							pam->sSyllFeats[pam->nCurrSyllable].phoneV[I1];
					}
					if (event_type == PICOPAM_EVENT_P_BOUND) {
		...
		case I2: /*processor for I2*/
			switch (direction) {
				...
				case PICOPAM_DIR_BACK:
					...
					if (event_type == PICOPAM_EVENT_S_BOUND) {
						pam->i2_next_secondary_phrase_word = pam->i2_secondary_phrase_word;
						pam->i2_secondary_phrase_word =
							pam->sSyllFeats[pam->nCurrSyllable].phoneV[I2];
					}
					if (event_type == PICOPAM_EVENT_P_BOUND) {
		...
		case J1: /*processor for J1 */
			switch (direction) {
				case PICOPAM_DIR_FORW:
					...
					break;
				case PICOPAM_DIR_BACK:
					pam->sSyllFeats[pam->nCurrSyllable].phoneV[J1] = pam->j1_utterance_syllable;
					break;
			}
			break;
		case J2: /*processor for J2*/
			switch (direction) {
				case PICOPAM_DIR_FORW:
					...
					break;
				case PICOPAM_DIR_BACK:
					pam->sSyllFeats[pam->nCurrSyllable].phoneV[J2] = pam->j2_utterance_word - 1;
					break;
			}
			break;
...
static pico_status_t pam_create_syllable(register picodata_ProcessingUnit this,
	...
	if (uMinDur > 0) {
		pos32 = Min;
		picoos_write_mem_pi_uint16(pam->sSyllFeats[pam->nCurrSyllable].phoneV, &pos32, uMinDur);
	}
	if (uMaxDur > 0) {
		pos32 = Max;
		picoos_write_mem_pi_uint16(pam->sSyllFeats[pam->nCurrSyllable].phoneV, &pos32, uMaxDur);
	}
	...
	if (syllType == PICOPAM_SYLL_PAUSE) {
		...
		if (sentType == PICOPAM_DECLARATIVE) {
			pam->sSyllFeats[pam->nCurrSyllable].phoneV[P6] = PICOPAM_DECLARATIVE;
		}
		if (sentType == PICOPAM_INTERROGATIVE) {
			pam->sSyllFeats[pam->nCurrSyllable].phoneV[P6] = PICOPAM_INTERROGATIVE;
		}

		pam->sSyllFeats[pam->nCurrSyllable].phoneV[bnd] = uBoundType;
		pam->sSyllFeats[pam->nCurrSyllable].phoneV[P1] =
			1; /*this means the syllable contains a pause-silence*/
			...
			pam->sSyllFeats[pam->nCurrSyllable].phoneV[B4] =
				pam->sSyllFeats[pam->nCurrSyllable].phoneV[B5] =
					pam->sSyllFeats[pam->nCurrSyllable].phoneV[B6] =
						pam->sSyllFeats[pam->nCurrSyllable].phoneV[B7] = 1;
	...
	if (syllType == PICOPAM_SYLL_SYLL) {
		...
		pam->sSyllFeats[pam->nCurrSyllable].phoneV[B16] =
			pam_get_vowel_name(this, sContent, &pos); /*name of the vowel in the syllable*/

		pam->sSyllFeats[pam->nCurrSyllable].phoneV[P8] =
			pos; /*temp for storing the position of the vowel*/
	...
	switch (itemBase[0]) {
		case PICODATA_ITEM_BOUND:
			...
			switch (itemBase[1]) {
				case PICODATA_ITEMINFO1_BOUND_SBEG:
					...
					pam->sType = pam_map_sentence_type(itemBase[1], itemBase[2]);
					pam->pType = pam_map_phrase_type(itemBase[1], itemBase[2]);
					/*create silence syll and process P_BOUND event*/
					sResult = pam_create_syllable(this, PICOPAM_SYLL_PAUSE, NULL, pam->sType,
												  pam->pType, itemBase[1], uMinDur, uMaxDur);
					if (sResult != PICO_OK) {
						return sResult;
					}
					sResult = pam_process_event(this, PICOPAM_EVENT_P_BOUND, PICOPAM_DIR_FORW);
					if (sResult != PICO_OK) {
						return sResult;
					}
					return PICOPAM_MORE;
				...
				case PICODATA_ITEMINFO1_BOUND_PHR3:
					...
					sResult = pam_process_event(this, PICOPAM_EVENT_S_BOUND, PICOPAM_DIR_FORW);
				...
				case PICODATA_ITEMINFO1_BOUND_PHR0:
					...
					sResult = pam_process_event(this, PICOPAM_EVENT_W_BOUND, PICOPAM_DIR_FORW);
					if (sResult != PICO_OK) {
						return sResult;
					}
					return PICOPAM_MORE;
		...
		case PICODATA_ITEM_SYLLPHON:
			...
			if (pam->nCurrSyllable == -1) {
				...
				sResult = pam_create_syllable(this, PICOPAM_SYLL_PAUSE, NULL, pam->sType,
											  pam->pType, PICODATA_ITEMINFO1_BOUND_SBEG, 0, 0);
				if (sResult != PICO_OK) {
					return sResult;
				}
				sResult = pam_process_event(this, PICOPAM_EVENT_P_BOUND, PICOPAM_DIR_FORW);
			...
			/* ------------------------------------------------------------------*/
			sResult = pam_create_syllable(this, PICOPAM_SYLL_SYLL, itemBase, pam->sType, pam->pType,
										  0, 0, 0);
			if (sResult != PICO_OK) {
				return sResult;
			}
			sResult = pam_process_event(this, PICOPAM_EVENT_SYLL, PICOPAM_DIR_FORW);
			if (sResult != PICO_OK) {
				return sResult;
			}
			return PICOPAM_MORE;
...
static pico_status_t pam_adapter_backward_step(register picodata_ProcessingUnit this) {
	...
	while (pam->nCurrSyllable >= 0) {
		...
		if ((nProcessed == 0) && (pam->sSyllFeats[pam->nCurrSyllable].phoneV[P3] == 1)) {
			/*secondary boundary*/
		...
		if ((nProcessed == 0) && (pam->sSyllFeats[pam->nCurrSyllable].phoneV[P4] == 1)) {
			/*word boundary*/
 ...
 * @param    this : pointer to PAM PU sub object pointer : processes a pause (silence) syllable
...
static pico_status_t pam_do_pause(register picodata_ProcessingUnit this) {
	...
	if (pam->nCurrSyllable > 0) {
		...
		pam->sSyllFeats[pam->nCurrSyllable]
			.phoneV[A3] = pam->sSyllFeats[pam->nCurrSyllable]
							  .phoneV[B8] = pam->sSyllFeats[pam->nCurrSyllable].phoneV[B10] =
			pam->sSyllFeats[pam->nCurrSyllable]
				.phoneV[B12] = pam->sSyllFeats[pam->nCurrSyllable]
								   .phoneV[B14] = pam->sSyllFeats[pam->nCurrSyllable].phoneV[B17] =
				pam->sSyllFeats[pam->nCurrSyllable].phoneV[B19] =
					pam->sSyllFeats[pam->nCurrSyllable].phoneV[B20] =
						pam->sSyllFeats[pam->nCurrSyllable].phoneV[B21] =
							pam->sSyllFeats[pam->nCurrSyllable].phoneV[E5] =
	...
	if (pam->nCurrSyllable < pam->nTotalSyllables - 1) {
		...
		pam->sSyllFeats[pam->nCurrSyllable].phoneV[C3] =
			pam->sSyllFeats[pam->nCurrSyllable].phoneV[F2] =
				pam->sSyllFeats[pam->nCurrSyllable].phoneV[I1] =
					pam->sSyllFeats[pam->nCurrSyllable].phoneV[I2] = 0;
	}
