 ...
 *
 ...
 each PHONE produces at least StatesPerPhone FRAME (if each state has FramesPerState == 1), but
 usually more.
...
#define PICOCEP_MAXWINLEN \
	10000 /* maximum number of frames that can be smoothed, i.e. maximum sentence length */
#define PICOCEP_MSGSTR_SIZE 32
#define PICOCEP_IN_BUFF_SIZE PICODATA_BUFSIZE_DEFAULT

#define PICOCEP_OUT_DATA_FORMAT                                                                    \
	PICODATA_ITEMINFO1_FRAME_PAR_DATA_FORMAT_FIXED /* we output coefficients as fixed point values \
													*/
...
typedef enum picocep_WantMeanOrIvar { PICOCEP_WANTMEAN, PICOCEP_WANTIVAR } picocep_WantMeanOrIvar_t;
...
/*
 *   Fixedpoint arithmetic (might go into a separate module if general enough and needed by other
 * modules)
 */
...
typedef struct cep_subobj {
	...
	picoos_uint16
		headxWritePos; /* next free position; headx is empty if headxBottom == headxWritePos */
	...
	picoos_int32 diag0[PICOCEP_MAXWINLEN], diag1[PICOCEP_MAXWINLEN], diag2[PICOCEP_MAXWINLEN],
		WUm[PICOCEP_MAXWINLEN], invdiag0[PICOCEP_MAXWINLEN];
...
#define picocep_highestBitNZ(x)                                                                  \
	(x >= POW17                                                                                  \
		 ? (x >= POW25                                                                           \
				? (x >= POW29 ? (x >= POW31 ? 31 : (x >= POW30 ? 30 : 29))                       \
							  : (x >= POW27 ? (x >= POW28 ? 28 : 27) : (x >= POW26 ? 26 : 25)))  \
				: (x >= POW21 ? (x >= POW23 ? (x >= POW24 ? 24 : 23) : (x >= POW22 ? 22 : 21))   \
							  : (x >= POW19 ? (x >= POW20 ? 20 : 19) : (x >= POW18 ? 18 : 17)))) \
		 : (x >= POW9                                                                            \
				? (x >= POW13 ? (x >= POW15 ? (x >= POW16 ? 16 : 15) : (x >= POW14 ? 14 : 13))   \
							  : (x >= POW11 ? (x >= POW12 ? 12 : 11) : (x >= POW10 ? 10 : 9)))   \
				: (x >= POW5 ? (x >= POW7 ? (x >= POW8 ? 8 : 7) : (x >= POW6 ? 6 : 5))           \
							 : (x >= POW3 ? (x >= POW4 ? 4 : 3) : (x >= POW2 ? 2 : 1)))))
#define picocep_highestBitU(x) (x == 0 ? 0 : picocep_highestBitNZ(x))
#define picocep_highestBitS(x, zz) \
	(x == 0 ? 0 : (x < 0 ? ((zz) = (-x), picocep_highestBitNZ(zz)) : picocep_highestBitNZ(x)))
...
static picoos_int32 getFromPdf(picokpdf_PdfMUL pdf,
							   picoos_uint32 vecstart,
							   picoos_uint8 cepnum,
							   picocep_WantMeanOrIvar_t wantMeanOrIvar,
							   picocep_WantStaticOrDelta_t wantStaticOrDeltax);

static void invMatrix(cep_subobj_t* cep,
					  picoos_uint16 N,
					  picoos_int16* smoothcep,
					  picoos_uint8 cepnum,
					  picokpdf_PdfMUL pdf,
					  picoos_uint8 invpow,
					  picoos_uint8 invDoubleDec);

static picoos_uint8 makeWUWandWUm(cep_subobj_t* cep,
								  picokpdf_PdfMUL pdf,
								  picoos_uint16* indices,
								  picoos_uint16 b,
								  picoos_uint16 N,
								  picoos_uint8 cepnum);

static void getDirect(picokpdf_PdfMUL pdf,
					  picoos_uint16* indices,
					  picoos_uint16 activeEndPos,
					  picoos_uint8 cepnum,
					  picoos_int16* smoothcep);

static void getVoiced(picokpdf_PdfMUL pdf,
					  picoos_uint16* indices,
					  picoos_uint16 activeEndPos,
...
static pico_status_t cepInitialize(register picodata_ProcessingUnit this, picoos_int32 resetMode) {
	...
	if (resetMode == PICO_RESET_FULL) {
		/* kb pdflfz */
		cep->pdflfz = picokpdf_getPdfMUL(this->voice->kbArray[PICOKNOW_KBID_PDF_LFZ]);

		/* kb pdfmgc */
		cep->pdfmgc = picokpdf_getPdfMUL(this->voice->kbArray[PICOKNOW_KBID_PDF_MGC]);
		...
		cep->framehead.len =
			sizeof(picoos_uint16) + (cep->framehead.info2 + 4) * sizeof(picoos_uint16);
...
#else
	PICODBG_INFO_MSG(
		("number of long mult is %d, number of short mult is %i\n", numlongmult, numshortmult));
#endif
	if (NULL != this) {
...
picodata_ProcessingUnit picocep_newCepUnit(picoos_MemoryManager mm,
	...
	cep->outF0 = (picoos_int16*)picoos_allocate(
		this->common->mm, PICOCEP_MAXWINLEN * PICOKPDF_MAX_MUL_LFZ_CEPORDER * sizeof(picoos_int16));
	cep->outXCep = (picoos_int16*)picoos_allocate(
		this->common->mm, PICOCEP_MAXWINLEN * PICOKPDF_MAX_MUL_MGC_CEPORDER * sizeof(picoos_int16));
	cep->outVoiced =
		(picoos_uint8*)picoos_allocate(this->common->mm, PICOCEP_MAXWINLEN * sizeof(picoos_uint8));

	if ((NULL == cep->outF0) || (NULL == cep->outXCep) || (NULL == cep->outVoiced)) {
		picoos_deallocate(this->common->mm, (void*)&(cep->outF0));
		picoos_deallocate(this->common->mm, (void*)&(cep->outXCep));
		picoos_deallocate(this->common->mm, (void*)&(cep->outVoiced));
...
static picoos_int32 picocep_fixptmultpow(picoos_int32 a, picoos_uint8 pow) {
	...
	if (picocep_highestBitS(a, zzz) + pow < 32) {
		...
		PICODBG_WARN(
			("picocep_fixptmultpow warning: overflow in fixed point multiplication %i*1<<%i.  "
			 "Clipping to %i\n",
			 a, pow, b));
	}
	return b;
 ...
 * @callergraph
 */
static picoos_int32 picocep_fixptmultdouble(picoos_int32 x,
											picoos_int32 y,
											picoos_uint8 bigpow,
											picoos_uint8 invDoubleDec) {
	...
	if (invDoubleDec == 1) {
		...
		z = ((a * c) << bigpow) + (a * d + b * c) +
			picocep_fixptdivpow(b * d, bigpow); /* 4 mult and 3 add instead of 1 mult. */
	}
 ...
 * under PICO_DEBUG the number of double and single precision multiplications is monitored for
 * accuracy/performance tuning Calls
 * - picocep_highestBit
 * - picocep_fixptmultdouble
 * @callgraph
 * @callergraph
 */
static picoos_int32 picocep_fixptmult(picoos_int32 x,
									  picoos_int32 y,
									  picoos_uint8 bigpow,
									  picoos_uint8 invDoubleDec) {
	picoos_int32 z;
	...
	if (multsz <= 30) { /* x*y < 1<<30 is safe including rounding in picocep_fixptdivpow, x*y <
						   1<<31 is safe but not with rounding */
		...
		if (multsz > 31 + pow) {
			PICODBG_WARN(
				("picocep_fixptmult warning: overflow in fixed point multiplication %i*%i, multsz "
				 "= %i, pow = %i, decrease bigpow\n",
				 x, y, multsz, pow));
		}
#endif
		z = picocep_fixptmultdouble(
			x, y, bigpow, invDoubleDec); /*  perform long multiplication for large x and y */
#if defined(PICO_DEBUG)
 ...
 * @callergraph
 */
static picoos_int32 picocep_fixptdiv(picoos_int32 a, picoos_int32 b, picoos_uint8 bigpow) {
 ...
 * @param    rowscpow (int) : fixed point base for each dimension of the vectors stored in the
 * database
...
static picoos_int32 picocep_fixptInvDiagEle(picoos_uint32 d,
	...
	if (invpow + bigpow >
		30 + dlen) { /* c must be < 2^32, hence d which is >= 2^(dlen-1) must be >
						2^(invpow+bigpow-32), or invpow+bigpow must be <= dlen+30*/
		*rowscpow = invpow + bigpow - 30 - dlen;
		PICODBG_DEBUG(
			("input to picocep_fixptInvDiagEle is %i <= 1<<%i = 1<<invpow+bigpow-32. Choose lower "
			 "invpow. For now scaling row by 1<<%i\n",
 ...
 * @param    invb(uint32)   : inverse of operand b, in fixed point P:Q representation (sign is
 * positive)
 ...
 * @param      invDoubleDec   : boolean to indicate that a and the return value c have 2*N binary
 * decimal digits instead of N
 ...
 * @callergraph
 */
static picoos_int32 picocep_fixptinv(picoos_int32 a,
									 picoos_uint32 invb,
									 picoos_uint8 bigpow,
									 picoos_uint8 invpow,
									 picoos_uint8 invDoubleDec) {
	...
	/* if invDoubleDec==0, picocep_fixptmult assumes a and invb are in base 1<<bigpow and returns c
	 = (a*b)/1<<bigpow Since invb is in base 1<<invpow instead of 1<<bigpow, normalize c by
	 1<<(bigpow-invpow) if invDoubleDec==1: multiply additionally by 1<<bigpow*2 (for invb and c) so
	 that base of c is again 2*bigpow this can be seen by setting a=A*big, b=B*big, invb=big2/B,
	 mult(a,invb) = a*invb/(big*big) = A/B*big*big2/(big*big) = A/B*big2/big and we want c =
	 A/B*big*big => normfactor = big^3/big2
	 */
	if (invDoubleDec == 1) {
		normpow = 3 * bigpow;
	} else {
...
/**
 * initializes the coefficients to calculate delta and delta-delta values and the squares of the
 * coefficients
 ...
 * @param    invDoubleDec : boolean indicating that result of picocep_fixptinv has fixed point base
 * 2*bigpow picocep_fixptmult absorbs double decimal size by dividing its result by extra factor big
 * @return  void
 * @remarks diag0, diag1, diag2, WUm, invdiag0  globals needed in this function (object members in
 * pico)
 * @callgraph
 * @callergraph
 */
static void invMatrix(cep_subobj_t* cep,
	...
	/* LDL factorization */
	prevrowscpow = 0;
	cep->invdiag0[0] = picocep_fixptInvDiagEle(
		cep->diag0[0], &rowscpow, bigpow, invpow); /* inverse has fixed point basis 1<<invpow */
	cep->diag1[0] = picocep_fixptinv((cep->diag1[0]) << rowscpow, cep->invdiag0[0], bigpow, invpow,
									 invDoubleDec); /* perform division via inverse */
	cep->diag2[0] = picocep_fixptinv((cep->diag2[0]) << rowscpow, cep->invdiag0[0], bigpow, invpow,
									 invDoubleDec);
	cep->WUm[0] = (cep->WUm[0]) << rowscpow; /* if diag0 too low, multiply LHS and RHS of row in
												matrix equation by 1<<rowscpow */
	for (j = 1; j < N; j++) {
		/* do forward substitution */
		cep->WUm[j] = cep->WUm[j] -
					  picocep_fixptmult(cep->diag1[j - 1], cep->WUm[j - 1], bigpow, invDoubleDec);
		if (j > 1) {
			cep->WUm[j] = cep->WUm[j] - picocep_fixptmult(cep->diag2[j - 2], cep->WUm[j - 2],
														  bigpow, invDoubleDec);
		}

		/* update row j */
		v1 = picocep_fixptmult((cep->diag1[j - 1]) / (1 << rowscpow), cep->diag0[j - 1], bigpow,
							   invDoubleDec); /* undo scaling by 1<<rowscpow because diag1(j-1)
												 refers to symm ele in column j-1 not in row j-1 */
		cep->diag0[j] =
			cep->diag0[j] - picocep_fixptmult(cep->diag1[j - 1], v1, bigpow, invDoubleDec);
		if (j > 1) {
			v2 = picocep_fixptmult(
				(cep->diag2[j - 2]) / (1 << prevrowscpow), cep->diag0[j - 2], bigpow,
				invDoubleDec); /* undo scaling by 1<<prevrowscpow because diag1(j-2) refers to symm
								  ele in column j-2 not in row j-2 */
			cep->diag0[j] =
				cep->diag0[j] - picocep_fixptmult(cep->diag2[j - 2], v2, bigpow, invDoubleDec);
		}
		prevrowscpow = rowscpow;
		cep->invdiag0[j] = picocep_fixptInvDiagEle(
			cep->diag0[j], &rowscpow, bigpow, invpow); /* inverse has fixed point basis 1<<invpow */
		cep->WUm[j] = (cep->WUm[j]) << rowscpow;
		if (j < N - 1) {
			h = picocep_fixptmult(cep->diag2[j - 1], v1, bigpow, invDoubleDec);
			cep->diag1[j] =
				picocep_fixptinv((cep->diag1[j] - h) << rowscpow, cep->invdiag0[j], bigpow, invpow,
								 invDoubleDec); /* eliminate column j below pivot */
		}
		if (j < N - 2) {
			cep->diag2[j] =
				picocep_fixptinv((cep->diag2[j]) << rowscpow, cep->invdiag0[j], bigpow, invpow,
								 invDoubleDec); /* eliminate column j below pivot */
		}
	}
	...
	for (j = 0; j < N; j++) {
		cep->WUm[j] = picocep_fixptinv(cep->WUm[j], cep->invdiag0[j], bigpow, invpow, invDoubleDec);
		if (invDoubleDec == 1) {
	...
	for (j = N - 2; j >= 0; j--) {
		cep->WUm[j] =
			cep->WUm[j] - picocep_fixptmult(cep->diag1[j], cep->WUm[j + 1], bigpow, invDoubleDec);
		if (j < N - 2) {
			cep->WUm[j] = cep->WUm[j] -
						  picocep_fixptmult(cep->diag2[j], cep->WUm[j + 2], bigpow, invDoubleDec);
		}
	}
	/* copy N frames into smoothcep (only for coeff # "cepnum")  */
	/* coefficients normalized to occupy short; for correct waveform energy, divide by
	 * (1<<(bigpow-meanpow)) then convert e.g. to picoos_single */
	k = cepnum;
	for (j = 0; j < N; j++) {
 ...
 * @param    cep : PU sub object pointer
 * @param    pdf :  pointer to picoos_uint8, sequence of pdf vectors, each vector of length
 * 1+ceporder*2+numdeltas*3+ceporder*3
...
static picoos_uint8 makeWUWandWUm(cep_subobj_t* cep,
	...
	for (i = 0; i < N; i++) {
		...
		if (i > 0 && indices[b + i] == indices[b + i - 1]) {
			...
			ivar = getFromPdf(pdf, vecstart, cepnum, PICOCEP_WANTIVAR, PICOCEP_WANTSTATIC);
			prev_diag0 = cep->diag0[i] =
				ivar << 2; /* multiply ivar by 4 (4 used to be first entry of xsq) */
			mean = getFromPdf(pdf, vecstart, cepnum, PICOCEP_WANTMEAN, PICOCEP_WANTSTATIC);
			prev_WUm = cep->WUm[i] = mean
									 << 1; /* multiply mean by 2 (2 used to be first entry of x) */
		}

		/* process delta means and delta inverse variances */
		for (j = 0; j < numd; j++) {
			vecstart = indices[b + Id[j]] * vecsize;
			ivar = getFromPdf(pdf, vecstart, cepnum, PICOCEP_WANTIVAR, PICOCEP_WANTDELTA);
			cep->diag0[i] += xsq[j] * ivar;

			mean = getFromPdf(pdf, vecstart, cepnum, PICOCEP_WANTMEAN, PICOCEP_WANTDELTA);
			if (mean != 0) {
				cep->WUm[i] += x[j] * mean;
			}
		}
		...
		for (j = 0; j < numdd; j++) {
			...
			ivar = getFromPdf(pdf, vecstart, cepnum, PICOCEP_WANTIVAR, PICOCEP_WANTDELTA2);
			cep->diag0[i] += xsq[numd + j] * ivar;

			mean = getFromPdf(pdf, vecstart, cepnum, PICOCEP_WANTMEAN, PICOCEP_WANTDELTA2);
			if (mean != 0) {
		...
		if (i < N - 1) {
			if (i < N - 2) {
				if (i > 0 && indices[b + i + 1] == indices[b + i]) {
					 ...
					 */
					prev_diag1 = cep->diag1[i] =
						getFromPdf(pdf, vecstart, cepnum, PICOCEP_WANTIVAR, PICOCEP_WANTDELTA2);
				}
				/*
			...
			if (i > 0) {
				if (i > 1 && indices[b + i] == indices[b + i - 1]) {
					...
					/*
					 k = vecstart + pdf->numvuv + pdf->ceporder * 2 + pdf->numdeltas * 3 +
					 pdf->ceporder * 2 + cepnum; cep->diag1[i] += (picoos_int32)(pdf->content[k]) <<
					 pdf->bigpow; */
	...
	for (i = 0; i < N - 2; i++) {
		if (i > 0 && indices[b + i + 1] == indices[b + i]) {
			...
			vecstart = indices[b + i + 1] * vecsize;
			/*
			 k = vecstart + pdf->numvuv + pdf->ceporder * 2 + pdf->numdeltas * 3 + pdf->ceporder * 2
			 + cepnum; cep->diag2[i] = (picoos_int32)(pdf->content[k]) << pdf->bigpow; k -=
			 pdf->ceporder; ivar = (picoos_int32)(pdf->content[k]) << pdf->bigpow;
			 */
			cep->diag2[i] = getFromPdf(pdf, vecstart, cepnum, PICOCEP_WANTIVAR, PICOCEP_WANTDELTA2);
			ivar = getFromPdf(pdf, vecstart, cepnum, PICOCEP_WANTIVAR, PICOCEP_WANTDELTA);
	...
	if (pdf->numdeltas == 0xFF) {
		switch (wantMeanOrIvar) {
			case PICOCEP_WANTMEAN:
				switch (wantStaticOrDeltax) {
					case PICOCEP_WANTSTATIC:
						...
						mean = ((picoos_int32)((picoos_int16)(*(p + 1) << 8)) | *p)
							   << (pdf->meanpowUm[cepnum]);
						break;
					case PICOCEP_WANTDELTA:
						cc = pdf->ceporder + cepnum;
						p = pdf->content +
							(vecstart + pdf->numvuv + cc * 2); /* cepnum'th delta mean */
						mean = ((picoos_int32)((picoos_int16)(*(p + 1) << 8)) | *p)
							   << (pdf->meanpowUm[cc]);
						break;
					case PICOCEP_WANTDELTA2:
						...
						p = pdf->content +
							(vecstart + pdf->numvuv + cc * 2); /* cepnum'th delta delta mean */
						mean = ((picoos_int32)((picoos_int16)(*(p + 1) << 8)) | *p)
							   << (pdf->meanpowUm[cc]);
						break;
					default:
			...
			case PICOCEP_WANTIVAR:
				switch (wantStaticOrDeltax) {
					case PICOCEP_WANTSTATIC:
						k = vecstart + pdf->numvuv + pdf->ceporder * 6 +
							cepnum; /* cepnum'th static ivar */
						ivar = (picoos_int32)(pdf->content[k]) << (pdf->ivarpow[cepnum]);
						break;
					case PICOCEP_WANTDELTA:
						ceporder = pdf->ceporder;
						k = vecstart + pdf->numvuv + ceporder * 7 +
							cepnum; /* cepnum'th delta ivar */
						ivar = (picoos_int32)(pdf->content[k]) << (pdf->ivarpow[ceporder + cepnum]);
						break;
					case PICOCEP_WANTDELTA2:
						ceporder = pdf->ceporder;
						k = vecstart + pdf->numvuv + ceporder * 8 +
							cepnum; /* cepnum'th delta delta ivar */
		...
		switch (wantMeanOrIvar) {
			case PICOCEP_WANTMEAN:
				switch (wantStaticOrDeltax) {
					...
					case PICOCEP_WANTDELTA:
						...
						s = 0;
						ind = 0;
						while ((s < pdf->numdeltas) &&
							   (ind < cepnum ||
								(ind == 0 && cepnum == 0))) { /* rawmean deltas are sparse so
																 investigate indices in column */
							k = vecstart + pdf->numvuv + ceporder * 2 + s; /* s'th delta index */
							ind = (picoos_uint8)(pdf->content[k]); /* is already picoos_uint8 but
																	  just to make explicit */
							if (ind == cepnum) {
								k = vecstart + pdf->numvuv + ceporder * 2 + pdf->numdeltas +
									s * 2; /* s'th sparse delta mean, corresponds to cepnum'th delta
											  mean */
								mean = ((picoos_int32)((picoos_int16)((pdf->content[k + 1]) << 8)) |
										pdf->content[k])
									   << (pdf->meanpowUm[ceporder + cepnum]);
								return mean;
							}
							s++;
						}
					...
					case PICOCEP_WANTDELTA2:
						...
						ind = 2 * ceporder;
						while ((s-- > 0) &&
							   (ind > ceporder + cepnum)) { /* rawmean deltas are sparse so
															   investigate indices in column */
							...
							if (ind == ceporder + cepnum) {
								...
								mean = ((picoos_int32)((picoos_int16)((pdf->content[k + 1]) << 8)) |
										pdf->content[k])
									   << (pdf->meanpowUm[ceporder2 + cepnum]);
								return mean;
							}
						}
						return 0;
			...
			case PICOCEP_WANTIVAR:
				switch (wantStaticOrDeltax) {
					...
					case PICOCEP_WANTDELTA:
						ceporder = pdf->ceporder;
						k = vecstart + pdf->numvuv + ceporder * 3 + pdf->numdeltas * 3 +
							cepnum; /* cepnum'th delta ivar */
						ivar = (picoos_int32)(pdf->content[k]) << (pdf->ivarpow[ceporder + cepnum]);
						break;
					case PICOCEP_WANTDELTA2:
						ceporder2 = 2 * pdf->ceporder;
						k = vecstart + pdf->numvuv + ceporder2 + pdf->numdeltas * 3 + ceporder2 +
							cepnum; /* cepnum'th delta delta ivar */
						ivar = (picoos_int32)(pdf->content[k])
...
static void treat_phone(cep_subobj_t* cep, picodata_itemhead_t* ihead) {
	...
	while ((state < ihead->info2) && (bufferFull == FALSE)) {
		...
		PICODBG_DEBUG(("finished state %i with %i frames, now at index write pos %i", state, frames,
					   cep->indexWritePos));
		state++;
 ...
 */
static picoos_uint8 forwardingItem(picodata_itemhead_t* ihead) {
	if ((PICODATA_ITEM_CMD == ihead->type) && (PICODATA_ITEMINFO1_CMD_IGNSIG == ihead->info1)) {
		return FALSE;
	} else {
		return TRUE;
	}
	...
	while (1) { /* exit via return */
		...
		switch (cep->procState) {
			case PICOCEP_STEPSTATE_COLLECT:
				...
				sResult = picodata_cbGetItem(this->cbIn, &(cep->inBuf[cep->inWritePos]),
											 cep->inBufSize - cep->inWritePos, &blen);
				if (PICO_EOF == sResult) { /* there are no more items available and we always need
											  more data here */
			...
			case PICOCEP_STEPSTATE_PROCESS_PARSE:
				...
				PICODBG_DEBUG(
					("getting info from inBuf in range: [%i,%i[", cep->inReadPos, cep->inWritePos));
				...
				if (!picodata_is_valid_item(cep->inBuf + cep->inReadPos,
					...
					sResult =
						picodata_get_iteminfo(cep->inBuf + cep->inReadPos,
											  cep->inWritePos - cep->inReadPos, &ihead, &icontents);
					PICODBG_DEBUG(("PARSE bad item %s",
								   picodata_head_to_string(&ihead, msgstr, PICOCEP_MSGSTR_SIZE)));
				...
				/* we decide what to do next depending on the item and the state of the index
				 * buffer:
				 * - process buffer if buffer not empty and sentence end or flush or ignsig/start
				...
				/* see if it is a sentence end boundary or termination boundary (flush) and there
				 * are indices to smooth -> smooth */
				if ((PICODATA_ITEM_BOUND == ihead.type) &&
					...
					PICODBG_DEBUG(
						("cep: PARSE found sentence terminator; setting activeEndPos to %i",
						 cep->activeEndPos));
					...
					/* sentence end or flush remaining after frame or other non-processable item,
					 * e.g. command */
					...
					if (forwardingItem(&ihead)) {
						...
						if (cep->indexWritePos <= 0) {
							...
							picodata_copy_item(cep->inBuf + cep->inReadPos,
											   cep->inWritePos - cep->inReadPos, cep->outBuf,
											   cep->outBufSize, &blen);
							cep->outWritePos += blen;
							PICODATA_INFO_ITEM(this->voice->kbArray[PICOKNOW_KBID_DBG],
											   (picoos_uint8*)"cep: do forward item ", cep->outBuf,
											   PICODATA_MAX_ITEMSIZE);
							/* output item and then go to parse to treat a new item. */
							cep->feedFollowState = PICOCEP_STEPSTATE_PROCESS_PARSE;
							cep->procState = PICOCEP_STEPSTATE_FEED;
						} else if ((cep->headxWritePos < PICOCEP_MAXNR_HEADX) &&
								   (cep->cbufWritePos + ihead.len < cep->cbufBufSize)) {
							/* there is enough space to store item */
							PICODBG_DEBUG(
								("unhandled item (type %c, length %i). Storing associated with "
								 "index %i",
								 ihead.type, ihead.len, cep->indexWritePos));
							sResult = picodata_get_itemparts(
								cep->inBuf + cep->inReadPos, cep->inWritePos - cep->inReadPos,
								&(cep->headx[cep->headxWritePos].head),
								&(cep->cbuf[cep->cbufWritePos]),
								cep->cbufBufSize - cep->cbufWritePos, &clen);

							if (sResult != PICO_OK) {
								PICODBG_ERROR(("problem getting item parts"));
								picoos_emRaiseException(this->common->em, sResult, NULL, NULL);
								return PICODATA_PU_ERROR;
							}
							/* remember sync position */
							cep->headx[cep->headxWritePos].frame = cep->indexWritePos;

							if (clen > 0) {
								cep->headx[cep->headxWritePos].cind = cep->cbufWritePos;
							...
							PICODBG_DEBUG((
								"PARSE is forced to smooth prematurely; setting activeEndPos to %i",
								cep->activeEndPos));
							cep->procState = PICOCEP_STEPSTATE_PROCESS_SMOOTH;
							/* don't consume item yet */
							break;
			...
			case PICOCEP_STEPSTATE_PROCESS_SMOOTH:
				...
				PICODBG_TRACE(("SMOOTH"));
				{
					...
					/* still to be experimented if higher order coeff can remain unsmoothed, i.e.
					 * simple copy from pdf */
					...
					for (cepnum = 0; cepnum < pdf->ceporder; cepnum++) {
						if (cep->activeEndPos <= 0) {
							...
							getDirect(pdf, cep->indicesLFZ, cep->activeEndPos, cepnum,
									  cep->outF0 + cep->outF0WritePos);
						}
					} /* end for cepnum  */
					...
					for (cepnum = 0; cepnum < pdf->ceporder; cepnum++) {
						if (cep->activeEndPos <= 0) {
							...
							makeWUWandWUm(cep, pdf, cep->indicesMGC, 0, N,
										  cepnum); /* update diag0, diag1, diag2, WUm */
							invMatrix(cep, N, cep->outXCep + cep->outXCepWritePos, cepnum, pdf,
									  PICOCEP_MGCINVPOW, PICOCEP_MGCDOUBLEDEC);
						} else {
							getDirect(pdf, cep->indicesMGC, cep->activeEndPos, cepnum,
									  cep->outXCep + cep->outXCepWritePos);
						}
					} /* end for cepnum  */
					cep->outXCepWritePos += cep->activeEndPos * pdf->ceporder;

					getVoiced(pdf, cep->indicesMGC, cep->activeEndPos,
							  cep->outVoiced + cep->outVoicedWritePos);
					cep->outVoicedWritePos += cep->activeEndPos;
				}
				/* setting indexReadPos to the next active index to be used. (will be advanced by
				 * FRAME when reading the phoneId */
				cep->indexReadPos = 0;
			...
			case PICOCEP_STEPSTATE_PROCESS_FRAME:
				...
				if ((cep->headxBottom < cep->headxWritePos) &&
					(cep->headx[cep->headxBottom].frame <= cep->indexReadPos)) {
					/* output item in headx/cbuf */
					/* copy item to outBuf */
					PICODBG_DEBUG(("FRAME copy item in inBuf to outBuf"));
					picodata_put_itemparts(&(cep->headx[cep->headxBottom].head),
				...
				if (cep->indexReadPos < cep->activeEndPos) {
...

					{
						...
						/* */
						PICODBG_DEBUG(("FRAME reading phoneId[%i] = %c:", cep->indexReadPos,
									   cep->phoneId[cep->indexReadPos]));
						/* */

						tmpUint16 = (picoos_uint16)cep->phoneId[cep->indexReadPos];

						picoos_mem_copy((void*)&tmpUint16, (void*)&cep->outBuf[cep->outWritePos],
										sizeof(tmpUint16));

						cep->outWritePos += sizeof(tmpUint16);

						PICODBG_DEBUG(
							("FRAME  writing position after phone id: %i", cep->outWritePos));

						for (i = 0; i < cep->pdflfz->ceporder; i++) {
							tmpUint16 = (cep->outVoiced[cep->outVoicedReadPos] & 0x01)
											? cep->outF0[cep->outF0ReadPos]
											: (picoos_uint16)0;
							...
							tmpUint16 = (picoos_uint16)(cep->outVoiced[cep->outVoicedReadPos]);
							picoos_mem_copy((void*)&tmpUint16,
											(void*)&cep->outBuf[cep->outWritePos],
											sizeof(tmpUint16));
							cep->outWritePos += sizeof(tmpUint16);
							tmpUint16 = (picoos_uint16)(cep->outF0[cep->outF0ReadPos]);
							picoos_mem_copy((void*)&tmpUint16,
											(void*)&cep->outBuf[cep->outWritePos],
											sizeof(tmpUint16));
							cep->outWritePos += sizeof(tmpUint16);
						...
						for (i = 0; i < cep->pdfmgc->ceporder; i++) {
							...
							picoos_mem_copy((void*)&tmpInt16, (void*)&cep->outBuf[cep->outWritePos],
											sizeof(tmpInt16));
							cep->outWritePos += sizeof(tmpInt16);
						}

						PICODBG_DEBUG(
							("FRAME  writing position after cepstrals: %i", cep->outWritePos));

						tmpUint16 = (picoos_uint16)cep->indicesMGC[cep->indexReadPos++];

						picoos_mem_copy((void*)&tmpUint16, (void*)&cep->outBuf[cep->outWritePos],
										sizeof(tmpUint16));

						PICODBG_DEBUG(
							("FRAME  writing position after mgc index: %i", cep->outWritePos));
					 ...
					 * ----------------------------------------*/
					PICODBG_INFO(("End of sentence - Processed frames : %d", cep->nNumFrames));
					cep->nNumFrames = 0;
					PICODBG_DEBUG(("FRAME no more active frames for this sentence"));
			...
			case PICOCEP_STEPSTATE_FEED:
				...
				sResult = picodata_cbPutItem(this->cbOut, cep->outBuf, cep->outBufSize, &blen);
