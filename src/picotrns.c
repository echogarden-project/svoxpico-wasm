...
void PICOTRNS_PRINTSYMSEQ1(picoknow_KnowledgeBase kbdbg,
						   const picotrns_possym_t seq[],
						   const picoos_uint16 seqLen,
...
void PICOTRNS_PRINTSYMSEQ(picoknow_KnowledgeBase kbdbg,
						  const picotrns_possym_t seq[],
						  const picoos_uint16 seqLen) {
...
picotrns_AltDesc picotrns_allocate_alt_desc_buf(picoos_MemoryManager mm,
												picoos_uint32 maxByteSize,
												picoos_uint16* numAltDescs) {
...
pico_status_t picotrns_eliminate_epsilons(const picotrns_possym_t inSeq[],
										  picoos_uint16 inSeqLen,
										  picotrns_possym_t outSeq[],
										  picoos_uint16* outSeqLen,
										  picoos_uint16 maxOutSeqLen) {
	picoos_uint16 i, j = 0;
 ...
 * inSeq is assumed to be at most PICOTRNS_MAX_NUM_POSSYM, outSeq at least of size
 * PICOTRNS_MAX_NUM_POSSYM  */
pico_status_t picotrns_trivial_syllabify(picoktab_Phones phones,
										 const picotrns_possym_t inSeq[],
										 const picoos_uint16 inSeqLen,
										 picotrns_possym_t outSeq[],
										 picoos_uint16* outSeqLen,
										 picoos_uint16 maxOutSeqLen) {
		...
		if (j < inSeqLen) { /* j is at the start of a new vowel */
			...
			while (i < j - 1) {
				if ((accent > 0) && (i == accentpos)) {
					...
					PICODBG_TRACE(("copying inSeq[%i].sym = %c (consonant) into output buffer", i,
								   inSeq[i].sym));
					outSeq[out++] = inSeq[i++];
				}
			}
			...
			if ((accent > 0) && (i == accentpos)) {
				...
				PICODBG_TRACE(
					("copying inSeq[%i].sym = %c (consonant) into output buffer", i, inSeq[i].sym));
				outSeq[out++] = inSeq[i++];
			}
			vowelFound = TRUE;
			...
			while ((i < inSeqLen) && picoktab_isSyllCarrier(phones, (picoos_uint8)inSeq[i].sym)) {
				PICODBG_TRACE(
					("copying inSeq[%i].sym = %c (vowel) into output buffer", i, inSeq[i].sym));
				outSeq[out++] = inSeq[i++];
			}
			j = i;
			...
			while (i < j) {
				PICODBG_TRACE(
					("copying inSeq[%i].sym = %c (consonant or stress) into output buffer", i,
					 inSeq[i].sym));
				outSeq[out++] = inSeq[i++];
			}
		}
...
static void GetNextAlternative(picokfst_FST fst,
							   picotrns_AltDesc altDesc,
							   const picotrns_possym_t inSeq[],
							   picoos_uint16 inSeqLen,
	...
	do {
		switch (altDesc->altState) {
			case 0: /* before pair search */
				if (altDesc->inPos < inSeqLen) {
					...
					if (inSym == PICOKFST_SYMID_EPS) {
						...
						picokfst_kfstStartPairSearch(fst, inSeq[altDesc->inPos].sym, &inSymFound,
													 &altDesc->searchState);
						if (!inSymFound) {
							altDesc->altState = 2;
							PICODBG_INFO_CTX();
			...
			case 1: /* within pair search */
				picokfst_kfstGetNextPair(fst, &altDesc->searchState, &pairFound, &(*outSym),
										 &pairClass);
				if (pairFound) {
			...
			case 2: /* before inEps trans search */
				picokfst_kfstStartInEpsTransSearch(fst, altDesc->startFSTState, &inEpsTransFound,
												   &altDesc->searchState);
				if (inEpsTransFound) {
					altDesc->altState = 3;
				} else {
			...
			case 3: /* within inEps trans search */
				picokfst_kfstGetNextInEpsTrans(fst, &altDesc->searchState, &inEpsTransFound,
											   &(*outSym), &(*endFSTState));
				if (inEpsTransFound) {
					(*found) = 1;
...
static void NoteSolution(picoos_uint32* nrSol,
						 picotrns_printSolutionFct printSolution,
						 picotrns_altDesc_t altDesc[],
						 picoos_uint16 pathLen,
						 picotrns_possym_t outSeq[],
						 picoos_uint16* outSeqLen,
						 picoos_uint16 maxOutSeqLen) {
...
/* Performs one step in the transduction of 'inSeqLen' input symbols with corresponding
   ...
   initialized by 'StartTransduction'. Repeat calls to this procedure until '*finished' returns
   true. The output is returned in 'outSeqLen' symbols and reference positions in 'outSeq'. The
   output reference positions refer to the corresponding input reference positions. Inserted output
   symbols receive the reference position -1. If several solutions are possible, only the last found
   solution is returned. 'altDesc' is a temporary workspace which should be at least one cell longer
   than 'outSeq'. 'firstSolOnly' determines whether only the first solution should be found or if
...
static void TransductionStep(picokfst_FST fst,
	...
	switch ((*transductionState).phase) {
		case 0: /* before initialization */
			...
			if (inSeqLen == 0) {
				NoteSolution(&(*transductionState).nrSol, printSolution, altDesc, 0, outSeq,
							 outSeqLen, maxOutSeqLen);
			}
		...
		case 1: /* before regular recursion step */
			if ((tmpRecPos < 0) || (firstSolOnly && ((*transductionState).nrSol > 0))) {
				...
				/* get first or next acceptable alternative in current position */
				GetNextAlternative(fst, &altDesc[tmpRecPos], inSeq, inSeqLen, &outSym, &outRefPos,
								   &endFSTState, &nextInPos, &found);
				if (found) {
					...
					if ((nextInPos == inSeqLen) &&
						picokfst_kfstIsAcceptingState(fst, endFSTState)) {
						NoteSolution(&(*transductionState).nrSol, printSolution, altDesc,
									 tmpRecPos + 1, outSeq, outSeqLen, maxOutSeqLen);
					}

					/* go to next position if possible, start search for follower alternative
					 * symbols */
					if (tmpRecPos < maxAltDescLen - 1) {
						...
						/* do not go on due to limited path but still treat alternatives in current
						 * position */
						PICODBG_WARN(
							("--- transduction path too long; may fail to find solution\n"));
					}
...
pico_status_t picotrns_transduce(picokfst_FST fst,
								 picoos_bool firstSolOnly,
								 picotrns_printSolutionFct printSolution,
								 const picotrns_possym_t inSeq[],
								 picoos_uint16 inSeqLen,
	...
	*nrSteps = 0;
	while (!finished) {
		TransductionStep(fst, &transductionState, altDescBuf, maxAltDescLen, firstSolOnly,
						 printSolution, inSeq, inSeqLen, outSeq, outSeqLen, maxOutSeqLen,
						 &finished);
		(*nrSteps)++;
	}
...
typedef struct picotrns_simple_transducer {
	...
	picoos_uint16 possymReadPos,
		possymWritePos; /* next pos to read from phonBufIn, next pos to write to phonBufIn */
...
pico_status_t picotrns_stTransduce(picotrns_SimpleTransducer this, picokfst_FST fst) {
	...
	status = picotrns_transduce(fst, TRUE, NULL, this->possymBuf, this->possymWritePos,
								this->possymBufTmp, &outSeqLen, PICOTRNS_MAX_NUM_POSSYM,
								this->altDescBuf, this->maxAltDescLen, &nrSteps);
	if (PICO_OK != status) {
		return status;
	}
	return picotrns_eliminate_epsilons(this->possymBufTmp, outSeqLen, this->possymBuf,
									   &this->possymWritePos, PICOTRNS_MAX_NUM_POSSYM);
}

/**
 * Add chars from NULLC-terminated string \c inStr, shifted to plane \c plane, to internal input
 * buffer of \c transducer.
 *
 ...
 */
pico_status_t picotrns_stAddWithPlane(picotrns_SimpleTransducer this,
									  picoos_char* inStr,
									  picoos_uint8 plane) {
	while ((*inStr) && (this->possymWritePos < PICOTRNS_MAX_NUM_POSSYM)) {
		...
		PICODBG_DEBUG(("inserting pos/sym = %i/'%c' at pos %i",
					   ...
					   this->possymBuf[this->possymWritePos].sym, this->possymWritePos));
		this->possymWritePos++;
		inStr++;
	}
	if (!(*inStr)) {
...
pico_status_t picotrns_stGetSymSequence(picotrns_SimpleTransducer this,
