...
#define PICODBG_INITIALIZE(level) picodbg_initialize(level)

#define PICODBG_TERMINATE() picodbg_terminate()

#define PICODBG_SET_LOG_LEVEL(level) picodbg_setLogLevel(level)

#define PICODBG_SET_LOG_FILTERFN(name) picodbg_setLogFilterFN(name)

#define PICODBG_SET_LOG_FILE(name) picodbg_setLogFile(name)

#define PICODBG_ENABLE_COLORS(flag) picodbg_enableColors(flag)

#define PICODBG_SET_OUTPUT_FORMAT(format) picodbg_setOutputFormat(format)

#define PICODBG_ASSERT(expr) \
	for (; !(expr); picodbg_assert(__FILE__, __LINE__, __FUNCTION__, #expr))

#define PICODBG_ASSERT_RANGE(val, min, max) PICODBG_ASSERT(((val) >= (min)) && ((val) <= (max)))

#define PICODBG_LOG(level, msg) \
	picodbg_log(level, 1, __FILE__, __LINE__, __FUNCTION__, picodbg_varargs msg)

#define PICODBG_ERROR(msg) PICODBG_LOG(PICODBG_LOG_LEVEL_ERROR, msg)

#define PICODBG_WARN(msg) PICODBG_LOG(PICODBG_LOG_LEVEL_WARN, msg)

#define PICODBG_INFO(msg) PICODBG_LOG(PICODBG_LOG_LEVEL_INFO, msg)

#define PICODBG_DEBUG(msg) PICODBG_LOG(PICODBG_LOG_LEVEL_DEBUG, msg)

#define PICODBG_TRACE(msg) PICODBG_LOG(PICODBG_LOG_LEVEL_TRACE, msg)

#define PICODBG_INFO_CTX() \
	picodbg_log(PICODBG_LOG_LEVEL_INFO, 0, __FILE__, __LINE__, __FUNCTION__, "")

#define PICODBG_INFO_MSG(msg) picodbg_log_msg(PICODBG_LOG_LEVEL_INFO, __FILE__, picodbg_varargs msg)
