...
static pico_status_t sigInitialize(register picodata_ProcessingUnit this, picoos_int32 resetMode) {
	...
	if (resetMode == PICO_RESET_FULL) {
		...
		sig_subObj->pdfmgc = picokpdf_getPdfMUL(this->voice->kbArray[PICOKNOW_KBID_PDF_MGC]);
		sig_subObj->pdflfz = picokpdf_getPdfMUL(this->voice->kbArray[PICOKNOW_KBID_PDF_LFZ]);
		sig_subObj->pdfphs = picokpdf_getPdfPHS(this->voice->kbArray[PICOKNOW_KBID_PDF_PHS]);

		sig_subObj->scmeanpowLFZ = sig_subObj->pdflfz->bigpow - sig_subObj->pdflfz->meanpow;
		sig_subObj->scmeanpowMGC = sig_subObj->pdfmgc->bigpow - sig_subObj->pdfmgc->meanpow;
...
static pico_status_t getPhsFromPdf(register picodata_ProcessingUnit this,
								   picoos_uint16 phsIndex,
								   picoos_int32* phsVect,
	...
	nIndexValue = (0xFF000000 & ((*(nCurrIndexOffset + 3)) << 24)) |
				  (0x00FF0000 & ((*(nCurrIndexOffset + 2)) << 16)) |
				  (0x0000FF00 & ((*(nCurrIndexOffset + 1)) << 8)) |
				  (0x000000FF & ((*nCurrIndexOffset)));
...
static pico_status_t sigProcess(register picodata_ProcessingUnit this,
								...
								picoos_uint16 numinb,
								picoos_uint16 outWritePos,
								picoos_uint16* numoutb) {
	...
	switch (sig_subObj->innerProcState) {
		case 0:
			...
			for (n_count = 0; n_count < CEPST_BUFF_SIZE - 1; n_count++) {
				...
				sig_subObj->sig_inner.PhIdBuff[n_count] =
					sig_subObj->sig_inner.PhIdBuff[n_count + 1];
				sig_subObj->sig_inner.VoicingBuff[n_count] =
					sig_subObj->sig_inner.VoicingBuff[n_count + 1];
			...
			for (n_count = 0; n_count < PHASE_BUFF_SIZE - 1; n_count++) {
				sig_subObj->sig_inner.VoxBndBuff[n_count] =
					sig_subObj->sig_inner.VoxBndBuff[n_count + 1];
			}
			...
			picoos_mem_copy(
				(void*)&sig_subObj->inBuf[inReadPos + sizeof(picodata_itemhead_t)], /*src*/
				(void*)&tmp_uint16, sizeof(tmp_uint16));							/*dest+size*/
			sig_subObj->sig_inner.PhIdBuff[CEPST_BUFF_SIZE - 1] =
				(picoos_int16)tmp_uint16;								  /*store into newest*/
			...
			for (i = 0; i < sig_subObj->pdflfz->ceporder; i++) {
				picoos_mem_copy(
					(void*)&(
						sig_subObj->inBuf[inReadPos + sizeof(picodata_itemhead_t) +
										  sizeof(tmp_uint16) + 3 * i * sizeof(tmp_uint16)]), /*src*/
					(void*)&tmp_uint16, sizeof(tmp_uint16)); /*dest+size*/

				sig_subObj->sig_inner.F0Buff[CEPST_BUFF_SIZE - 1] =
					(picoos_int16)tmp_uint16;								/*store into newest*/
				tmp_uint16 = (picoos_int16)sig_subObj->sig_inner.F0Buff[0]; /*assign oldest*/

				/*convert in float*/
				sig_subObj->sig_inner.F0_p =
					(tmp_uint16 ? ((picoos_single)tmp_uint16 / sig_subObj->scmeanLFZ)
								: (picoos_single)0.0);

				if (sig_subObj->sig_inner.F0_p != (picoos_single)0.0f) {
					sig_subObj->sig_inner.F0_p =
						(picoos_single)exp((picoos_single)sig_subObj->sig_inner.F0_p);
				}
				/* voicing */
				picoos_mem_copy(
					(void*)&(sig_subObj->inBuf[inReadPos + sizeof(picodata_itemhead_t) +
											   sizeof(tmp_uint16) + 3 * i * sizeof(tmp_uint16) +
											   sizeof(tmp_uint16)]), /*src*/
					(void*)&tmp_uint16, sizeof(tmp_uint16));		 /*dest+size*/

				sig_subObj->sig_inner.VoicingBuff[CEPST_BUFF_SIZE - 1] =
					(picoos_int16)tmp_uint16; /*store into newest*/
				tmp_uint16 = (picoos_int16)sig_subObj->sig_inner.VoicingBuff[0]; /*assign oldest*/

				sig_subObj->sig_inner.voicing =
					(picoos_single)((tmp_uint16 & 0x01) * 8 + (tmp_uint16 & 0x0e) / 2) /
					(picoos_single)15.0f;
				...
				picoos_mem_copy(
					(void*)&(sig_subObj->inBuf[inReadPos + sizeof(picodata_itemhead_t) +
											   sizeof(tmp_uint16) + 3 * i * sizeof(tmp_uint16) +
											   2 * sizeof(tmp_uint16)]), /*src*/
					(void*)&tmp_uint16, sizeof(tmp_uint16));			 /*dest+size*/

				sig_subObj->sig_inner.FuVBuff[CEPST_BUFF_SIZE - 1] =
					(picoos_int16)tmp_uint16;								 /*store into newest*/
				tmp_uint16 = (picoos_int16)sig_subObj->sig_inner.FuVBuff[0]; /*assign oldest*/

				sig_subObj->sig_inner.Fuv_p = (picoos_single)tmp_uint16 / sig_subObj->scmeanLFZ;
				sig_subObj->sig_inner.Fuv_p =
					(picoos_single)EXP((double)sig_subObj->sig_inner.Fuv_p);
			}
			/*load cep values*/
			offset = inReadPos + sizeof(picodata_itemhead_t) + sizeof(tmp_uint16) +
			...
			for (i = 0; i < sig_subObj->pdfmgc->ceporder; i++) {
				picoos_mem_copy((void*)&(sig_subObj->inBuf[offset + i * sizeof(tmp_int16)]), /*src*/
			...
			if (sig_subObj->inBuf[inReadPos + 3] > sig_subObj->inBuf[inReadPos + 2] * 2 + 8) {
				...
				getPhsFromPdf(this, tmp_int16, tmp1,
							  &(sig_subObj->sig_inner.VoxBndBuff[PHASE_BUFF_SIZE - 1]));
			} else {
			...
			if (sig_subObj->sig_inner.n_available > 3) {
				sig_subObj->sig_inner.n_available = 3;
			}
		...
		case 9:
			...
			*numoutb = 0;
			...
			for (n_count = 0; n_count < n_frames; n_count++) {
				sig_subObj->outBuf[outWritePos] = (picoos_uint8)PICODATA_ITEM_FRAME;
				sig_subObj->outBuf[outWritePos + 1] = (picoos_uint8)(hop_p_half);
				sig_subObj->outBuf[outWritePos + 2] =
					(picoos_uint8)(sig_subObj->nNumFrame % ((hop_p_half)));
				sig_subObj->outBuf[outWritePos + 3] = (picoos_uint8)sig_subObj->sig_inner.hop_p;
			...
			tmp2 = &(sig_subObj->sig_inner.WavBuff_p[sig_subObj->sig_inner.hop_p]);
			FAST_DEVICE(cnt, *(tmp1++) = *(tmp2++););
			cnt = sig_subObj->sig_inner.m2_p -
				  (sig_subObj->sig_inner.m2_p - sig_subObj->sig_inner.hop_p);
...
static pico_status_t sig_deal_with(const picoos_uint8* item) {
	...
	switch (head.type) {
		...
		case PICODATA_ITEM_CMD:
			if ((head.info1 == PICODATA_ITEMINFO1_CMD_PLAY) ||
				(head.info1 == PICODATA_ITEMINFO1_CMD_SAVE) ||
				(head.info1 == PICODATA_ITEMINFO1_CMD_UNSAVE)) {
				if (head.info2 == PICODATA_ITEMINFO2_CMD_TO_SIG) {
					return TRUE;
				}
			}
			if ((head.info1 == PICODATA_ITEMINFO1_CMD_PITCH) ||
				(head.info1 == PICODATA_ITEMINFO1_CMD_VOLUME) ||
				(head.info1 == PICODATA_ITEMINFO1_CMD_SPEAKER)) {
				return TRUE;
			}
			break;
		default:
	...
	while (1) { /* exit via return */
		...
		switch (sig_subObj->procState) {
			case PICOSIG_COLLECT:
				...
				s_result =
					picodata_cbGetItem(this->cbIn, &(sig_subObj->inBuf[sig_subObj->inWritePos]),
				...
				if ((PICO_OK == s_result) && (blen > 0)) {
					...
					s_result =
						picodata_is_valid_item(&(sig_subObj->inBuf[sig_subObj->inWritePos]), blen);
					if (s_result != TRUE) {
			...
			case PICOSIG_SCHEDULE:
				/* *************** item processing ***********************************/
				numinb = PICODATA_ITEM_HEADSIZE + sig_subObj->inBuf[sig_subObj->inReadPos + 3];

				/*verify that current item has to be dealth with by this PU*/
				s_deal_with = sig_deal_with(&(sig_subObj->inBuf[sig_subObj->inReadPos]));

				switch (s_deal_with) {
					case TRUE:
						/* we have to manage this item */
						if (FALSE == sig_is_command(&(sig_subObj->inBuf[sig_subObj->inReadPos]))) {
							...
							switch (sig_subObj->inBuf[sig_subObj->inReadPos + 1]) {
								case PICODATA_ITEMINFO1_CMD_PLAY:
									...
									sig_subObj->procState = sig_subObj->retState = PICOSIG_COLLECT;
									...
									picoos_strlcpy(
										...
										(picoos_char*)&(
											sig_subObj->inBuf[sig_subObj->inReadPos + 4]),
									...
									if (sig_subObj->sOutSDFile != NULL) {
										if (picoos_strncmp(
												...
												picoos_strlen((picoos_char*)s_temp_file_name)) ==
											0) {
											PICODBG_WARN(
									...
									b_res =
										picoos_sdfOpenIn(this->common, &(sig_subObj->sInSDFile),
														 s_temp_file_name, &sf, &enc, &numSamples);
									if (b_res != TRUE) {
										PICODBG_DEBUG(
									...
									sig_subObj->procState = sig_subObj->retState = PICOSIG_PLAY;
									return PICODATA_PU_BUSY;
									break;
								...
								case PICODATA_ITEMINFO1_CMD_SAVE:
									...
									picoos_strlcpy(
										...
										sig_subObj->inBuf[sig_subObj->inReadPos + 3] + 1);
									/*check extension*/
									if (picoos_has_extension(
											s_temp_file_name,
											PICODATA_PUTYPE_WAV_OUTPUT_EXTENSION) == FALSE) {
									...
									if (sig_subObj->sInSDFile != NULL) {
										if (picoos_strncmp(
												...
												picoos_strlen(
													(picoos_char*)sig_subObj->sInSDFileName)) ==
											0) {
											/*input and output files has the same name : do not
											 * allow opening for writing*/
											PICODBG_WARN(
												("input and output files has the same name!\n"));
											return PICODATA_PU_BUSY;
										}
									...
									/*try to open*/
									picoos_sdfOpenOut(this->common, &(sig_subObj->sOutSDFile),
													  s_temp_file_name, SAMPLE_FREQ_16KHZ,
													  PICOOS_ENC_LIN);
									if (sig_subObj->sOutSDFile == NULL) {
								...
								case PICODATA_ITEMINFO1_CMD_UNSAVE:
									...
									if ((sig_subObj->sOutSDFile == NULL) ||
										...
										PICODBG_DEBUG(
											("Error on requesting a binary samples file output "
											 "closing : no file output handle exist\n"));
										return PICODATA_PU_BUSY;
									}
									picoos_sdfCloseOut(this->common, &(sig_subObj->sOutSDFile));
									sig_subObj->outSwitch = 0;
								...
								case PICODATA_ITEMINFO1_CMD_SPEAKER:
									...
									picoos_read_mem_pi_uint16(
										&(sig_subObj->inBuf[sig_subObj->inReadPos]), &n_pos,
										&n_value);
									b_res = FALSE;
									switch (sig_subObj->inBuf[sig_subObj->inReadPos + 2]) {
										case 'a':
											/*absloute modifier*/
											f_value =
												(picoos_single)n_value / (picoos_single)100.0f;
											b_res = TRUE;
											break;
										case 'r':
											/*relative modifier*/
											f_value =
												(picoos_single)n_value / (picoos_single)1000.0f;
											b_res = TRUE;
											break;
										default:
									...
									if (b_res) {
										switch (sig_subObj->inBuf[sig_subObj->inReadPos + 1]) {
											...
											case PICODATA_ITEMINFO1_CMD_SPEAKER:
												...
												sig_subObj->sig_inner.sMod_p = sig_subObj->sMod;
												/*call the function needed to initialize the speaker
												 * factor*/
												mel_2_lin_init(&(sig_subObj->sig_inner));
												break;
											default:
												break;
										}
					...
					case FALSE:
						...
						s_result = picodata_copy_item(
							&(sig_subObj->inBuf[sig_subObj->inReadPos]), numinb,
							&(sig_subObj->outBuf[sig_subObj->outWritePos]),
							sig_subObj->outBufSize - sig_subObj->outWritePos, &numoutb);
						if (s_result != PICO_OK) {
						...
						/*if end of sentence reset number of frames(only needed for debugging
						 * purposes)*/
						if ((sig_subObj->inBuf[sig_subObj->inReadPos] == PICODATA_ITEM_BOUND) &&
							((sig_subObj->inBuf[sig_subObj->inReadPos + 1] ==
							  PICODATA_ITEMINFO1_BOUND_SEND) ||
							 (sig_subObj->inBuf[sig_subObj->inReadPos + 1] ==
							  PICODATA_ITEMINFO1_BOUND_TERM))) {
							PICODBG_INFO(
								("End of sentence - Processed frames : %d", sig_subObj->nNumFrame));
							sig_subObj->nNumFrame = 0;
						}
			...
			case PICOSIG_PROCESS:
				...
				/*Process a full item*/
				s_result = sigProcess(this, sig_subObj->inReadPos, numinb, sig_subObj->outWritePos,
									  &numoutb);
				...
				if (s_result == PICO_OK) {
					...
					sig_subObj->retState = PICOSIG_COLLECT;
					PICODBG_DEBUG(
						("picosig.sigStep -- leaving PICO_PROC, inReadPos = %i, outWritePos = %i",
						 sig_subObj->inReadPos, sig_subObj->outWritePos));
			...
			case PICOSIG_PLAY:
				...
				n_samp = hop_p_half;
				b_res = picoos_sdfGetSamples(sig_subObj->sInSDFile, sig_subObj->sInSDFilePos,
											 &n_samp, s_data);
				...
				/*Add header info*/
				sig_subObj->outBuf[sig_subObj->outWritePos] = (picoos_uint8)PICODATA_ITEM_FRAME;
				sig_subObj->outBuf[sig_subObj->outWritePos + 1] = (picoos_uint8)n_samp;
				sig_subObj->outBuf[sig_subObj->outWritePos + 2] =
					(picoos_uint8)(sig_subObj->nNumFrame % (hop_p_half)); /*number of frame % 64*/
				sig_subObj->outBuf[sig_subObj->outWritePos + 3] = (picoos_uint8)n_samp * 2;
				/*Item content*/
			...
			case PICOSIG_FEED:
				...
				switch (sig_subObj->outSwitch) {
					case 0:
						...
						s_result = picodata_cbPutItem(
							this->cbOut, &(sig_subObj->outBuf[sig_subObj->outReadPos]),
							sig_subObj->outWritePos - sig_subObj->outReadPos, &numoutb);
						break;
					case 1:
						/*feeding items to file*/
						if (sig_subObj->outBuf[sig_subObj->outReadPos] == PICODATA_ITEM_FRAME) {
							if ((sig_subObj->sOutSDFile) != NULL) {
								n_start = (picoos_uint32)(sig_subObj->outReadPos) +
										  PICODATA_ITEM_HEADSIZE;
								n_bytes =
									(picoos_uint32)sig_subObj
										->outBuf[(sig_subObj->outReadPos) + PICODATA_ITEMIND_LEN];
								n_fram =
									(picoos_uint32)sig_subObj
										->outBuf[(sig_subObj->outReadPos) + PICODATA_ITEMIND_INFO2];
								if (picoos_sdfPutSamples(
										sig_subObj->sOutSDFile, n_bytes / 2,
				...
				} else if ((PICO_EXC_BUF_UNDERFLOW == s_result) || (PICO_ERR_OTHER == s_result)) {
