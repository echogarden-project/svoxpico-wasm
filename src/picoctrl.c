 ...
 * the role of "current PU" to another sub-PU, according to the status information returned from
 * each PU.
 */
...
static pico_status_t ctrlInitialize(register picodata_ProcessingUnit this, picoos_int32 resetMode) {
	...
	if (PICO_OK != status) {
		picoos_emRaiseException(this->common->em, status, NULL,
								(picoos_char*)"problem (re-)initializing the engine");
	}
	return status;
} /*ctrlInitialize*/
...
static picodata_step_result_t ctrlStep(register picodata_ProcessingUnit this,
	...
	*bytesOutput = 0;
	...
	/* --------------------- */
	status = ctrl->procStatus[ctrl->curPU] =
		ctrl->procUnit[ctrl->curPU]->step(ctrl->procUnit[ctrl->curPU], mode, &puBytesOutput);
	...
	switch (status) {
		...
		case PICODATA_PU_BUSY:
			...
			if ((ctrl->curPU + 1 < ctrl->numProcUnits) &&
				(PICODATA_PU_BUSY == ctrl->procStatus[ctrl->curPU + 1])) {
				ctrl->curPU++;
			}
			return status;
			break;
		...
		case PICODATA_PU_IDLE:
			...
			if ((ctrl->curPU + 1 < ctrl->numProcUnits) &&
				...
				PICODBG_DEBUG(("find non-idle pu above from pu %d with status %d", ctrl->curPU,
							   ctrl->procStatus[ctrl->curPU]));
				while ((ctrl->curPU > 0) && (PICODATA_PU_IDLE == ctrl->procStatus[ctrl->curPU])) {
					ctrl->curPU--;
				}
				ctrl->procStatus[ctrl->curPU] = PICODATA_PU_BUSY;
			}
			PICODBG_DEBUG(
				("going to pu %d with status %d", ctrl->curPU, ctrl->procStatus[ctrl->curPU]));
...
static pico_status_t ctrlAddPU(register picodata_ProcessingUnit this,
	...
	if (last) {
		...
		ctrl->procCbOut[newPU] = picodata_newCharBuffer(this->common->mm, this->common, bufSize);
	...
	switch (puType) {
		case PICODATA_PUTYPE_TOK:
			...
			ctrl->procUnit[newPU] = picotok_newTokenizeUnit(this->common->mm, this->common, cbIn,
															ctrl->procCbOut[newPU], this->voice);
			break;
		case PICODATA_PUTYPE_PR:
			PICODBG_DEBUG(("creating PreprocUnit for pu %i", newPU));
			ctrl->procUnit[newPU] = picopr_newPreprocUnit(this->common->mm, this->common, cbIn,
														  ctrl->procCbOut[newPU], this->voice);
			break;
		case PICODATA_PUTYPE_WA:
			PICODBG_DEBUG(("creating WordAnaUnit for pu %i", newPU));
			ctrl->procUnit[newPU] = picowa_newWordAnaUnit(this->common->mm, this->common, cbIn,
														  ctrl->procCbOut[newPU], this->voice);
			break;
		case PICODATA_PUTYPE_SA:
			PICODBG_DEBUG(("creating SentAnaUnit for pu %i", newPU));
			ctrl->procUnit[newPU] = picosa_newSentAnaUnit(this->common->mm, this->common, cbIn,
														  ctrl->procCbOut[newPU], this->voice);
			break;
		case PICODATA_PUTYPE_ACPH:
			PICODBG_DEBUG(("creating AccPhrUnit for pu %i", newPU));
			ctrl->procUnit[newPU] = picoacph_newAccPhrUnit(this->common->mm, this->common, cbIn,
														   ctrl->procCbOut[newPU], this->voice);
			break;
		case PICODATA_PUTYPE_SPHO:
			PICODBG_DEBUG(("creating SentPhoUnit for pu %i", newPU));
			ctrl->procUnit[newPU] = picospho_newSentPhoUnit(this->common->mm, this->common, cbIn,
															ctrl->procCbOut[newPU], this->voice);
			break;
		case PICODATA_PUTYPE_PAM:
			PICODBG_DEBUG(("creating PAMUnit for pu %i", newPU));
			ctrl->procUnit[newPU] = picopam_newPamUnit(this->common->mm, this->common, cbIn,
													   ctrl->procCbOut[newPU], this->voice);
			break;
		case PICODATA_PUTYPE_CEP:
			PICODBG_DEBUG(("creating CepUnit for pu %i", newPU));
			ctrl->procUnit[newPU] = picocep_newCepUnit(this->common->mm, this->common, cbIn,
													   ctrl->procCbOut[newPU], this->voice);
			break;
#if defined(PICO_DEVEL_MODE)
		case PICODATA_PUTYPE_SINK:
			PICODBG_DEBUG(("creating SigUnit for pu %i", newPU));
			ctrl->procUnit[newPU] = picosink_newSinkUnit(this->common->mm, this->common, cbIn,
														 ctrl->procCbOut[newPU], this->voice);
			break;
#endif
		case PICODATA_PUTYPE_SIG:
			PICODBG_DEBUG(("creating SigUnit for pu %i", newPU));
			ctrl->procUnit[newPU] = picosig_newSigUnit(this->common->mm, this->common, cbIn,
													   ctrl->procCbOut[newPU], this->voice);
			break;
		default:
			ctrl->procUnit[newPU] = picodata_newProcessingUnit(this->common->mm, this->common, cbIn,
															   ctrl->procCbOut[newPU], this->voice);
			break;
	}
...
void picoctrl_disposeControl(picoos_MemoryManager mm, picodata_ProcessingUnit* this);
...
picodata_ProcessingUnit picoctrl_newControl(picoos_MemoryManager mm,
	...
	if ((PICO_OK == ctrlAddPU(this, PICODATA_PUTYPE_TOK, FALSE, /*last*/ FALSE)) &&
		...
		/* we don't call ctrlInitialize here because ctrlAddPU does initialize the PUs allready and
		 * the only thing remaining to initialize is:
		 */
		ctrl->curPU = 0;
		return this;
	} else {
...
#define SET_MAGIC_NUMBER(eng) (eng)->magic = ((picoos_uint32)(eng)) ^ MAGIC_MASK

#define CHECK_MAGIC_NUMBER(eng) ((eng)->magic == (((picoos_uint32)(eng)) ^ MAGIC_MASK))
...
pico_status_t picoctrl_engReset(picoctrl_Engine this, picoos_int32 resetMode) {
	...
	if (PICO_OK != status) {
		picoos_emRaiseException(this->common->em, status, NULL,
								(picoos_char*)"problem resetting engine");
	}
	return status;
}
...
picoctrl_Engine picoctrl_newEngine(picoos_MemoryManager mm,
	...
	if (done) {
		...
		this->cbIn = picodata_newCharBuffer(this->common->mm, this->common, bSize);
		bSize = picodata_get_default_buf_size(PICODATA_PUTYPE_SIG);

		this->cbOut = picodata_newCharBuffer(this->common->mm, this->common, bSize);

		PICODBG_DEBUG(("cbOut has address %i", (picoos_uint32)this->cbOut));

		this->control = picoctrl_newControl(this->common->mm, this->common, this->cbIn, this->cbOut,
											this->voice);
		done = (NULL != this->cbIn) && (NULL != this->cbOut) && (NULL != this->control);
	}
	if (done) {
 ...
 * @callergraph
 */
void picoctrl_disposeEngine(picoos_MemoryManager mm,
							picorsrc_ResourceManager rm,
 ...
 * @callergraph
 */
void picoctrl_engResetExceptionManager(picoctrl_Engine this) {
	picoos_emReset(this->common->em);
 ...
 * @callergraph
 */
picodata_step_result_t picoctrl_engFetchOutputItemBytes(picoctrl_Engine this,
														picoos_char* buffer,
	...
	if (PICODATA_PU_ERROR != stepResult) {
		...
		rv = picodata_cbGetSpeechData(this->cbOut, (picoos_uint8*)buffer, bufferSize, &ui);
 ...
 * @callergraph
 */
picodata_step_result_t picoctrl_getLastScheduledPU(picoctrl_Engine this) {
	ctrl_subobj_t* ctrl;
 ...
 * @callergraph
 */
picodata_step_result_t picoctrl_getLastProducedItemType(picoctrl_Engine this) {
	ctrl_subobj_t* ctrl;
	if (NULL == this || NULL == this->control->subObj) {
		return PICO_ERR_OTHER;
	}
