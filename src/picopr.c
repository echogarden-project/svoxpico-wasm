...
#define PR_MAX_DATA_LEN_Z                                                 \
	PR_MAX_DATA_LEN + 1 /* all strings in picopr should use this constant \
...
static void pr_ALLOCATE(picodata_ProcessingUnit this,
						pr_MemTypes mType,
						void** adr,
...
/* allocates 'byteSize' bytes in the memery partition given by 'mType' */
{
	...
	if (mType == pr_WorkMem) {
		if ((pr->workMemTop + byteSize) < PR_WORK_MEM_SIZE) {
...
#if PR_TRACE_MEM
			PICODBG_INFO(
				("pr_WorkMem: +%u, tot:%i of %i", byteSize, pr->workMemTop, PR_WORK_MEM_SIZE));
#endif
			...
			picoos_emRaiseException(this->common->em, PICO_EXC_OUT_OF_MEM,
									(picoos_char*)"pr out of dynamic memory", (picoos_char*)"");
			pr->outOfMemory = TRUE;
		}
		...
		if ((*adr) != NULL) {
			...
			picoos_getMemUsage(pr->dynMemMM, 1, &pr->dynMemSize, &incrUsedBytes,
							   &pr->maxDynMemSize);
#if PR_TRACE_MEM
			PICODBG_INFO(
				("pr_DynMem : +%i, tot:%i of %i", incrUsedBytes, pr->dynMemSize, PR_DYN_MEM_SIZE));
#endif
...
#if PR_TRACE_MEM
		PICODBG_INFO(("pr_DynMem : %i, tot:%i of %i: adr: %u", incrUsedBytes, pr->dynMemSize,
					  PR_DYN_MEM_SIZE, *adr));
#endif
	} else {
		(*adr) = NULL;
...
static void pr_getMemState(picodata_ProcessingUnit this,
						   pr_MemTypes mType,
						   picoos_uint32* lmemState) {
	pr_subobj_t* pr = (pr_subobj_t*)this->subObj;
	mType = mType; /* avoid warning "var not used in this function"*/
	*lmemState = pr->workMemTop;
}

static void pr_resetMemState(picodata_ProcessingUnit this,
							 pr_MemTypes mType,
							 picoos_uint32 lmemState) {
	pr_subobj_t* pr = (pr_subobj_t*)this->subObj;
...
#if PR_TRACE_MEM
	PICODBG_INFO(
		("pr_WorkMem: -%i, tot:%i of %i", pr->workMemTop - lmemState, lmemState, PR_WORK_MEM_SIZE));
#endif
	mType = mType; /* avoid warning "var not used in this function"*/
...
static void pr_getTermPartStr(picoos_uchar string[],
							  picoos_int32* ind,
							  picoos_uchar termCh,
							  picoos_uchar str[],
							  picoos_bool* done) {
...
static picoos_int32 tok_tokenDigitStrToInt(picodata_ProcessingUnit this,
										   pr_subobj_t* pr,
										   picoos_uchar stokenStr[]) {
	picoos_uint32 i;
	picoos_uint32 l;
	picoos_int32 id;
	picoos_int32 val;
...
static pico_Status pr_addContext(register picodata_ProcessingUnit this,
								 pr_subobj_t* pr,
								 pr_ContextList* ctxList,
								 picokpr_VarStrPtr contextNamePtr,
	...
	i = 0;
	net = pr->preproc[i];
	while ((i < PR_MAX_NR_PREPROC) && (net != NULL) &&
		   !(pr_strEqual(netNamePtr, picokpr_getPreprocNetName(net)))) {
		i++;
		net = pr->preproc[i];
	}
	if (net != NULL) {
...
static pico_Status pr_createContextList(register picodata_ProcessingUnit this) {
	...
	for (p = 0; p < PR_MAX_NR_PREPROC; p++) {
		if (pr->preproc[p] != NULL) {
			...
			for (i = 1; i < n; i++) {
				ctxNamePtr = picokpr_getVarStrPtr(pr->preproc[p],
												  picokpr_getCtxCtxNameOfs(pr->preproc[p], i));
				netNamePtr = picokpr_getVarStrPtr(pr->preproc[p],
												  picokpr_getCtxNetNameOfs(pr->preproc[p], i));
				prodNamePtr = picokpr_getVarStrPtr(pr->preproc[p],
												   picokpr_getCtxProdNameOfs(pr->preproc[p], i));
...
static void pr_setContext(register picodata_ProcessingUnit this,
						  pr_subobj_t* pr,
						  picoos_uchar context[]) {
	pr_ContextList ctx;
	...
	if (ctx != NULL) {
		...
		picoos_emRaiseWarning(this->common->em, PICO_ERR_CONTEXT_NOT_FOUND,
							  (picoos_char*)"context '%s' not found; no change",
							  (picoos_char*)context);
	}
}
...
static picoos_uint32 pr_copyData(picoos_uint8* dest,
								 const picoos_uint8* src,
								 picoos_int32 nrBytes,
								 picoos_bool zeroTerm) {
	picoos_int32 i = 0;
...
static void pr_newItem(picodata_ProcessingUnit this,
					   pr_MemTypes mType,
					   pr_ioItemPtr* item,
					   picoos_uint8 itemType,
					   picoos_int32 size,
					   picoos_bool inItem) {
	pr_subobj_t* pr = (pr_subobj_t*)this->subObj;
...
static void pr_copyItemContent(picodata_ProcessingUnit this,
							   pr_ioItem* inItem,
							   pr_ioItem* outItem) {
	if (outItem != NULL) {
...
static void pr_copyItem(picodata_ProcessingUnit this,
						pr_MemTypes mType,
						pr_ioItemPtr inItem,
						pr_ioItemPtr* outItem) {
	pr_subobj_t* pr = (pr_subobj_t*)this->subObj;
...
static void pr_appendItem(picodata_ProcessingUnit this,
						  pr_ioItemPtr* firstItem,
						  pr_ioItemPtr* lastItem,
						  pr_ioItemPtr item) {
	if (item != NULL) {
		item->next = NULL;
		if ((*lastItem) == NULL) {
...
static void pr_putItem(picodata_ProcessingUnit this,
					   pr_subobj_t* pr,
					   pr_ioItemPtr* first,
					   pr_ioItemPtr* last,
					   picoos_uint8 itemType,
...
static void pr_appendItemToOutItemList(picodata_ProcessingUnit this,
									   pr_subobj_t* pr,
									   pr_ioItemPtr* firstItem,
									   pr_ioItemPtr* lastItem,
									   pr_ioItemPtr item) {
	...
	item->next = NULL;
	if (((pr->spellMode != 0) && (item->head.type == PICODATA_ITEM_TOKEN) &&
		 (item->head.info1 != PICODATA_ITEMINFO1_TOKTYPE_SPACE))) {
		li = 0;
		...
		while (li < ln) {
			...
			if ((lid > 0) &&
				picoktab_getIntPropTokenType(pr->graphs, lid, &ltype) && ((ltype == PICODATA_ITEMINFO1_TOKTYPE_LETTERV) /*|| (ltype == PICODATA_ITEMINFO1_TOKTYPE_DIGIT)*/)) {
	...
	} else if (pr_isCmdType(item, PICODATA_ITEMINFO1_CMD_SPELL) &&
			   pr_isCmdInfo2(item, PICODATA_ITEMINFO2_CMD_START)) {
		pr->spellMode = item->data[0] + 256 * item->data[1];
		pr_disposeItem(this, &item);
	} else if (pr_isCmdType(item, PICODATA_ITEMINFO1_CMD_SPELL) &&
			   pr_isCmdInfo2(item, PICODATA_ITEMINFO2_CMD_END)) {
		pr->spellMode = 0;
		pr_disposeItem(this, &item);
	} else {
...
static void pr_genCommands(picodata_ProcessingUnit this,
						   pr_subobj_t* pr,
						   picokpr_Preproc network,
						   picokpr_OutItemArrOffset outitem,
						   pr_OutItemVarPtr vars,
	...
	switch (ltype) {
		case PR_OIgnore:
			if (picokpr_getOutItemVal(network, outitem) == 0) {
				pr_putItem(this, pr, &(*first), &(*last), PICODATA_ITEM_CMD,
						   PICODATA_ITEMINFO1_CMD_IGNORE, PICODATA_ITEMINFO2_CMD_START, 0,
						   (picoos_uchar*)"");
			} else {
				pr_putItem(this, pr, &(*first), &(*last), PICODATA_ITEM_CMD,
						   PICODATA_ITEMINFO1_CMD_IGNORE, PICODATA_ITEMINFO2_CMD_END, 0,
						   (picoos_uchar*)"");
			}
			break;
		case PR_OPitch:
		...
		case PR_OVolume:
			...
			if ((picokpr_getOutItemArgOfs(network, outitem) != 0)) {
				switch (
					picokpr_getOutItemType(network, picokpr_getOutItemArgOfs(network, outitem))) {
					case PR_OVal:
						pr_putItem(this, pr, &(*first), &(*last), PICODATA_ITEM_CMD, lcmd,
								   PICODATA_ITEMINFO2_CMD_ABSOLUTE,
								   picokpr_getOutItemVal(
									   network, picokpr_getOutItemArgOfs(network, outitem)),
								   (picoos_uchar*)"");
						break;
					case PR_OVar:
						lvar = pr_findVariable(
							vars, picokpr_getOutItemVal(
									  network, picokpr_getOutItemArgOfs(network, outitem)));
						if ((lvar != NULL) && (lvar->first != NULL) &&
							(lvar->first->head.type == PICODATA_ITEM_TOKEN)) {
							pr_putItem(this, pr, &(*first), &(*last), PICODATA_ITEM_CMD, lcmd,
									   PICODATA_ITEMINFO2_CMD_ABSOLUTE,
									   picoos_atoi((picoos_char*)lvar->first->data),
									   (picoos_uchar*)"");
						}
						break;
					default:
						pr_startItemList(&lf, &ll);
						pr_getOutputItemList(this, pr, network,
											 picokpr_getOutItemArgOfs(network, outitem), vars, &lf,
											 &ll);
						if (pr->outOfMemory) {
							return;
						}
						if (((lf != NULL) && (lf->head.type == PICODATA_ITEM_TOKEN))) {
							pr_putItem(this, pr, &(*first), &(*last), PICODATA_ITEM_CMD, lcmd,
									   PICODATA_ITEMINFO2_CMD_ABSOLUTE,
									   picoos_atoi((picoos_char*)lf->data), (picoos_uchar*)"");
						}
						break;
				}
			} else {
				pr_putItem(this, pr, &(*first), &(*last), PICODATA_ITEM_CMD, lcmd,
						   PICODATA_ITEMINFO2_CMD_ABSOLUTE, lnum, (picoos_uchar*)"");
			}
			break;
		...
		case PR_OPhonSAMPA:
			if (picokpr_getOutItemArgOfs(network, outitem) != 0) {
				...
				pr_startItemList(&lf, &ll);
				pr_getOutputItemList(this, pr, network, picokpr_getOutItemArgOfs(network, outitem),
									 vars, &lf, &ll);
				if (pr->outOfMemory) {
					return;
				}
				if (lf != NULL) {
					...
					if (lf->head.type == PICODATA_ITEM_TOKEN) {
						if (picodata_mapPAStrToPAIds(
								pr->transducer, this->common, pr->xsampa_parser, pr->svoxpa_parser,
								pr->xsampa2svoxpa_mapper, lf->data, alphabet, pr->tmpStr1,
								sizeof(pr->tmpStr1) - 1) == PICO_OK) {
							pr_putItem(this, pr, &(*first), &(*last), PICODATA_ITEM_CMD,
									   PICODATA_ITEMINFO1_CMD_PHONEME, PICODATA_ITEMINFO2_CMD_START,
									   0, pr->tmpStr1);
							ldone = TRUE;
						} else {
							PICODBG_WARN(
								("cannot map phonetic string '%s'; synthesizeing text instead",
								 lf->data));
					...
					if (ldone) {
						...
						while (lf != NULL) {
							if (lf->head.type == PICODATA_ITEM_TOKEN) {
								pr_putItem(this, pr, &(*first), &(*last), PICODATA_ITEM_TOKEN,
						...
						pr_putItem(this, pr, &(*first), &(*last), PICODATA_ITEM_CMD,
								   PICODATA_ITEMINFO1_CMD_PHONEME, PICODATA_ITEMINFO2_CMD_END, 0,
								   (picoos_char*)"");
					}
				}
			}
		...
		case PR_OSent:
			pr_putItem(this, pr, &(*first), &(*last), PICODATA_ITEM_CMD,
					   PICODATA_ITEMINFO1_CMD_FLUSH, PICODATA_ITEMINFO2_NA, 0, (picoos_uchar*)"");
			break;
		case PR_OPara:
			pr_putItem(this, pr, &(*first), &(*last), PICODATA_ITEM_CMD,
					   PICODATA_ITEMINFO1_CMD_FLUSH, PICODATA_ITEMINFO2_NA, 0, (picoos_uchar*)"");
			if (picokpr_getOutItemVal(network, outitem) == 1) {
				pr_putItem(this, pr, &(*first), &(*last), PICODATA_ITEM_CMD,
						   PICODATA_ITEMINFO1_CMD_SIL, PICODATA_ITEMINFO2_NA,
						   PICO_PARAGRAPH_PAUSE_DUR, (picoos_uchar*)"");
			}
			break;
		case PR_OBreak:
			if ((picokpr_getOutItemArgOfs(network, outitem) != 0)) {
			...
			if (picokpr_getOutItemArgOfs(network, outitem) != 0) {
				switch (
					...
					case PR_OVar:
						...
						pr_putItem(this, pr, &(*first), &(*last), PICODATA_ITEM_CMD, lcmd, lnum, 0,
								   pr->tmpStr1);
						break;
					case PR_OStr:
						if (picokpr_getOutItemStrOfs(
								network, picokpr_getOutItemArgOfs(network, outitem)) != 0) {
							lstrp = picokpr_getOutItemStr(
								network, picokpr_getOutItemArgOfs(network, outitem));
							pr_putItem(this, pr, &(*first), &(*last), PICODATA_ITEM_CMD, lcmd, lnum,
									   0, lstrp);
						}
						break;
					default:
			...
			if (picokpr_getOutItemArgOfs(network, outitem) != 0) {
				switch (
					...
					case PR_OVar:
						...
						if ((lnum =
								 picodata_getPuTypeFromExtension(pr->tmpStr1, /*input*/ FALSE)) !=
							PICODATA_ITEMINFO2_CMD_TO_UNKNOWN) {
							if (pr->saveFile[0] != 0) {
								pr_putItem(
									this, pr, &(*first), &(*last), PICODATA_ITEM_CMD,
									PICODATA_ITEMINFO1_CMD_UNSAVE,
									picodata_getPuTypeFromExtension(pr->saveFile, /*input*/ FALSE),
									0, pr->saveFile);
							}
							pr_putItem(this, pr, &(*first), &(*last), PICODATA_ITEM_CMD, lcmd, lnum,
									   0, pr->tmpStr1);
					...
					case PR_OStr:
						if (picokpr_getOutItemStrOfs(
							...
							lstrp = picokpr_getOutItemStr(
								network, picokpr_getOutItemArgOfs(network, outitem));
							if ((lnum = picodata_getPuTypeFromExtension(lstrp, /*input*/ FALSE)) !=
								PICODATA_ITEMINFO2_CMD_TO_UNKNOWN) {
								if (pr->saveFile[0] != 0) {
									pr_putItem(this, pr, &(*first), &(*last), PICODATA_ITEM_CMD,
											   PICODATA_ITEMINFO1_CMD_UNSAVE,
											   picodata_getPuTypeFromExtension(pr->saveFile,
																			   /*input*/ FALSE),
											   0, pr->saveFile);
								}
								pr_putItem(this, pr, &(*first), &(*last), PICODATA_ITEM_CMD, lcmd,
					...
					default:
						...
						if (((lf != NULL) && (lf->head.type == PICODATA_ITEM_TOKEN))) {
							if ((lnum =
									 picodata_getPuTypeFromExtension(lf->data, /*input*/ FALSE)) !=
								PICODATA_ITEMINFO2_CMD_TO_UNKNOWN) {
								if (pr->saveFile[0] != 0) {
									pr_putItem(this, pr, &(*first), &(*last), PICODATA_ITEM_CMD,
											   PICODATA_ITEMINFO1_CMD_UNSAVE,
				...
				/*
								...
								pr_putItem(this, pr,& (*first),&
				   (*last),PICODATA_ITEM_CMD,lcmd,lnum,0,pr->tmpStr1);
				*/
			}
			break;
		case PR_OUseSig:
		case PR_OPlay:
			...
			if (picokpr_getOutItemArgOfs(network, outitem) != 0) {
				switch (
					...
					case PR_OVar:
						...
						pr_putItem(this, pr, &(*first), &(*last), PICODATA_ITEM_CMD,
								   PICODATA_ITEMINFO1_CMD_PLAY,
								   picodata_getPuTypeFromExtension(pr->tmpStr1, /*input*/ TRUE), 0,
								   pr->tmpStr1);
						lout = TRUE;
						break;
					case PR_OStr:
			...
			if (argOfs != 0) {
				...
				if (nextOfs != 0) {
					...
					if (nextOfs2 != 0) {
						...
						if (nextOfs3 != 0) {
							if ((picokpr_getOutItemType(network, nextOfs3) == PR_OStr) &&
								(picokpr_getOutItemStrOfs(network, nextOfs3) != 0)) {
								lstrp = picokpr_getOutItemStr(network, nextOfs3);
								pr_strcpy(lxfadebeg, lstrp);
							}
							nextOfs4 = picokpr_getOutItemNextOfs(network, nextOfs3);
							if (nextOfs4 != 0) {
								if ((picokpr_getOutItemType(network, nextOfs4) == PR_OStr) &&
									(picokpr_getOutItemStrOfs(network, nextOfs4) != 0)) {
			...
			if (lout) {
				pr_putItem(
					this, pr, &(*first), &(*last), PICODATA_ITEM_CMD, PICODATA_ITEMINFO1_CMD_PLAY,
					picodata_getPuTypeFromExtension(pr->tmpStr1, /*input*/ TRUE), 0, pr->tmpStr1);
			}
			pr_putItem(this, pr, &(*first), &(*last), PICODATA_ITEM_CMD, lcmd, linfo2, 0,
					   (picoos_uchar*)"");
			break;
		default:
	...
	while (lo != 0) {
		switch (picokpr_getOutItemType(network, lo)) {
			...
			case PR_OSpell:
				...
				if ((largOfs != 0) &&
					((lnextOfs = picokpr_getOutItemNextOfs(network, largOfs)) != 0)) {
								...
								if (lid > 0) {
									if (picoktab_getIntPropTokenType(pr->graphs, lid, &ltype)) {
										if ((ltype == PICODATA_ITEMINFO1_TOKTYPE_LETTERV)/* || (ltype == PICODATA_ITEMINFO1_TOKTYPE_DIGIT)*/) {
											pr->tmpItem.head.len = pr_strcat(
												pr->tmpItem.data, (picoos_uchar*)SPEC_CHAR);
										}
									}
									picoktab_getIntPropTokenSubType(pr->graphs, lid, &lsubtype);
								} else {
			...
			case PR_OConcat:
				pr_startItemList(&lf, &ll);
				pr_getOutputItemList(this, pr, network, picokpr_getOutItemArgOfs(network, lo), vars,
									 &lf, &ll);
				if (pr->outOfMemory) {
					return;
				}
			...
			case PR_OLeft:
				pr_startItemList(&lf, &ll);
				pr_getOutputItemList(
					this, pr, network,
					picokpr_getOutItemNextOfs(network, picokpr_getOutItemArgOfs(network, lo)), vars,
					&lf, &ll);
				if (pr->outOfMemory) {
					return;
				}
			...
			case PR_ORight:
				...
				if ((lf != NULL) && (lf->head.type == PICODATA_ITEM_TOKEN)) {
					...
					ln = 0;
					ln2 = 0;
					while ((ln < llen) && (ln2 < llen2) &&
						   (ln2 < (llen2 - picokpr_getOutItemVal(
											   network, picokpr_getOutItemArgOfs(network, lo))))) {
						ln = ln + picobase_det_utf8_length(pr->tmpItem.data[ln]);
						ln2++;
					}
...
static picoos_int32 pr_attrVal(picokpr_Preproc network,
							   picokpr_TokArrOffset tok,
							   pr_TokSetEleWP type) {
	pr_TokSetEleWP tse;
	picoos_int32 n;
...
static void pr_getOutput(picodata_ProcessingUnit this,
						 pr_subobj_t* pr,
						 picoos_int32* i,
						 picoos_int32 d,
						 pr_ioItemPtr* o,
	...
	while ((*i) < pr->rbestpath.rlen) {
		...
		if (with__0->rprodname != 0) {
			PICODBG_INFO(("pp path  :%s%s(", pr->lspaces,
						  picokpr_getVarStrPtr(with__0->rnetwork, with__0->rprodname)));
		}
		if ((pr->ritems[with__0->ritemid + 1] != NULL) &&
			(pr->ritems[with__0->ritemid + 1]->head.type == PICODATA_ITEM_TOKEN)) {
			PICODBG_INFO(
				("pp in (1): %s'%s'", pr->lspaces, pr->ritems[with__0->ritemid + 1]->data));
		}
		if ((pr->ritems[with__0->ritemid + 1] != NULL)) {
			while ((pr->rinItemList != NULL) &&
				   (pr->rinItemList != pr->ritems[with__0->ritemid + 1]) &&
				   (pr->rinItemList->head.type != PICODATA_ITEM_TOKEN)) {
				lit = pr->rinItemList;
		...
		if ((PR_TSE_MASK_PROD & wpset) != 0) {
			if ((PR_TSE_MASK_VAR & wpset) != 0) {
				lvar =
					pr_findVariable(lvars, pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSEVar));
				if (lvar == NULL) {
			...
			if (((PR_TSE_MASK_LEX & wpset) != 0) && ((PR_TSE_MASK_LETTER & npset) == 0)) {
				if (lfirst) {
					pr_newItem(this, pr_WorkMem, &lit, PICODATA_ITEM_TOKEN,
							   sizeof(struct pr_ioItem), /*inItem*/ FALSE);
					if (pr->outOfMemory) {
						return;
					}
					...
					if (pr->ritems[with__0->ritemid + 1]->head.info1 ==
						PICODATA_ITEMINFO1_TOKTYPE_SPACE) {
						lit->head.len = pr_strcpy(lit->data, (picoos_uchar*)"_");
					} else {
						lit->head.len =
							pr_strcpy(lit->data, pr->ritems[with__0->ritemid + 1]->data);
					}
					lvar->first = lit;
					lvar->last = lit;
					lfirst = FALSE;
					...
					if ((pr->ritems[with__0->ritemid + 1]->head.info1 ==
						...
						lvar->last->head.len =
							pr_strcat(lvar->last->data, pr->ritems[with__0->ritemid + 1]->data);
					}
			...
			pr_getOutputItemList(this, pr, with__0->rnetwork,
								 pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSEOut), lvars,
								 &(*o), &(*ol));
			if (pr->outOfMemory) {
				return;
			}
			(*i)++;
				   ...
				   (d != pr->rbestpath.rele[(*i) + 1].rdepth)) {
	...
	while (lf != NULL) {
		...
		if (pr_isCmdType(lit, PICODATA_ITEMINFO1_CMD_PLAY)) {
			...
			if (!lastPlayFileFound) {
				PICODBG_WARN(
					("file '%s' not found; synthesizing enclosed text instead", lit->data));
				picoos_emRaiseWarning(
					this->common->em, PICO_EXC_CANT_OPEN_FILE, (picoos_char*)"",
					(picoos_char*)"file '%s' not found; synthesizing enclosed text instead",
					lit->data);
			}
		}
		if (pr_isCmdType(lit, PICODATA_ITEMINFO1_CMD_PHONEME) &&
			pr_isCmdInfo2(lit, PICODATA_ITEMINFO2_CMD_START)) {
			pr->insidePhoneme = TRUE;
		} else if (pr_isCmdType(lit, PICODATA_ITEMINFO1_CMD_PHONEME) &&
				   pr_isCmdInfo2(lit, PICODATA_ITEMINFO2_CMD_END)) {
			pr->insidePhoneme = FALSE;
		}
		if ((pr_isCmdType(lit, PICODATA_ITEMINFO1_CMD_PLAY) && !lastPlayFileFound)) {
		} else if (pr_isCmdType(lit, PICODATA_ITEMINFO1_CMD_IGNORE) &&
				   pr_isCmdInfo2(lit, PICODATA_ITEMINFO2_CMD_START)) {
			if (lastPlayFileFound) {
				pr->rignore++;
			}
		...
		} else if (pr_isCmdType(lit, PICODATA_ITEMINFO1_CMD_IGNSIG) &&
				   pr_isCmdInfo2(lit, PICODATA_ITEMINFO2_CMD_START) && !lastPlayFileFound) {
		} else if (pr_isCmdType(lit, PICODATA_ITEMINFO1_CMD_IGNSIG) &&
				   pr_isCmdInfo2(lit, PICODATA_ITEMINFO2_CMD_END) && !lastPlayFileFound) {
			...
			if ((pr->rignore <= 0) &&
				!(pr->insidePhoneme && (pr_isCmdType(lit, PICODATA_ITEMINFO1_CMD_PLAY) ||
...
static void pr_compare(picoos_uchar str1lc[], picoos_uchar str2[], picoos_int16* result) {
	...
	j = 0;
	i = 0;
	while ((i < PR_MAX_DATA_LEN) && (str1lc[i] != 0) && (l > 0) && (j <= 3) &&
		   (str1lc[i] == utf8char[j])) {
		i++;
		j++;
		if (j >= l) {
			l = picobase_get_next_utf8char(str2, PR_MAX_DATA_LEN, &pos, utf8char);
			picobase_lowercase_utf8_str(utf8char, (picoos_char*)utf8char, PICOBASE_UTF8_MAXLEN + 1,
										&done);
			l = picobase_det_utf8_length(utf8char[0]);
			j = 0;
		}
...
static picoos_bool pr_hasToken(picokpr_TokSetWP* tswp, picokpr_TokSetNP* tsnp) {
	return ((((PR_TSE_MASK_SPACE | PR_TSE_MASK_DIGIT | PR_TSE_MASK_LETTER | PR_TSE_MASK_SEQ |
			   PR_TSE_MASK_CHAR | PR_TSE_MASK_BEGIN | PR_TSE_MASK_END) &
			  (*tsnp)) != 0) ||
...
static picoos_bool pr_findProduction(picodata_ProcessingUnit this,
									 pr_subobj_t* pr,
									 picoos_uchar str[],
									 picokpr_Preproc* network,
									 picokpr_TokArrOffset* tokOfs) {
	picoos_bool found;
	picoos_int32 p;
	picoos_int32 ind;
	picoos_int32 i;
		...
		if (!found && (pr->preproc[p] != NULL)) {
			if (pr_strEqual(pr->tmpStr1, picokpr_getPreprocNetName(pr->preproc[p]))) {
				...
				while (!found && (i <= (lprodarrlen - 1))) {
					lstrp = picokpr_getVarStrPtr(pr->preproc[p],
												 picokpr_getProdNameOfs(pr->preproc[p], i));
	...
	if ((pr->ractpath.rlen > 0) && (pr->ractpath.rlen < PR_MAX_PATH_LEN)) {
		...
		if ((PR_TSE_MASK_PROD & wpset) != 0) {
			if ((PR_TSE_MASK_PRODEXT & wpset) != 0) {
				...
				lstrp = picokpr_getVarStrPtr(
					with__0->rnetwork, pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSEProdExt));
				if (pr_findProduction(this, pr, lstrp,
									  &pr->ractpath.rele[pr->ractpath.rlen].rnetwork,
									  &pr->ractpath.rele[pr->ractpath.rlen].rtok)) {
					with__0->rprodname = picokpr_getProdNameOfs(
						with__0->rnetwork,
						pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSEProd));
					with__0->rprodprefcost = picokpr_getProdPrefCost(
						with__0->rnetwork,
						pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSEProd));
				...
				pr->ractpath.rele[pr->ractpath.rlen].rtok = picokpr_getProdATokOfs(
					with__0->rnetwork, pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSEProd));
				with__0->rprodname = picokpr_getProdNameOfs(
					with__0->rnetwork, pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSEProd));
				with__0->rprodprefcost = picokpr_getProdPrefCost(
					with__0->rnetwork, pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSEProd));
...
static picoos_bool pr_getProdContToken(picodata_ProcessingUnit this, pr_subobj_t* pr) {
	...
	while (
		(li > 0) &&
		!((pr->ractpath.rele[li].rdepth == (pr->ractpath.rele[pr->ractpath.rlen - 1].rdepth - 1)) &&
		  ((PR_TSE_MASK_PROD & picokpr_getTokSetWP(pr->ractpath.rele[li].rnetwork,
												   pr->ractpath.rele[li].rtok)) != 0))) {
		li--;
	}
	if (((li >= 0) && (pr->ractpath.rlen < PR_MAX_PATH_LEN) &&
		 (PR_TSE_MASK_NEXT &
		  picokpr_getTokSetNP(pr->ractpath.rele[li].rnetwork, pr->ractpath.rele[li].rtok)) != 0)) {
		pr_initPathEle(&pr->ractpath.rele[pr->ractpath.rlen]);
		pr->ractpath.rele[pr->ractpath.rlen].rnetwork = pr->ractpath.rele[li].rnetwork;
		pr->ractpath.rele[pr->ractpath.rlen].rtok =
			picokpr_getTokNextOfs(pr->ractpath.rele[li].rnetwork, pr->ractpath.rele[li].rtok);
...
static picoos_bool pr_getTopLevelToken(picodata_ProcessingUnit this,
									   pr_subobj_t* pr,
									   picoos_bool firstprod) {
	if (firstprod) {
		if (pr->actCtx != NULL) {
	...
	if ((pr->prodList != NULL) && (pr->prodList->rProdOfs != 0) &&
		...
		pr->ractpath.rele[pr->ractpath.rlen].rtok =
			picokpr_getProdATokOfs(pr->prodList->rNetwork, pr->prodList->rProdOfs);
		pr->ractpath.rele[pr->ractpath.rlen].rlState = PR_LSInit;
		pr->ractpath.rele[pr->ractpath.rlen].rcompare = -1;
		pr->ractpath.rele[pr->ractpath.rlen].rprodname =
			picokpr_getProdNameOfs(pr->prodList->rNetwork, pr->prodList->rProdOfs);
		pr->ractpath.rele[pr->ractpath.rlen].rprodprefcost =
			picokpr_getProdPrefCost(pr->prodList->rNetwork, pr->prodList->rProdOfs);
		pr->ractpath.rlen++;
		return TRUE;
	} else {
		return FALSE;
...
static pr_MatchState pr_matchMultiToken(picodata_ProcessingUnit this,
										pr_subobj_t* pr,
										picokpr_TokSetNP npset,
										picokpr_TokSetWP wpset) {
...
static pr_MatchState pr_matchTokensSpace(picodata_ProcessingUnit this,
										 pr_subobj_t* pr,
										 picoos_int32 cmpres,
										 picokpr_TokSetNP npset,
										 picokpr_TokSetWP wpset) {
	...
	if (((PR_TSE_MASK_LEN & wpset) != 0) &&
		(lulen != pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSELen))) {
		return PR_MSNotMatched;
	}
	if (((PR_TSE_MASK_MIN & wpset) != 0) &&
		(lulen < pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSEMin))) {
		return PR_MSNotMatched;
	}
	if (((PR_TSE_MASK_MAX & wpset) != 0) &&
		(lulen > pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSEMax))) {
		return PR_MSNotMatched;
	}
	...
	if (((PR_TSE_MASK_ID & wpset) != 0) &&
		(pr->ritems[with__0->ritemid + 1]->head.info2 !=
		 pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSEID))) {
		return PR_MSNotMatched;
	}
	return PR_MSMatched;
}

static pr_MatchState pr_matchTokensDigit(picodata_ProcessingUnit this,
										 pr_subobj_t* pr,
										 picoos_int32 cmpres,
										 picokpr_TokSetNP npset,
										 picokpr_TokSetWP wpset) {
	...
	if (((PR_TSE_MASK_MIN & wpset) != 0) &&
		(lval < pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSEMin))) {
		return PR_MSNotMatched;
	}
	if (((PR_TSE_MASK_MAX & wpset) != 0) &&
		(lval > pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSEMax))) {
		return PR_MSNotMatched;
	}
	if (((PR_TSE_MASK_STR & wpset) != 0) && (cmpres != PR_EQUAL)) {
		return PR_MSNotMatched;
	...
	if (((PR_TSE_MASK_VAL & wpset) != 0) &&
		(lval != pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSEVal))) {
		return PR_MSNotMatched;
	}
	if ((((PR_TSE_MASK_NLZ & npset) != 0) && lstrp[0] == '0')) {
		return PR_MSNotMatched;
	}
	if (((PR_TSE_MASK_HEAD & wpset) != 0) &&
		!(picokpr_isEqualHead(with__0->rnetwork, lstrp, PR_MAX_DATA_LEN,
							  pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSEHead)))) {
		return PR_MSNotMatched;
	}
	if (((PR_TSE_MASK_MID & wpset) != 0) &&
		!(picokpr_isEqualMid(with__0->rnetwork, lstrp, PR_MAX_DATA_LEN,
							 pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSEMid)))) {
		return PR_MSNotMatched;
	}
	if (((PR_TSE_MASK_TAIL & wpset) != 0) &&
		!(picokpr_isEqualTail(with__0->rnetwork, lstrp, PR_MAX_DATA_LEN,
							  pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSETail)))) {
		return PR_MSNotMatched;
	}
	if (((PR_TSE_MASK_ID & wpset) != 0) &&
		(pr->ritems[with__0->ritemid + 1]->head.info2 !=
...
static pr_MatchState pr_matchTokensSeq(picodata_ProcessingUnit this,
									   pr_subobj_t* pr,
									   picoos_int32 cmpres,
									   picokpr_TokSetNP npset,
									   picokpr_TokSetWP wpset) {
...
static pr_MatchState pr_matchTokensChar(picodata_ProcessingUnit this,
										pr_subobj_t* pr,
										picoos_int32 cmpres,
										picokpr_TokSetNP npset,
										picokpr_TokSetWP wpset) {
...
static pr_MatchState pr_matchTokensLetter(picodata_ProcessingUnit this,
										  pr_subobj_t* pr,
										  picoos_int32 cmpres,
										  picokpr_TokSetNP npset,
										  picokpr_TokSetWP wpset) {
	...
	if ((PR_TSE_MASK_CI & npset) != 0) {
		...
		if (((PR_TSE_MASK_HEAD & wpset) != 0) &&
			!(picokpr_isEqualHead(with__0->rnetwork, pr->ritems[with__0->ritemid + 1]->strci,
								  PR_MAX_DATA_LEN,
								  pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSEHead)))) {
			return PR_MSNotMatched;
		}
		if (((PR_TSE_MASK_MID & wpset) != 0) &&
			!(picokpr_isEqualMid(with__0->rnetwork, pr->ritems[with__0->ritemid + 1]->strci,
								 PR_MAX_DATA_LEN,
								 pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSEMid)))) {
			return PR_MSNotMatched;
		}
		if (((PR_TSE_MASK_TAIL & wpset) != 0) &&
			!(picokpr_isEqualTail(with__0->rnetwork, pr->ritems[with__0->ritemid + 1]->strci,
								  PR_MAX_DATA_LEN,
								  pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSETail)))) {
			return PR_MSNotMatched;
		}
		...
		if (((PR_TSE_MASK_STR & wpset) != 0) &&
			!(picokpr_isEqual(with__0->rnetwork, pr->ritems[with__0->ritemid + 1]->strcis,
							  PR_MAX_DATA_LEN,
							  pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSEStr)))) {
			return PR_MSNotMatched;
		}
		if (((PR_TSE_MASK_HEAD & wpset) != 0) &&
			!(picokpr_isEqualHead(with__0->rnetwork, pr->ritems[with__0->ritemid + 1]->strcis,
								  PR_MAX_DATA_LEN,
								  pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSEHead)))) {
			return PR_MSNotMatched;
		}
		if (((PR_TSE_MASK_MID & wpset) != 0) &&
			!(picokpr_isEqualMid(with__0->rnetwork, pr->ritems[with__0->ritemid + 1]->strcis,
								 PR_MAX_DATA_LEN,
								 pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSEMid)))) {
			return PR_MSNotMatched;
		}
		if (((PR_TSE_MASK_TAIL & wpset) != 0) &&
			!(picokpr_isEqualTail(with__0->rnetwork, pr->ritems[with__0->ritemid + 1]->strcis,
								  PR_MAX_DATA_LEN,
								  pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSETail)))) {
			return PR_MSNotMatched;
		}
		...
		if (((PR_TSE_MASK_STR & wpset) != 0) &&
			!(picokpr_isEqual(with__0->rnetwork, pr->ritems[with__0->ritemid + 1]->data,
							  PR_MAX_DATA_LEN,
							  pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSEStr)))) {
			return PR_MSNotMatched;
		}
		if (((PR_TSE_MASK_HEAD & wpset) != 0) &&
			!(picokpr_isEqualHead(with__0->rnetwork, pr->ritems[with__0->ritemid + 1]->data,
								  PR_MAX_DATA_LEN,
								  pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSEHead)))) {
			return PR_MSNotMatched;
		}
		if (((PR_TSE_MASK_MID & wpset) != 0) &&
			!(picokpr_isEqualMid(with__0->rnetwork, pr->ritems[with__0->ritemid + 1]->data,
								 PR_MAX_DATA_LEN,
								 pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSEMid)))) {
			return PR_MSNotMatched;
		}
		if (((PR_TSE_MASK_TAIL & wpset) != 0) &&
			!(picokpr_isEqualTail(with__0->rnetwork, pr->ritems[with__0->ritemid + 1]->data,
								  PR_MAX_DATA_LEN,
								  pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSETail)))) {
			return PR_MSNotMatched;
		}
	...
	if (((PR_TSE_MASK_ROMAN & npset) != 0) &&
		!(pr_isLatinNumber(pr->ritems[with__0->ritemid + 1]->data, &lromanval))) {
		return PR_MSNotMatched;
	}
	if (((PR_TSE_MASK_ID & wpset) != 0) &&
		(pr->ritems[with__0->ritemid + 1]->head.info2 !=
...
static pr_MatchState pr_matchTokensBegin(picodata_ProcessingUnit this,
										 pr_subobj_t* pr,
										 picokpr_TokSetNP npset,
										 picokpr_TokSetWP wpset) {
	npset = npset; /* avoid warning "var not used in this function"*/
	wpset = wpset; /* avoid warning "var not used in this function"*/
	if ((PR_TSE_MASK_BEGIN & picokpr_getTokSetNP(pr->ractpath.rele[pr->ractpath.rlen - 1].rnetwork,
												 pr->ractpath.rele[pr->ractpath.rlen - 1].rtok)) !=
		0) {
		return PR_MSMatched;
	} else {
		return PR_MSNotMatched;
...
static pr_MatchState pr_matchTokensEnd(picodata_ProcessingUnit this,
	...
	npset = npset; /* avoid warning "var not used in this function"*/
	wpset = wpset; /* avoid warning "var not used in this function"*/
	if ((PR_TSE_MASK_END & picokpr_getTokSetNP(pr->ractpath.rele[pr->ractpath.rlen - 1].rnetwork,
											   pr->ractpath.rele[pr->ractpath.rlen - 1].rtok)) !=
		0) {
		return PR_MSMatched;
	} else {
...
static pr_MatchState pr_matchTokens(picodata_ProcessingUnit this,
	...
	*cmpres = PR_EQUAL;
	if ((PR_TSE_MASK_STR & wpset) != 0) {
		lstrp = picokpr_getVarStrPtr(with__0->rnetwork,
									 pr_attrVal(with__0->rnetwork, with__0->rtok, PR_TSEStr));
...
static void pr_calcPathCost(struct pr_Path* path) {
	...
	for (li = 0; li < path->rlen; li++) {
		...
		if ((PR_TSE_MASK_COST & wpset) != 0) {
			if (((PR_TSE_MASK_LEX & wpset) == PR_TSE_MASK_LEX) &&
				((PR_TSE_MASK_LETTER & npset) == 0)) {
				if (lfirst) {
					path->rcost =
						path->rcost - PR_COST +
						pr_attrVal(path->rele[li].rnetwork, path->rele[li].rtok, PR_TSECost);
				} else {
				...
				case PR_LSInit:
					...
					if ((PR_TSE_MASK_ACCEPT & npset) != 0) {
						if (with__0->rdepth == 1) {
							...
							if ((pr->rbestpath.rlen == 0) ||
								(pr->ractpath.rcost < pr->rbestpath.rcost)) {
...
static void pr_processItems(picodata_ProcessingUnit this, pr_subobj_t* pr) {
	...
	while ((pr->rinItemList != NULL) && (pr->rinItemList->head.type != PICODATA_ITEM_TOKEN)) {
		...
		if (pr->insidePhoneme && (pr_isCmdType(lit, PICODATA_ITEMINFO1_CMD_PLAY) ||
								  pr_isCmdType(lit, PICODATA_ITEMINFO1_CMD_IGNSIG))) {
			pr_disposeItem(this, &lit);
		} else if (pr_isCmdType(lit, PICODATA_ITEMINFO1_CMD_CONTEXT)) {
			pr_setContext(this, pr, lit->data);
...
extern void pr_treatItem(picodata_ProcessingUnit this, pr_subobj_t* pr, pr_ioItemPtr item) {
	...
	if (!PR_ENABLED || (pr->rgState == PR_GSNoPreproc)) {
		...
		if (pr_isCmdType(item, PICODATA_ITEMINFO1_CMD_CONTEXT) ||
...
pico_status_t prReset(register picodata_ProcessingUnit this, picoos_int32 resetMode) {
	...
	pr->xsampa2svoxpa_mapper =
		picokfst_getFST(this->voice->kbArray[PICOKNOW_KBID_FST_XSAMPA2SVOXPA]);
...
pico_status_t prSubObjDeallocate(register picodata_ProcessingUnit this, picoos_MemoryManager mm) {
	pr_subobj_t* pr;
...
picodata_ProcessingUnit picopr_newPreprocUnit(picoos_MemoryManager mm,
	...
	PICODBG_INFO(("max dyn size: %i",
				  PR_MAX_PATH_LEN *
					  ((((PR_IOITEM_MIN_SIZE + 2) + PICOOS_ALIGN_SIZE - 1) / PICOOS_ALIGN_SIZE) *
						   PICOOS_ALIGN_SIZE +
					   16)));
#endif
	if (this->subObj == NULL) {
	...
	*numBytesOutput = 0;
	while (1) { /* exit via return */
		if ((pr->outWritePos - pr->outReadPos) > 0) {
			...
			if (picodata_cbPutItem(this->cbOut, &pr->outBuf[pr->outReadPos],
								   pr->outWritePos - pr->outReadPos, numBytesOutput) == PICO_OK) {
			...
			if (it->head.type == PICODATA_ITEM_TOKEN) {
				if ((it->head.info1 != PICODATA_ITEMINFO1_TOKTYPE_SPACE) && (it->head.len > 0)) {
					...
					if ((nrUtfChars == 1) &&
						...
						/* do subgraphs substitutions and deliver token items as
						   PICODATA_ITEM_WORDGRAPH items to the output buffer */
						split = FALSE;
						...
						while (pos < (picoos_uint32)len) {
							if (picobase_get_next_utf8char(it->data, it->head.len, &pos,
														   inUtf8char)) {
								if (inUtf8char[0] <= 32) {
									...
									*/
									split = TRUE;
											...
											0) &&
										   picoktab_getStrPropGraphsubs1(pr->graphs, id,
																		 outUtf8char)) {
									if (split) {
									...
									if (picoktab_getStrPropGraphsubs2(pr->graphs, id,
																	  outUtf8char)) {
										outUtf8charlen = picobase_det_utf8_length(outUtf8char[0]);
										for (i = 0; i < outUtf8charlen; i++) {
											pr->outBuf[pr->outWritePos++] = outUtf8char[i];
			...
			if (pr->dynMemSize < (45 * PR_DYN_MEM_SIZE / 100)) {
				...
				if ((pr->inBuf[0] == PICODATA_ITEM_TOKEN) &&
					((pr->inBuf[1] == PICODATA_ITEMINFO1_TOKTYPE_DIGIT))) {
			...
			/* there is not data in the output buffer and there is no data in the output item list,
			   so check whether input data is available */
			rv = picodata_cbGetItem(this->cbIn, pr->inBuf, IN_BUF_SIZE + PICODATA_ITEM_HEADSIZE,
									&pr->inBufLen);
			if (PICO_OK == rv) {
			} else if (PICO_EOF == rv) {
