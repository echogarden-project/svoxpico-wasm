...
/* copy src into dst, but make sure that dst is not accessed beyond its size 'siz' and is allways
 * NULLC-terminated. 'siz' is the number of bytes of the destination, including one byte for NULLC!
...
picopal_objsize_t picopal_fwrite_bytes(picopal_File f,
									   ...
									   picopal_objsize_t objsize,
									   picopal_uint32 nobj) {
	return (picopal_objsize_t)fwrite(ptr, objsize, nobj, (FILE*)f);
}
...
/*
 *  Reference:
 * A Fast, Compact Approximation of the Exponential Function by Nicol N. Schraudolph in Neural
 * Computation, 11,853-862 (1999) See also:
