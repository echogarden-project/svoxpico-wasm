 ...
 * =======================
 * The main function, sphoStep, is divided into the subprocesses (processing states) described
 * further down.
 *
 * Flow control:
 * ------------
 ...
 *                       - 'feedFollowState' : the state to be processed after the feed state (the
 * feed state is treated like a primitive "subroutine")
 *                       - some other flags
 *
 * Buffering:
 * ---------
 ...
 *                       - headx : structured buffer containing item heads, each expanded by a
 * pointer to the item contents and space for a boundary potentially to be inserted (to the left of
 * the original item)
 ...
 * Windowing:
 * ---------
 *   Optimal solutions are achieved if a whole sentence is processed at once. However, if any of the
 * buffers are too small, only sentence parts are processed. To improve the quality of such
 * sub-optimal solutions, a moving-window-with-overlap is applied:
 ...
 *   - penultima                     : the position (within the active range) that should be used as
 * new window start when shifting the window
 *
 * After PROCESS_PARSE:
 ...
 * |                                        |
 *                                    |------------... (only left context is known; new active
 * range,  penultima, and right context to be established at next parse)
 *
 * Processing states:
 * -----------------
 * - INIT              : initialize state variables
 * - COLLECT           : collect items into internal buffers ("inBuf")
 * - PROCESS_PARSE     : go through inBuf items and extract position/phoneme pairs into phoneme
 * buffer 'phonBuf' word boundary phonemes are inserted between words
 * - PROCESS_TRANSDUCE : transduce phonBuf
 * - PROCESS_BOUNDS    : go through inBuf items again and match against transduced pos/phoneme
 *                       this is the first round of alignment, only inserting/deleting/modifying
 * bounds, according to
 *                       - existing BOUND items
 *                       - newly produced word bounds separating WORDPHON items
 *                       - bound upgrades/downgrades from transduction
 *                       - bound upgrades/downgrades/insertions from SIL command items (originating
 * e.g. from <break> text commands) all relevant bounds are placed in the corresponding headx
 ...
 * - PROCESS_SYL       : the WORDPHON coming from RECOMB is matched against the phonBuf and (new)
 * SYLLPHON items are created. (the original wordphon is consumed)
 * - FEED              : feeds one item and returns to spho->feedFollowState
 * - SHIFT             : items in inBuf are shifted left to make room for new items. If a sentence
 * doesn't fit inBuf in its entirety, left and/or right contexts are kept so they can be considered
 * in the next transduction.
 */
...
typedef struct spho_subobj {
	...
	/* we use int16 for buffer positions so we can indicate exceptional positions (invalid etc.)
	...
	picoos_uint16
		headxBufSize; /* actually allocated size (if one day headxBuf is allocated dynamically) */
	...
	picoos_uint8
		outBuf[PICODATA_BUFSIZE_DEFAULT]; /* internal output buffer to hold just one item */
	picoos_uint16
		outBufSize; /* actually allocated size (if one day outBuf is allocated dynamically) */
	...
	picoos_uint8 sylReadPos,
		sylWritePos; /* next pos to read from sylBuf, next pos to write to sylBuf */
	...
	picoos_uint16 phonReadPos,
		phonWritePos; /* next pos to read from phonBufIn, next pos to write to phonBufIn */

	picoos_int16
		activeStartPos; /* start position of items to be treated (at end of left context) */
	picoos_int16 penultima,
		activeEndPos; /* positions of last two bounds/words; SPHO_POS_INVALID means uninitialized */
	picoos_int16 lastPhraseBoundPos; /* position of the last bound encountered (<0 if inexistent or
										not reachable */
...
static pico_status_t sphoReset(register picodata_ProcessingUnit this) {
	...
	if (NULL == this || NULL == this->subObj) {
		return picoos_emRaiseException(this->common->em, PICO_ERR_NULLPTR_ACCESS, NULL, NULL);
	}
...
picodata_ProcessingUnit picospho_newSentPhoUnit(picoos_MemoryManager mm,
	...
	spho->altDescBuf = picotrns_allocate_alt_desc_buf(spho->common->mm, SPHO_MAX_ALTDESC_SIZE,
													  &spho->maxAltDescLen);
...
static picoos_int16 shift_range_left_1(spho_subobj_t* spho, picoos_int16* from, picoos_int16 to) {
	...
	picoos_uint16 c_i, c_j, c_diff, c_writePos, i, j, diff, writePos;
	i = to;
	j = *from;
	diff = j - i;
	...
	PICODBG_DEBUG(("shifting buffer region [%i,%i[ down to %i", *from, writePos, to));
	...
	*from = to;
	...
	/*  */
	PICODBG_DEBUG(("readPos,WritePos are now [%i,%i[, returning shift amount %i", *from,
				   spho->headxWritePos, diff));
	return diff;
}

static pico_status_t sphoAddPhoneme(register spho_subobj_t* spho,
									picoos_int16 pos,
									picoos_int16 sym) {
	picoos_uint8 plane, unshifted;
	/* just for debuging */
	unshifted = picotrns_unplane(sym, &plane);
	PICODBG_TRACE(("adding %i/%i (%c on plane %i) at phonBuf[%i]", pos, sym, unshifted, plane,
				   spho->phonWritePos));
...
/* return "syllable accent" (or prominence) symbol, given "word accent" symbol 'wacc' and stress
 * value (no=0, primary=1, secondary=2) */
static picoos_uint16 sphoGetSylAccent(register spho_subobj_t* spho,
									  picoos_uint8 wacc,
									  picoos_uint8 sylStress) {
	...
	switch (sylStress) {
		...
		case 2: /* secondary-stressed syllable gets no prominence or secondary stress prom. (4) */
			return (PICODATA_ACC0 == wacc) ? PICODATA_ACC0 : PICODATA_ACC4;
...
static pico_status_t sphoExtractPhonemes(register picodata_ProcessingUnit this,
										 ...
										 picoos_uint16 pos,
										 picoos_uint8 convertAccents,
										 picoos_uint8* suppressWB) {
	...
	/*
	 Items considered in a transduction are a BOUND or a WORDPHON item. its starting offset within
	 the headxBuf is given as 'pos'. Elements that go into the transduction receive "their" position
	 in the buffer.
	 */
	...
	switch (head->type) {
		case PICODATA_ITEM_BOUND:
			...
			fstSymbol = (PICODATA_ITEMINFO1_BOUND_SBEG == head->info1 ||
			...
			PICODBG_TRACE(
				("inserting phrase bound phoneme %c and setting suppresWB=1\n", fstSymbol));
		...
		case PICODATA_ITEM_WORDPHON:
			...
			(*suppressWB) = 0;
			/* for the time being, we force to use POS so we can transduce all fsts in a row without
			 * reconsulting the items */

			/* If 'convertAccents' then the accentuation is not directly encoded. It rather
			 * influences the mapping of the word accent symbol to the actual accent phoneme which
			 * is put after the syllable separator. */
			if (convertAccents) {
				PICODBG_TRACE(("converting accents"));
				/* extracting phonemes IN REVERSE order replacing syllable symbols with prominence
				 * symbols */
				...
				for (i = head->len; i > 0;) {
					...
					if (spho->primStressId == content[i]) {
						curStress = 1;
						PICODBG_DEBUG(
							("skipping primary stress at pos %i (in 1 .. %i)", i, head->len));
						continue; /* skip primary stress symbol */
					} else if (spho->secondStressId == content[i]) {
						curStress = 2;
						PICODBG_DEBUG(
							("skipping secondary stress at pos %i (in 1 .. %i)", i, head->len));
				...
				if (PICO_OK == rv) {
					/* bug 366: we position the "head" into the item header and not on the first
					 * phoneme because there might be no phonemes at all */
...
/* *readPos is the next position in phonBuf to be read, and *writePos is the first position not to
 * be read (may be outside buf). 'rangeEnd' is the first possym position outside the desired range.
 * Possible return values:
 * SPHO_POSSYM_OK            : 'pos' and 'sym' are set to the read possym, *readPos is advanced
 * SPHO_POSSYM_OUT_OF_RANGE  : pos is out of range. 'pos' is set to that of the read possym, 'sym'
 * is undefined SPHO_POSSYM_UNDERFLOW     : no more data in buf. 'pos' is set to
 * PICOTRNS_POS_INVALID,    'sym' is undefined SPHO_POSSYM_INVALID       : "strange" pos. 'pos' is
 * set to PICOTRNS_POS_INVALID,    'sym' is undefined
 */
static pico_status_t getNextPosSym(spho_subobj_t* spho,
								   picoos_int16* pos,
								   picoos_int16* sym,
								   picoos_int16 rangeEnd) {
	/* skip POS_IGNORE */
	while ((spho->phonReadPos < spho->phonWritePos) &&
		   (PICOTRNS_POS_IGNORE == spho->phonBuf[spho->phonReadPos].pos)) {
		PICODBG_DEBUG(
			("ignoring phone at spho->phonBuf[%i] because it has pos==IGNORE", spho->phonReadPos));
		spho->phonReadPos++;
	}
 ...
 *
 * Given the original bound strength 'orig' and the desired target strength 'target' (suggested by
 * fst), calculate the modified bound strength.
 *
...
static picoos_uint8 breakModifiedBoundStrength(picoos_uint8 orig,
											   picoos_uint16 time,
											   picoos_bool wasPrimary) {
	picoos_uint8 modified = (0 == time)	  ? PICODATA_ITEMINFO1_BOUND_PHR3
							: (50 < time) ? PICODATA_ITEMINFO1_BOUND_PHR1
										  : PICODATA_ITEMINFO1_BOUND_PHR2;
	switch (orig) {
		/* for word and phrase breaks, return 'modified', unless a non-silence gets time==0, in
		 * which case return no break (word break) */
		case PICODATA_ITEMINFO1_BOUND_PHR0:
			if (0 == time) {
...
static picoos_bool breakStateInterrupting(picodata_itemhead_t* head,
										  picoos_bool* breakBefore,
										  picoos_bool* breakAfter) {
...
static void putSideBoundToOutput(spho_subobj_t* spho) {
	...
	if ((sildur < 0) || (PICODATA_ITEMINFO1_BOUND_PHR0 == ohead.info1) ||
		(PICODATA_ITEMINFO1_BOUND_PHR3 == ohead.info1)) {
		PICODBG_DEBUG(
			("outputting a bound of strength '%c' and type '%c' without duration constraints",
			 ohead.info1, ohead.info2));
		ohead.len = 0;
	} else {
		picoos_uint32 pos = 0;
		...
		picoos_write_mem_pi_uint16(ocontent, &pos, sildur);
		PICODBG_DEBUG(
			("outputting a bound of strength '%c' and type '%c' with duration constraints [%i,%i]",
			 ohead.info1, ohead.info2, sildur, sildur));
		ohead.len = pos;
	}
	picodata_put_itemparts(&ohead, ocontent, ohead.len, spho->outBuf, spho->outBufSize, &clen);
	/* disable side bound */
 ...
 *
 * given the original bound strength 'orig_strength' and the fst-suggested bound strength
 * 'fst_strength' and possibly being in a pending break state, calculate the resulting bound
 * strength and set boundstrength and sildur of the current item (spho->headx[spho->outReadPos])
 * accordingly. if a boundstrength was set, also calculate the phrasetype and if necessary (and
 * reachable), modify the phrase type of the previous phrase boundary.
 *
 * @param spho
 * @param orig_strength
 * @param orig_type
 * @param fst_strength
 */
static void setSideBound(spho_subobj_t* spho,
						 picoos_uint8 orig_strength,
						 picoos_uint8 orig_type,
						 picoos_uint8 fst_strength) {
	...
	if (PICODATA_ITEMINFO1_NA == orig_strength) {
		...
		PICODBG_DEBUG(("setting bound strength to fst-suggested value %c (was %c)", strength,
					   spho->headx[spho->outReadPos].boundstrength, spho->breakTime));
	}

	/* insert modified bound according to pending break, if any */
	if (spho->breakPending) {
		/* the calculation is based on the fst-modified value (because this is what the customer
		 * wants to override)
		 */
		strength = breakModifiedBoundStrength(strength, spho->breakTime,
											  (PICODATA_ITEMINFO1_BOUND_PHR1 == orig_strength));
		PICODBG_DEBUG(("setting bound strength to break-imposed value %c (was %c) and time to %i",
					   strength, spho->headx[spho->outReadPos].boundstrength, spho->breakTime));
		spho->headx[spho->outReadPos].boundstrength = strength;
		spho->headx[spho->outReadPos].sildur = spho->breakTime;
		spho->breakPending = FALSE;
	}
	if (spho->headx[spho->outReadPos].boundstrength) {
		/* we did set a bound strength, possibly promoting or demoting a boundary; now set the
		 * phrase type possibly also changing the phrase type of the previous phrase bound
		 */
		...
		if (toPhrase) {
			...
			spho->headx[spho->lastPhraseBoundPos].phrasetype = spho->lastPhraseType;
...
static picodata_step_result_t sphoStep(register picodata_ProcessingUnit this,
	...
	*numBytesOutput = 0;
	while (1) { /* exit via return */
		PICODBG_INFO(("doing state %i, headxReadPos: %d, headxWritePos: %d", spho->procState,
					  spho->headxReadPos, spho->headxWritePos));
		...
		switch (spho->procState) {
			...
			case SPHO_STEPSTATE_COLLECT:
				...
				while ((PICO_OK == rv) && (remHeadxSize > 0) && (remCbufSize > 0)) {
					PICODBG_DEBUG(("COLLECT getting item at headxWritePos %i (remaining %i)",
								   spho->headxWritePos, remHeadxSize));
					rv = picodata_cbGetItem(this->cbIn, spho->tmpbuf, PICODATA_MAX_ITEMSIZE, &blen);
					if (PICO_OK == rv) {
						rv = picodata_get_itemparts(spho->tmpbuf, PICODATA_MAX_ITEMSIZE,
													&(spho->headx[spho->headxWritePos].head),
													&(spho->cbuf[spho->cbufWritePos]), remCbufSize,
													&blen);
						if (PICO_OK == rv) {
				...
				/* in normal circumstances, rv is either PICO_EOF (no more items in cbIn) or
				 * PICO_BUF_OVERFLOW (if no more items fit into headx) */
				...
				if (PICO_EOF == rv) {					/* there are no more items available */
					if (curPos < spho->headxWritePos) { /* we did get some new items */
						PICODBG_DEBUG(("COLLECT read %i items", spho->headxWritePos - curPos));
			...
			case SPHO_STEPSTATE_PROCESS_PARSE:
				...
				if (spho->headxReadPos >= spho->headxWritePos) {
					...
					if (spho->force) {
						...
						/* headx is full; we are forced to transduce before reaching the sentence
						 * end */
						spho->force = FALSE;
				...
				PICODBG_DEBUG(("PARSE looking at item %s",
							   picodata_head_to_string(&ihead, msgstr, SPHO_MSGSTR_SIZE)));
				/* treat header */
				if (PICODATA_ITEM_BOUND == ihead.type) {
					...
					if ((PICODATA_ITEMINFO1_BOUND_SEND == ihead.info1) ||
						...
						if (spho->sentenceStarted) {
							...
							spho->headxReadPos++;
							spho->activeEndPos = spho->headxReadPos;
							/* we may discard all information up to activeEndPos, after processing
							 * of last sentence part
							 */
							...
							if (PICODATA_ITEMINFO1_BOUND_TERM == ihead.info1) {
								...
								picodata_put_itemparts(&ihead, icontent, ihead.len, spho->outBuf,
								...
								spho->penultima = SPHO_POS_INVALID;
								PICODBG_ERROR(
									("PARSE found a sentence end without a sentence start; "
									 "eliminated"));
							}
						}
				...
				if ((PICODATA_ITEM_WORDPHON == ihead.type) || (PICODATA_ITEM_BOUND == ihead.type)) {
					...
					if (PICO_OK == rv) {
						...
						/* replace activeEndPos if the new item is a word, or activeEndPos was not
						 * set yet, or activeEndPos was a bound */
						if ((spho->activeStartPos <= spho->headxReadPos) &&
							...
							PICODBG_INFO(("PARSE found new activeEndPos: %i,%i -> %i,%i",
										  spho->penultima, spho->activeEndPos, spho->activeEndPos,
										  spho->headxReadPos));
							spho->penultima = spho->activeEndPos;
							spho->activeEndPos = spho->headxReadPos;
						}
						...
						/* phoneme buffer cannot take this item anymore;
						   if the phoneme buffer has some contents, we are forced to transduce
						   before reaching the sentence end else we skip the (too long word) */
						PICODBG_INFO(
							("PARSE returned from phoneme extraction with overflow, number of "
							 "phonemes in phonBuf: %i; forced to TRANSDUCE",
							 spho->phonWritePos));
						if ((SPHO_POS_INVALID == spho->activeEndPos) ||
						...
						PICODBG_ERROR(
							("PARSE returned from phoneme extraction with exception %i", rv));
						return (picodata_step_result_t)picoos_emRaiseException(
							this->common->em, PICO_ERR_OTHER, NULL, NULL);
					}
				} else {
				...
				PICODBG_INFO(("PARSE going to next item: %i -> %i", spho->headxReadPos,
							  spho->headxReadPos + 1));
				spho->headxReadPos++;
				break;
			...
			case SPHO_STEPSTATE_PROCESS_TRANSDUCE:
				...
				if (spho->curFst >= spho->numFsts) {
#if defined(PICO_DEBUG)
					{
						...
						PICOTRNS_PRINTSYMSEQ(this->voice->kbArray[PICOKNOW_KBID_DBG],
											 spho->phonBufOut, spho->phonWritePos);
						PICODBG_INFO_MSG(("\n"));
					}
#endif
				...
				/* transduce from phonBufIn to PhonBufOut */
				{
...
#if defined(PICO_DEBUG)
					{
						...
						PICOTRNS_PRINTSYMSEQ(this->voice->kbArray[PICOKNOW_KBID_DBG], spho->phonBuf,
											 spho->phonWritePos);
						PICODBG_INFO_MSG(("\n"));
					}
#endif
					rv = picotrns_transduce(spho->fst[spho->curFst], FALSE, picotrns_printSolution,
											spho->phonBuf, spho->phonWritePos, spho->phonBufOut,
											&spho->phonWritePos, 4 * PICOTRNS_MAX_NUM_POSSYM,
											spho->altDescBuf, spho->maxAltDescLen, &nrSteps);
					if (PICO_OK == rv) {
#if defined(PICO_DEBUG)
						{
							PICODBG_INFO_CTX();
							PICODBG_INFO_MSG(("result of transduction: (output symbols: %i)",
											  spho->phonWritePos));
							PICOTRNS_PRINTSYMSEQ(this->voice->kbArray[PICOKNOW_KBID_DBG],
												 spho->phonBufOut, spho->phonWritePos);
							PICODBG_INFO_MSG(("\n"));
			...
			case SPHO_STEPSTATE_PROCESS_BOUNDS:
				...
				while (spho->outReadPos < spho->activeStartPos) {
					...
					PICODBG_INFO(("in position %i, looking at item %s", spho->outReadPos,
								  picodata_head_to_string(&ihead, msgstr, SPHO_MSGSTR_SIZE)));
					if (PICODATA_ITEM_BOUND == ihead.type) {
				...
				spho->phonReadPos = 0;
				while (SPHO_POSSYM_OK ==
					   (rv = getNextPosSym(spho, &pos, &sym, spho->activeStartPos))) {
					/* ignore */
				}
				PICODBG_INFO(("skipping left context phones results in %s",
							  (SPHO_POSSYM_OUT_OF_RANGE == rv) ? "OUT_OF_RANGE"
							  : (SPHO_POSSYM_END == rv)		   ? "END"
															   : "OTHER"));

				/*
				 * Align input items with transduced phones and note bound stregth changes and break
				 * commands
				 */
				...
				while (spho->outReadPos < spho->activeEndPos) {
					...
					if ((PICODATA_ITEM_BOUND == ihead.type) ||
						 ...
						 */
						PICODBG_INFO(
							("got BOUND or WORDPHON item and expects corresponding phone"));
						rv = getNextPosSym(spho, &pos, &sym, nextInPos);
						if (SPHO_POSSYM_OK != rv) {
							PICODBG_ERROR(
								("unexpected symbol or unexpected end of phoneme list (%s)",
								 (SPHO_POSSYM_OUT_OF_RANGE == rv) ? "OUT_OF_RANGE"
								 : (SPHO_POSSYM_END == rv)		  ? "END"
																  : "OTHER"));
							return (picodata_step_result_t)picoos_emRaiseException(
						...
						/* insert modified bound according to transduction and possibly pending
						 * break */
						setSideBound(spho, orig_strength, orig_type, (picoos_uint8)sym);
						...
						if (spho->breakPending) {
							...
							spho->breakPending = TRUE;
							   ...
							   (PICODATA_ITEMINFO1_CMD_PLAY == ihead.info1)) {
						/* insert break of at least one ms */
						if (!spho->breakPending || (spho->breakTime <= 0)) {
							spho->breakTime = SPHO_SMALLEST_SIL_DUR;
							spho->breakPending = TRUE;
						}
						setSideBound(spho, PICODATA_ITEMINFO1_NA, PICODATA_ITEMINFO2_NA,
									 PICODATA_ITEMINFO1_NA);
			...
			case SPHO_STEPSTATE_PROCESS_RECOMB:
				...
				PICODBG_DEBUG(("RECOMB looking at item %s",
							   picodata_head_to_string(&ihead, msgstr, SPHO_MSGSTR_SIZE)));
				...
				if (nextInPos <=
					spho->activeStartPos) { /* we're in the (passive) left context. Just skip it */
					PICODBG_DEBUG(("RECOMB skipping item in the left context (%i <= %i)", nextInPos,
								   spho->activeStartPos));
					if (PICODATA_ITEM_BOUND == ihead.type) {
					...
					if (spho->headx[spho->outReadPos].boundstrength) {
						...
						spho->suppressRecombWordBound =
							TRUE; /* if word following, don't need word boundary */
						...
						/* i expect a word boundary symbol in this range unless a phrase boundary
						...
						if (spho->suppressRecombWordBound) {
							...
							PICODBG_DEBUG(
								("RECOMB got WORDPHON item and expects corresponding bound phone"));
							rv = getNextPosSym(spho, &pos, &sym, nextInPos);
							if (SPHO_POSSYM_OK != rv) {
								PICODBG_ERROR(
									("unexpected symbol or unexpected end of phoneme list"));
								return (picodata_step_result_t)picoos_emRaiseException(
									this->common->em, PICO_ERR_OTHER, NULL, NULL);
							}
						}
						...
						PICODBG_DEBUG(("RECOMB consuming item from inBuf %i -> %i",
									   spho->outReadPos, nextInPos));
						spho->outReadPos = nextInPos;
					} else {
						/* ***************** OTHER *********************/
			...
			case SPHO_STEPSTATE_PROCESS_SYL:
				...
				/* consume all transduced phonemes with pos in in the range
				 * [spho->outReadPos,nextInPos[ */
				PICODBG_DEBUG(("SYL"));
				...
				PICODBG_TRACE(("SYL expects accent at phonBuf[%i] = (%i,%i) (outReadPos=%i)",
							   spho->phonReadPos, spho->phonBuf[spho->phonReadPos].pos,
							   spho->phonBuf[spho->phonReadPos].sym, spho->outReadPos));
				rv = getNextPosSym(spho, &pos, &sym, nextInPos);
				if (SPHO_POSSYM_OK != rv) {
				...
				/* for the time being, we force to use POS so we can transduce all fsts in a row
				 * without reconsulting the items */
				PICODBG_TRACE(("SYL expects POS"));
				PICODBG_DEBUG(
					("SYL (2) treating item in inBuf range [%i,%i[", spho->outReadPos, nextInPos));
				rv = getNextPosSym(spho, &pos, &sym, nextInPos);
				if (SPHO_POSSYM_OK != rv) {
					PICODBG_ERROR(("unexpected symbol or unexpected end of phoneme list"));
					return (picodata_step_result_t)picoos_emRaiseException(
				...
				PICODBG_DEBUG(
					("SYL (3) treating item in inBuf range [%i,%i[", spho->outReadPos, nextInPos));
				/* get phonemes of that syllable; stop if syllable boundary or outside word */
				sylsym = (PICOKFST_PLANE_PHONEMES << 8) + spho->syllSepId;
				...
				while (SPHO_POSSYM_OK == (rv = getNextPosSym(spho, &pos, &sym, nextInPos)) &&
					...
					/*  */
					PICODBG_TRACE(
						("SYL adding phoneme to syllable: (pos %i,sym %i)[plane %i,sym %c]", pos,
						 sym, plane, sym & 0xFF));
					PICODBG_ASSERT((PICOKFST_PLANE_PHONEMES == plane));
				}
				PICODBG_DEBUG(
					("SYL (4) treating item in inBuf range [%i,%i[", spho->outReadPos, nextInPos));
				ohead.len = spho->sylWritePos;
				if (SPHO_POS_INVALID == rv) {
					PICODBG_ERROR(("unexpected symbol or unexpected end of phoneme list"));
					return (picodata_step_result_t)picoos_emRaiseException(
						this->common->em, PICO_WARN_INCOMPLETE, NULL, NULL);
				} else if ((SPHO_POSSYM_OUT_OF_RANGE == rv) || (SPHO_POSSYM_END == rv)) {
					PICODBG_DEBUG(
						("SYL arrived at end of word and/or end of phon buffer, go to next word"));
					spho->outReadPos = nextInPos; /* advance to next item */
					spho->feedFollowState =
						SPHO_STEPSTATE_PROCESS_RECOMB; /* go to RECOMB after feed */
				} else {
					PICODBG_ASSERT((sym == sylsym));
				}
				PICODBG_DEBUG(
					("SYL (5) treating item in inBuf range [%i,%i[", spho->outReadPos, nextInPos));

				if (ohead.len > 0) {
					/* prepare syllable output */
					picodata_put_itemparts(&ohead, spho->sylBuf, PICODATA_BUFSIZE_DEFAULT,
			...
			case SPHO_STEPSTATE_FEED:
				...
				rv = picodata_cbPutItem(this->cbOut, spho->outBuf, spho->outBufSize, &clen);

				PICODATA_INFO_ITEM(this->voice->kbArray[PICOKNOW_KBID_DBG],
								   (picoos_uint8*)"spho: ", spho->outBuf, spho->outBufSize);
			...
			case SPHO_STEPSTATE_SHIFT:
				...
				/* SHIFT                                                              */
				/* **********************************************************************/
				/* If there exists a valid penultima, it should replace any left context (from 0 to
				 * activeStartPos) else discard the current active range (from activeStartPos to
				 * activeEndPos), leaving the current left context intact. Often, PARSE would move
				 * activeStartPos to 0, so that there is no left context after the shift.
				 */
				...
				if (spho->penultima != SPHO_POS_INVALID) {
					picoos_int16 shift;
					/* set penultima as new left context and set activeStartPos to the shifted
					 * activeEndPos */
					PICODBG_DEBUG(("SHIFT shifting penultima from %i to 0", spho->penultima));
					shift = shift_range_left_1(spho, &spho->penultima, 0);
					if (shift < 0) {
						picoos_emRaiseException(this->common->em, PICO_ERR_OTHER, NULL, NULL);
						return PICODATA_PU_ERROR;
					}
					spho->activeStartPos = spho->activeEndPos - shift;
					...
					/* dummy comment */
					PICODBG_DEBUG(("SHIFT shift active end from %i to %i", spho->activeEndPos,
								   spho->activeStartPos));
